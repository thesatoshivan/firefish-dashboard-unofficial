...
<body style="margin:0">
    ...
    #ffd-root.hide-amounts #debt-stats { filter: blur(6px); user-select: none; pointer-events: none; }
    ...
    <div class="sec" id="s-to" style="display:none;flex-direction:column;gap:0">
      ...
      <!-- Vor Kredit -->
      <div class="tool-tab" id="tt-vor" style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;align-items:start">
        ...
        <div class="card">
          ...
          <div class="vor-field"><label>Kreditbetrag</label><input type="number" id="tl" placeholder="10000" oninput="d.ltv()"><select id="tc" onchange="d.ltv()"><option>EUR</option><option>CHF</option><option>CZK</option><option>PLN</option><option>USDC</option><option>USDT</option></select></div>
          <div class="vor-field"><label>Collateral (BTC)</label><input type="number" id="tc2" placeholder="0.25" step="0.001" oninput="d.ltv()"></div>
          <div class="rb" id="ltv-r" style="display:none">
            <div class="rr"><span class="rl">Collateral-Wert (USD)</span><span class="rv" id="rcol-usd">–</span></div>
            <div class="rr"><span class="rl">Collateral-Wert (Kreditwährung)</span><span class="rv" id="rcol-ccy">–</span></div>
            <div class="rr"><span class="rl">Aktueller LTV</span><span class="rv" id="rltv">–</span></div>
            <div class="rr"><span class="rl">MC 1 (73%) — BTC/USD</span><span class="rv wrn" id="rmc1">–</span></div>
            <div class="rr"><span class="rl">MC 2 (79%) — BTC/USD</span><span class="rv wrn" id="rmc2">–</span></div>
            <div class="rr"><span class="rl">MC 3 (86%) — BTC/USD</span><span class="rv err" id="rmc3">–</span></div>
            <div class="rr"><span class="rl">Liquidation (95%) — BTC/USD</span><span class="rv err" id="rliq">–</span></div>
        ...
        <div class="card">
          ...
          <div class="vor-field"><label>Kreditbetrag</label><input type="number" id="bel" placeholder="10000" oninput="d.be()"><select id="bec" onchange="d.be()"><option>EUR</option><option>CHF</option><option>CZK</option><option>PLN</option><option>USDC</option><option>USDT</option></select></div>
          ...
          <div class="vor-field"><label>BTC-Preis bei Aufnahme (USD)</label><input type="number" id="bebp" placeholder="80000" oninput="d.be()"></div>
          <div class="rb" id="be-r" style="display:none">
            <div class="rr"><span class="rl">Zinskosten (Kreditwährung)</span><span class="rv" id="becost">–</span></div>
            <div class="rr"><span class="rl">Zinskosten in USD</span><span class="rv" id="becostusd">–</span></div>
            <div class="rr"><span class="rl">BTC die du h&#228;ttest verkaufen m&#252;ssen</span><span class="rv" id="bebtc">–</span></div>
            <div class="rr"><span class="rl">Break-even BTC-Preis (USD)</span><span class="rv" id="bebep">–</span></div>
        ...
        <div class="card">
          ...
          <div class="vor-field"><label>Betrag</label><input type="number" id="cva" placeholder="10000" oninput="d.conv()"><select id="cvf" onchange="d.conv()"><option>EUR</option><option>CHF</option><option>CZK</option><option>PLN</option><option>USDC</option><option>USDT</option><option>USD</option><option>BTC</option></select></div>
          <div id="cv-r" style="display:none"><div class="ccyg" id="cv-grid"></div></div>
        </div>
      </div>

      <!-- Während Kredit -->
      <div class="tool-tab" id="tt-wae" style="display:none;grid-template-columns:1fr 1fr;gap:1rem;align-items:start">
        <div class="card">
      ...
      <!-- Nach Kredit -->
      <div class="tool-tab" id="tt-nach" style="display:none;grid-template-columns:1fr 1fr;gap:1rem;align-items:start">
        <div class="card">
      ...
      <!-- Powerlaw -->
      <div class="tool-tab" id="tt-pow" style="display:none;grid-template-columns:1fr 1fr;gap:1rem;align-items:start">
    ...
    window.d={
      ...
      toolTab:function(t,el){
        ...
        el.classList.add('on');
        document.querySelectorAll('#ffd-root .tool-tab').forEach(function(s){s.style.display='none';});
        var tab=g('tt-'+t);
      ...
      ov:function(){
        ...
        var intCcyStr=CC.filter(function(c){return c!=='BTC'&&c!=='SAT';}).map(function(c){return '<b>'+c+'</b> '+fmt(frU(totalInterestUSD,c),c);}).join(' · ');
        ...
        var debtByCcy=ccyBreakdown(function(l){return l.amount;});
        var dueByCcy=ccyBreakdown(function(l){return l.amount+l.amount*(l.rate/100)*(l.term/12);});
        var costByCcy=ccyBreakdown(function(l){return l.amount*(l.rate/100)*(l.term/12)+(l.feeBtc||0)*(R.BTC/(R[l.c]||1));});
        ...
        function grpCard(title,tiles,cols){
          return '<div class="card" style="padding:.75rem;display:flex;flex-direction:column">'+
            ...
            '<div style="display:grid;grid-template-columns:repeat('+(cols||3)+',1fr);gap:8px;flex:1;align-content:start">'+tiles+'</div>'+
          '</div>';
        }
        var grpPortfolio=
          ...
          '<div class="mc" style="margin:0"><span class="mc-lbl">Offene Schuld (USD)</span><span class="mc-val">'+(empty?dash:fmt(aDue,'USD'))+'</span><span class="mc-sub" style="font-size:10px;line-height:1.4">'+(empty?'&nbsp;':dueByCcy)+'</span></div>'+
          '<div class="mc" style="margin:0"><span class="mc-lbl">Collateral (aktiv)</span><span class="mc-val">'+(empty?dash:aC.toFixed(8)+' BTC')+'</span><span class="mc-sub" style="font-size:10px;line-height:1.4">'+(empty?'&nbsp;':CC.filter(function(c){return c!=='BTC'&&c!=='SAT';}).map(function(c){return '<b>'+c+'</b> '+fmt(frU(colVal,c),c);}).join(' &middot; '))+'</span></div>';
        var grpRisiko=
        ...
        var fmtAllCcy=function(usd){
          return CC.filter(function(c){return c!=='BTC'&&c!=='SAT';}).map(function(c){return fmt(frU(usd,c),c);}).join(' &middot; ');
        };
        var grpKosten=
          ...
          '<div class="mc" style="margin:0"><span class="mc-lbl">Monatl. Zinslast</span><span class="mc-val" style="font-size:15px;line-height:1.5">'+(empty?dash:fmtAllCcy(monthlyInterestUSD))+'</span><span class="mc-sub">'+(empty?'&nbsp;':'Richtwert p.m.')+'</span></div>'+
          '<div class="mc" style="margin:0"><span class="mc-lbl">Gesamtkosten</span><span class="mc-val" style="font-size:15px;line-height:1.5">'+(empty?dash:fmtAllCcy(totalInterestUSD+totalFeeUSD))+'</span><span class="mc-sub" style="font-size:10px;line-height:1.4">'+(empty?'&nbsp;':costByCcy)+'</span></div>';
...
function mc(t){var r=parseFloat(g('ber')?g('ber').value:0)||0;var tm=parseInt(g('bet')?g('bet').value:12)||12;var intU=toU(parseFloat(g('tl').value)||0,g('tc').value)*(r/100)*(tm/12);return((lU+intU)/(t/100))/col;}
      ...
      be:function(){
        ...
        var interest=l*(r/100)*(t/12),fee=parseFloat(g('befee').value)||0,feeUSD=fee*R.BTC,iU=toU(interest,c),lU=toU(l,c),dueUSD=lU+iU+feeUSD,sold=lU/bp,bep=bp*(dueUSD/lU);
        ...
        g('bebep').textContent='$ '+bep.toLocaleString('de-CH',{maximumFractionDigits:0});
        var v=g('be-v');
      ...
      stress:function(){
        ...
        mcRows.forEach(function(mc){
          h+='<tr style="background:var(--bg2)"><td style="font-size:11px;font-weight:600;color:'+mc.color+';white-space:nowrap">'+
      ...
      debtChart:function(ccy){
        ...
        var data=months.map(function(m,mi){
          var isPast=mi<ni,isFuture=mi>ni;
          return frU(loans.reduce(function(s,l){
            var lStart=new Date(l.start);
            var lEnd=aM(l.start,l.term);
            /* Loan active in this month? */
            if(lStart>m||lEnd<m)return s;
            if(isPast){
              /* Historical: only count if loan was truly active (started, not yet ended) */
              /* A closed loan might have ended early — use its end date, not status */
              /* We count it as active in past months within its scheduled term */
              return s+toU(l.amount,l.c);
            } else {
              /* Present and future: only count active loans */
      ...
      ch:function(){
        ...
        setTimeout(function(){
          ...
          var mcdC=mk('ch-mcd');
          if(mcdC&&act.length){
            var mcdVals=act.map(function(l){
              if(!l.col)return 0;
              var lU=toU(l.amount,l.c);
              var mc1Price=(toU(l.amount,l.c)+intU(l))/0.73/l.col; /* BTC price at MC1, no fees */
              var dist=((R.BTC-mc1Price)/R.BTC*100);
              return parseFloat(Math.max(0,dist).toFixed(1));
            });
          ...
          if(ganttEl&&all.length){
            var now=new Date();
            var minD=new Date(Math.min.apply(null,all.map(function(l){return new Date(l.start);})));
            var maxD=new Date(Math.max.apply(null,all.map(function(l){return aM(l.start,l.term);})));
            var total=maxD-minD||1;
            /* Build X-axis ticks & gridlines */
            var xTicks=(function(){
              var totalMonths=Math.round((maxD-minD)/(1000*60*60*24*30.44));
              var step=totalMonths<=6?1:totalMonths<=12?2:totalMonths<=24?3:totalMonths<=48?6:12;
              var ticks=[],lines=[];
      ...
      vorCcyChange:function(){
        ...
        ['mkb-btc','szb-btc','mkr-btc','btr-btc','ltvc-btc','alp-btc'].forEach(function(id){
      ...
      sx:function(){
        ...
        html+='<div class="mc"><span class="mc-lbl">Volumen</span><span class="mc-val">'+fmt(tU,'USD')+'</span>'+
        ...
        var feePaidFiatStr=fiatCCYs.map(function(c){return '<b>'+c+'</b> '+fmt(frU(totalFeePaid,c),c);}).join(' &middot; ');
        ...
        html+='<div class="mc"><span class="mc-lbl">Geb\u00fchren bezahlt (alle)</span><span class="mc-val">'+totalFeePaidBtc.toFixed(8)+' BTC</span><span class="mc-sub" style="font-size:12px;line-height:1.6">'+feePaidFiatStr+'</span></div>';
        html+=tile('Gesamtkosten (Zins+Geb\u00fchr)',fmt(totalInterestOut+totalFeePaid,'USD'),'aktive Kredite');
        html+=tile('Monatl. Zinslast / 100 USD',monthlyPer100.toFixed(2)+' USD','auf aktives Volumen');
        html+='</div>';
        ...
        var btcValLines=btcCcys.map(function(c){
          var v=aC*R.BTC/(R[c]||1);
          return '<span style="display:block;font-size:11px;color:var(--text3);line-height:1.6">'+fmt(v,c)+'</span>';
        }).join('');
        html+='<div class="mc"><span class="mc-lbl">BTC gebunden (aktiv)</span>'+
          ...
          (btcValLines?'<span class="mc-sub" style="margin-top:.35rem">'+btcValLines+'</span>':'')+
        '</div>';
      ...
      tl:function(){
        ...
        var sorted=loans.slice().sort(function(a,b){return new Date(a.start)-new Date(b.start);});
      ...
      (async function(){
        try{
          ...
          if(data&&data.bitcoin){
            if(data.bitcoin.usd){liveBtc=data.bitcoin.usd;R.BTC=data.bitcoin.usd;R.SAT=data.bitcoin.usd/1e8;}
            if(data.bitcoin.eur&&data.bitcoin.usd)R.EUR=data.bitcoin.usd/data.bitcoin.eur;
            if(data.bitcoin.chf&&data.bitcoin.usd)R.CHF=data.bitcoin.usd/data.bitcoin.chf;
            if(data.bitcoin.pln&&data.bitcoin.usd)R.PLN=data.bitcoin.usd/data.bitcoin.pln;
        ...
        /* Historische BTC-Preise im Hintergrund vorladen (Cache befüllen) */
        setTimeout(function(){d.prefetchHistPrices();},3000); /* Nach LTV-Chart starten */
      })();
    }

    if(typeof Chart!=='undefined'){
      if(typeof ChartDataLabels!=='undefined'){init();}
      else{var dl2=document.createElement('script');dl2.src='https://cdn.jsdelivr.net/npm/chartjs-plugin-datalabels@2.2.0/dist/chartjs-plugin-datalabels.min.js';dl2.onload=init;document.head.appendChild(dl2);}
    }
    else{var s=document.createElement('script');s.src='https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js';
      s.onload=function(){
        ...
        dl.onload=init;document.head.appendChild(dl);
      };document.head.appendChild(s);}
    /* Run immediately so alarm banner, pills and overview show on first paint regardless of Chart.js */
    try{syncStatus();pills();d.renderCsw();d.loans();d.checkAlarms();d.ov();d.updateHdrStats();}catch(e){}
