...
<body style="margin:0">
    ...
    #ffd-root.hide-amounts #abl-total-sub,
    #ffd-root.hide-amounts .abl-amt,
    ...
    #ffd-root button.sm { padding: 4px 10px; font-size: 12px; border-radius: 10px; }
    ...
    #ffd-root .due-windows-grid { display: grid; grid-template-columns: repeat(8,1fr); gap: 8px; }
    ...
    #ffd-root .alarm-banner { display: none; padding: 3px 10px; border-radius: 10px; margin: 0 0 0 auto; font-size: 11px; font-weight: 600; align-items: center; overflow: hidden; white-space: nowrap; flex: 0 1 auto; max-width: 100%; }
    /* Schwebender Tooltip für die Roll-Over-Timeline. Fixiert positioniert,
       weil die Balken in einem Container mit overflow:hidden sitzen — ein
       Kindelement würde dort abgeschnitten. */
    #ffd-root .tl-tip { display: none; position: fixed; z-index: 1200; max-width: 300px; background: var(--card-bg); border: 1px solid var(--border); border-radius: 10px; padding: 9px 12px; font-size: 11px; line-height: 1.65; color: var(--text2); box-shadow: 0 8px 24px rgba(0,0,0,.16); pointer-events: none; }
    #ffd-root .tl-tip b { color: var(--text); }
    #ffd-root .tl-tip .tl-tip-hd { font-size: 12px; font-weight: 700; color: var(--text); margin-bottom: 3px; }
    #ffd-root .tl-tip .tl-tip-row { display: flex; justify-content: space-between; gap: 14px; }
    #ffd-root .tl-tip .tl-tip-row span:first-child { color: var(--text4); }
    #ffd-root .tl-tip hr { border: none; border-top: 1px solid var(--border); margin: 5px 0; }
    #ffd-root.hide-amounts .tl-tip { filter: blur(6px); }
    #ffd-root .ffd-notices { display: flex; flex-direction: column; gap: 6px; margin: 0 0 1rem; }
    #ffd-root .ffd-notice { display: flex; align-items: center; gap: 8px; padding: .55rem .85rem; border-radius: 8px; font-size: 12px; font-weight: 600; background: var(--bg2); border: 1px solid var(--border); color: var(--text2); }
    #ffd-root .ffd-notice.warn { background: var(--warn-bg); border-color: var(--warn-border); color: var(--warn); }
    #ffd-root .alarm-banner-inner { display: inline-block; padding-right: 4rem; }
    #ffd-root .alarm-banner-inner.marquee { animation: ffd-marquee 18s linear infinite; }
    @keyframes ffd-marquee { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
    ...
    #ffd-root .ov-due-card { grid-column: span 2; }
    ...
    @media (max-width: 639px) {
      ...
      #ffd-root .due-windows-grid { grid-template-columns: 1fr; }
      #ffd-root .ccyg { grid-template-columns: 1fr; }
      #ffd-root .ov-due-card { grid-column: span 1; }
    ...
    <div class="sidebar">
      ...
      <button class="nav-item" onclick="d.tab('bk',this)">
        <svg class="nav-icon" viewBox="0 0 16 16" fill="currentColor"><path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm.93 8.588-.077.079c-.149.127-.37.179-.63.179-.519 0-.898-.26-.898-.618 0-.337.253-.557.76-.64l.845-.126v1.126zm2.007-1.3c0 1.155-.909 1.985-2.183 1.985-.483 0-.932-.097-1.27-.276l.124-.64c.333.177.719.267 1.108.267.627 0 1.016-.3 1.016-.782 0-.434-.297-.675-.935-.918-.839-.313-1.366-.74-1.366-1.487 0-.765.634-1.338 1.573-1.42V5.5h.563v.486c.396.033.666.127.857.24l-.148.603c-.198-.104-.497-.228-.879-.228-.582 0-.829.294-.829.607 0 .373.27.563.993.843.907.341 1.376.79 1.376 1.585zM5.5 9.5h-1v-5h1v5z"/></svg>
        <span>Portfolio</span>
      </button>
    ...
    <div class="main-hdr">
      <div class="main-hdr-row1">
        <div class="pills" id="pills-bar" style="flex-shrink:0">
          ...
          <span class="pill">BTC/USD<input type="number" id="p-btc" title="BTC/USD bearbeiten" oninput="d.rate()" onblur="d.rateBlur()" step="1"></span>
          <span class="pill">EUR/USD <input type="number" id="p-eur" title="EUR/USD bearbeiten" oninput="d.rate()" onblur="d.rateBlur()" step="0.001"></span>
          <span class="pill">CHF/USD <input type="number" id="p-chf" title="CHF/USD bearbeiten" oninput="d.rate()" onblur="d.rateBlur()" step="0.001"></span>
    ...
    <div class="tabs">
      ...
      <button class="tab" onclick="d.tab('bk',this)">&#8383; Portfolio</button>
    ...
    <div id="next-action-widget" style="margin-bottom:.75rem"></div>
      ...
      <div id="ov-ltv" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(min(340px,100%),1fr));gap:1rem"></div>
    </div>
    ...
    <div class="sec" id="s-lo">
      ...
      <div class="add-panel" id="add-panel" style="display:none">
        ...
        <div class="fg">
          ...
          <div class="ff"><label>R&#252;ckzahlungsdatum <span style="font-size:11px;font-weight:400;color:var(--text4)">(optional)</span></label><input type="date" id="fcd"><span style="font-size:11px;color:var(--text4);display:block;margin-top:3px">Nur bei abgeschlossenen Krediten. Leer = F&#228;lligkeitsdatum.</span></div>
          ...
          <div class="ff" style="grid-column:span 2">
            <label style="display:flex;align-items:center;gap:.45rem;font-weight:400;cursor:pointer;margin:0">
              <input type="checkbox" id="ftx" style="accent-color:var(--accent);width:15px;height:15px;flex-shrink:0">
              <span>Passende Portfolio-Transaktionen anlegen</span>
            </label>
            <span style="font-size:11px;color:var(--text4);display:block;margin-top:3px">Bucht das Collateral als Transfer aus dem Cold Storage und die Geb&#252;hr als Geb&#252;hr-Transaktion, beide auf das Startdatum. Bei einem bereits abgeschlossenen Kredit wird das Collateral zum F&#228;lligkeitsdatum wieder eingebucht.</span>
          </div>
    ...
    <div class="sec" id="s-to" style="display:none;flex-direction:column;gap:0">
      ...
      <div class="tool-tab on" id="tt-vor">
        ...
        <div class="card">
          ...
          <div class="tg2">
            ...
            <div class="vor-field"><label id="szb-fee-lbl">Bearbeitungsgeb&#252;hr (USD)</label><input type="text" id="szb-fee" placeholder="—" oninput="d.feeManual('szb-fee');d.szb()"><p class="note2" style="margin-top:2px">Bei 12 Monaten 1.5% des Kreditbetrags</p></div>
        ...
        <div class="card">
          ...
          <div class="tg2">
            <div class="vor-field"><label>Geb&#252;hren (BTC)</label><input type="text" id="mkr-fee" placeholder="—" readonly style="background:var(--bg2);color:var(--text3)"><p class="note2" style="margin-top:2px">Bei 12 Monaten 1.5% des Kreditbetrags</p></div>
        ...
        <div class="card">
          ...
          <div class="tg2">
            ...
            <div class="vor-field"><label id="mkk-fee-lbl">Bearbeitungsgeb&#252;hr (USD)</label><input type="text" id="mkk-fee" placeholder="—" oninput="d.feeManual('mkk-fee');d.mkk()"><p class="note2" style="margin-top:2px">Bei 12 Monaten 1.5% des Kreditbetrags</p></div>
        ...
        <div class="card">
          ...
          <div class="tg2">
            ...
            <div class="vor-field"><label>Zinssatz % p.a.</label><input type="number" id="tlr" placeholder="10" step="0.1" oninput="d.ltv()"></div>
            <div class="vor-field"><label>Laufzeit (Monate)</label><input type="number" id="tlt" placeholder="12" step="1" oninput="d.ltv()"></div>
          </div>
          <div id="ltv-r" style="display:none;margin-top:.75rem">
            <div class="tg2" style="gap:.5rem">
              ...
              <div style="padding:.6rem .75rem;background:var(--bg2);border-radius:8px;border:1px solid var(--border)">
                <div style="font-size:11px;color:var(--text4)" id="rmc1-lbl">MC 1 (73%) — BTC/USD</div>
                <div class="rv wrn" id="rmc1" style="font-size:14px;font-weight:700">–</div>
              </div>
              <div style="padding:.6rem .75rem;background:var(--bg2);border-radius:8px;border:1px solid var(--border)">
                <div style="font-size:11px;color:var(--text4)" id="rmc2-lbl">MC 2 (79%) — BTC/USD</div>
              ...
              <div style="padding:.6rem .75rem;background:var(--bg2);border-radius:8px;border:1px solid var(--border);grid-column:span 2">
                <div style="font-size:11px;color:var(--text4)" id="rmc3-lbl">MC 3 (86%) — BTC/USD</div>
      ...
      <div class="tool-tab" id="tt-wae">
        <div class="card">
          <span class="card-title">Abl&#246;se-Rechner &#8212; alle offenen Kredite zur&#252;ckzahlen</span>
          <p class="note2" style="margin-bottom:.75rem">Was kostet es, heute alle aktiven Kredite abzul&#246;sen, und wie viel Bitcoin kommt dabei aus den Sicherheiten zur&#252;ck? Gerechnet mit Kapital + Zinsen &#8212; die Bearbeitungsgeb&#252;hr wurde bereits bei Kreditaufnahme bezahlt.</p>
          <p class="note2" id="abl-empty" style="display:none">Keine aktiven Kredite vorhanden.</p>
          <div id="abl-pick-wrap" style="display:none;margin-bottom:.85rem">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:.5rem;flex-wrap:wrap;margin-bottom:.4rem">
              <span class="note2" style="margin:0">Ber&#252;cksichtigte Kredite</span>
              <span style="display:flex;gap:.35rem">
                <button class="sm" style="padding:2px 9px;font-size:11px" onclick="d.ablAll(true)">Alle</button>
                <button class="sm" style="padding:2px 9px;font-size:11px" onclick="d.ablAll(false)">Keine</button>
              </span>
            </div>
            <div id="abl-pick" style="display:flex;flex-wrap:wrap;gap:.4rem"></div>
          </div>
          <p class="note2" id="abl-none" style="display:none;color:var(--warn)">Kein Kredit ausgew&#228;hlt.</p>
          <div id="abl-r" style="display:none">
            <div class="rb">
              <div class="rr"><span class="rl">Offene Schulden <span id="abl-cnt" style="color:var(--text4);font-weight:400"></span></span><span class="rv" id="abl-due">–</span></div>
              <div class="rr"><span class="rl">Ben&#246;tigte Bitcoin zum aktuellen Kurs</span><span class="rv wrn" id="abl-need">–</span></div>
              <div class="rr"><span class="rl">Sicherheiten zur&#252;ck (alle Collaterals)</span><span class="rv ok" id="abl-col">–</span></div>
              <div class="rr"><span class="rl" id="abl-net-lbl">Verbleibend, wenn aus dem Collateral zur&#252;ckgezahlt wird</span><span class="rv" id="abl-net">–</span></div>
              <div class="rr"><span class="rl">Bitcoin-Bestand im Portfolio (Cold Storage)</span><span class="rv" id="abl-res">–</span></div>
              <div class="rr" style="border-top:1px solid var(--border);padding-top:.5rem;margin-top:.25rem"><span class="rl" style="font-weight:700">Gesamter Bitcoin-Bestand danach<br><span id="abl-total-sub" style="color:var(--text4);font-weight:400;font-size:11px"></span></span><span class="rv" id="abl-total" style="font-weight:700">–</span></div>
            </div>
            <p class="note2" style="margin:.75rem 0 0">R&#252;ckzahlungsbetrag umgerechnet</p>
            <div class="ccyg" id="abl-grid"></div>
            <div id="abl-percy-wrap" style="display:none">
              <p class="note2" style="margin:.75rem 0 0">Tats&#228;chlich geschuldet je Kreditw&#228;hrung &#8212; eine EUR-Schuld l&#228;sst sich nicht mit CHF begleichen</p>
              <div class="ccyg" id="abl-percy"></div>
            </div>
          </div>
        </div>
        <div class="card">
          ...
          <div class="rb" id="cn-r" style="display:none">
            ...
            <div class="rr"><span class="rl" id="cn75-lbl">Nachschuss um MC 1 (73%) zu vermeiden</span><span class="rv" id="cn75">–</span></div>
      ...
      <div class="tool-tab" id="tt-nach">
        <div class="card">
          ...
          <div class="tg2" style="margin-top:.25rem">
            ...
            <div class="vor-field"><label id="gvl-fee-lbl">Bearbeitungsgeb&#252;hr (USD, auto) <span id="gvl-fee-reset" onclick="d.gvlFeeReset()" style="display:none;font-size:11px;font-weight:400;color:var(--accent);cursor:pointer;margin-left:.4rem">↺ Auto</span><span id="gvl-fee-src" style="display:none;font-size:11px;font-weight:400;color:var(--text4);margin-left:.4rem">echte Geb&#252;hr des Kredits</span></label><input type="number" id="gvl-fee" placeholder="0" step="0.01" oninput="d.gvlFeeManual()"></div>
    ...
    <div class="sec" id="s-st">
      ...
      <div class="card" style="margin-bottom:1rem">
        ...
        <p class="note2" style="margin-bottom:.75rem"><span id="heat-desc">Zeigt bei welchem BTC-Preis jeder aktive Kredit die MC1-, MC3- und Liquidations-Schwelle (95%) erreicht.</span></p>
    ...
    <div class="modal-bg" id="edit-modal-bg" onclick="if(event.target===this)d.closeEdit()">
      <div class="modal">
        ...
        <div class="fg">
          ...
          <div class="ff"><label>R&#252;ckzahlungsdatum <span style="font-size:11px;font-weight:400;color:var(--text4)">(optional)</span></label><input type="date" id="ef-cd"><span style="font-size:11px;color:var(--text4);display:block;margin-top:3px">Nur bei abgeschlossenen Krediten. Leer = F&#228;lligkeitsdatum.</span></div>
          ...
          <div class="ff" style="grid-column:span 2">
            <label style="display:flex;align-items:center;gap:.45rem;font-weight:400;cursor:pointer;margin:0">
              <input type="checkbox" id="ef-tx" style="accent-color:var(--accent);width:15px;height:15px;flex-shrink:0">
              <span>Im Portfolio gebucht</span>
            </label>
            <span style="font-size:11px;color:var(--text4);display:block;margin-top:3px">Angehakt hält die Buchungen f&#252;r Collateral und Geb&#252;hr auf dem Stand des Kredits. Haken entfernen l&#246;scht sie wieder aus dem Ledger.</span>
          </div>
    ...
    <div class="sec" id="s-ch">
      ...
      <div class="tg2" style="gap:1rem;margin-bottom:1rem">
        ...
        <div class="card">
          <span class="card-title" id="ch-mcd-title">Margin-Call-Distanz (% BTC-Preisfall bis MC1)</span>
      ...
      <div class="card" style="margin-bottom:1rem">
        ...
        <p class="note2" style="margin-top:.4rem;font-size:11px">* Kein BTC-Startpreis hinterlegt — historischer Preis vom Kreditdatum wird via Coingecko geladen. &#8224; Preis nicht ermittelbar — ohne Startpreis gibt es keinen Break-even, der Balken bleibt leer.</p>
      </div>

      <!-- Row 4: Zinskosten-Vergleich + Effektivkosten in BTC -->
      <div class="tg2" style="gap:1rem;margin-bottom:1rem">
        <div class="card">
          <span class="card-title" id="ch-t-int">Zinskosten gesamt vs. bisher aufgelaufen (USD)</span>
      ...
      <div class="card" style="margin-bottom:1rem">
        <span class="card-title" id="ch-t-mint">Monatliche Zinslast — Verlauf über Zeit (USD)</span>
      ...
      <div class="tg2" style="gap:1rem;margin-bottom:1rem">
        <div class="card">
          <span class="card-title" id="ch-t-cf">Fällige Gesamtbeträge nächste 12 Monate — Kapital + Zinsen (USD)</span>
          <div class="ch-wrap" style="height:220px"><canvas id="ch-cf"></canvas></div>
        </div>
        <div class="card">
          <span class="card-title" id="ch-t-dcol">Schulden vs. Collateral-Wert (USD, 12 Monate)</span>
    ...
    <div class="sec" id="s-se">
      ...
      <div class="card" style="margin-bottom:1rem">
        ...
        <div class="ff" style="margin-top:1rem;max-width:320px">
          <label>Hauptw&#228;hrung</label>
          <select id="se-main-ccy"></select>
          <span class="note2" style="margin-top:4px;display:block">Wird in allen Anzeigen als prim&#228;re W&#228;hrung verwendet &#8212; Kennzahlen, Diagramme und Rechner. Ausw&#228;hlbar sind nur oben aktivierte W&#228;hrungen.</span>
        </div>
        ...
        <div class="fg" style="margin-top:.75rem">
          <div class="ff">
            ...
            <select id="se-default-tab">
              ...
              <option value="bk">Portfolio</option>
            </select>
          </div>
          <div class="ff">
      ...
      <div class="card" style="margin-bottom:1rem">
        ...
        <p class="note2" style="margin-bottom:.75rem">Alle Daten liegen ausschliesslich im Speicher dieses Browsers. Werden die Website-Daten gel&#246;scht &#8212; von Hand, durch eine Aufr&#228;umfunktion oder im privaten Modus &#8212; sind sie ohne Sicherung unwiederbringlich weg. Exportiere regelm&#228;ssig eine JSON-Datei.</p>
        <p class="note2" id="backup-hint" style="margin-bottom:.75rem"></p>
        <div class="fg" style="margin-top:.75rem">
          <div class="ff">
            <label>Erinnern nach (Tagen) <span style="font-size:11px;font-weight:400;color:var(--text4)">0 = aus</span></label>
            <input type="number" id="se-backup" min="0" max="365" step="1" placeholder="30">
          </div>
        </div>
        <div style="margin-top:.75rem">
          <button class="sm primary" onclick="d.seBackupSave()">&#10003; Speichern</button>
          <span id="se-backup-msg" style="display:none;margin-left:.75rem;font-size:12px;color:var(--ok)">&#10003; Gespeichert.</span>
        </div>
      </div>
    ...
    <div class="modal-bg" id="tu-modal-bg" onclick="if(event.target===this)d.closeTopup()">
      <div class="modal" style="max-width:440px">
        <div class="modal-title">
          <span>Collateral nachlegen</span>
          <button class="modal-close" onclick="d.closeTopup()">&#10005;</button>
        </div>
        <p id="tu-name" style="font-size:15px;font-weight:700;color:var(--text);margin:0 0 .85rem"></p>
        <div class="fg">
          <div class="ff"><label>Zus&#228;tzliches Collateral (BTC)</label><input type="number" id="tu-btc" placeholder="0.05" step="0.00000001" min="0" oninput="d.topupPreview()"></div>
          <div class="ff"><label>Datum</label><input type="date" id="tu-date" onchange="d.topupPreview()"></div>
        </div>
        <div class="rb" style="margin-top:.85rem">
          <div class="rr"><span class="rl">Collateral</span><span class="rv" id="tu-col">–</span></div>
          <div class="rr"><span class="rl">LTV</span><span class="rv" id="tu-ltv">–</span></div>
          <div class="rr"><span class="rl">Liquidationspreis</span><span class="rv" id="tu-liq">–</span></div>
          <div class="rr" style="border-top:1px solid var(--border);padding-top:.5rem;margin-top:.25rem"><span class="rl" style="font-weight:700">Verbesserung</span><span class="rv ok" id="tu-diff" style="font-weight:700">–</span></div>
        </div>
        <label style="display:flex;align-items:center;gap:.45rem;font-size:13px;cursor:pointer;margin:.9rem 0 0">
          <input type="checkbox" id="tu-tx" style="accent-color:var(--accent);width:15px;height:15px;flex-shrink:0">
          <span>Passende Portfolio-Transaktion anlegen</span>
        </label>
        <span id="tu-tx-hint" style="font-size:11px;color:var(--text4);display:block;margin-top:3px"></span>
        <div class="modal-actions" style="margin-top:1.1rem">
          <button class="sm" onclick="d.closeTopup()">Abbrechen</button>
          <button class="primary sm" onclick="d.confirmTopup()">Nachlegen</button>
        </div>
      </div>
    </div>
    ...
    <div class="modal-bg" id="del-modal-bg" onclick="if(event.target===this)d.closeDel()">
      <div class="modal" style="max-width:400px">
        ...
        <p id="del-modal-tx" style="display:none;font-size:12px;color:var(--warn);margin:0 0 .75rem"></p>
    ...
    ───────────────────────────────────────────────────────────────────────────── */
    function _ffdSetBanner(el,text){
      el.innerHTML='<span class="alarm-banner-inner" id="_ffd_bi">'+text+'</span>';
      requestAnimationFrame(function(){
        var sp=document.getElementById('_ffd_bi');
        if(sp&&sp.scrollWidth>el.clientWidth*1.05){
          sp.innerHTML=text+'      '+text;
          sp.classList.add('marquee');
        }
      });
    }
    ...
    /* Currency breakdown of a USD amount. The main currency comes first,
       because in several tiles this grid is the only place the amount is
       shown at all. */
    function usdGrid(ccys,usd){
      var m=(typeof cfg!=='undefined')?mainC():'USD';
      var list=ccys.filter(function(c){return c!=='BTC'&&c!=='SAT';});
      if(list.indexOf(m)>=0)list=[m].concat(list.filter(function(c){return c!==m;}));
      return cGrid(list.map(function(c){return {c:c,v:frU(usd,c)};}));
    }
    /* Same, but without the main currency — for sub-lines under a headline
       that already states the amount in the main currency. */
    function usdGridSub(ccys,usd){
      var m=(typeof cfg!=='undefined')?mainC():'USD';
      return cGrid(ccys.filter(function(c){return c!=='BTC'&&c!=='SAT'&&c!==m;}).map(function(c){return {c:c,v:frU(usd,c)};}));
    }
    var chD=null,chI=null,dCcy=null; /* resolved from cfg.mainCcy on first use */
    ...
    var ltvThresh=73; /* overwritten from cfg.ltvDisplay once settings load */
    var lFilter='active';
    var lCcyFilter='all';
    var lView='grid';
    /* Im Ablöse-Rechner abgewählte Kredite (nur zur Laufzeit — die Rechnung ist
       ein Gedankenspiel, keine Einstellung, die einen Neustart überdauern muss). */
    var ablOff={};
    /* ── Notices (storage/data problems the user must know about) ───────────── */
    var pendingNotices=[];
    function ffdNotice(msg,kind){
      var root=document.getElementById('ffd-root');
      if(!root){pendingNotices.push([msg,kind]);return;}
      var box=document.getElementById('ffd-notice-box');
      if(!box){
        box=document.createElement('div');
        box.id='ffd-notice-box';
        box.className='ffd-notices';
        /* In den Inhaltsbereich, nicht an die Wurzel: die Sidebar ist fixiert
           und läge sonst über der Meldung — sichtbar blieb nur ihr rechtes
           Ende. Betraf jede Meldung, auch die Speicher-Warnungen. */
        var main=root.querySelector('.main');
        if(main)main.insertBefore(box,main.firstChild);
        else root.insertBefore(box,root.firstChild);
      }
      var n=document.createElement('div');
      n.className='ffd-notice'+(kind==='warn'?' warn':'');
      var txt=document.createElement('span');
      txt.textContent=msg;
      var cl=document.createElement('button');
      cl.className='sm';cl.textContent='×';cl.style.marginLeft='auto';
      cl.onclick=function(){n.remove();};
      n.appendChild(txt);n.appendChild(cl);
      box.appendChild(n);
    }
    function flushNotices(){
      if(!pendingNotices.length)return;
      var p=pendingNotices.slice();pendingNotices=[];
      p.forEach(function(x){ffdNotice(x[0],x[1]);});
    }
    /* Persistence is blocked only for the key whose stored value is unreadable —
       never for the whole app, or one corrupt ledger would silently discard
       every later loan and settings change. */
    var storeBlocked={};
    var storeWarned={};
    function backupRaw(key,raw){
      try{
        /* One backup per key is enough — re-backing up on every page load would
           fill the origin quota with copies of the same broken value. */
        for(var i=0;i<localStorage.length;i++){
          if(String(localStorage.key(i)).indexOf(key+'_backup_')===0)return true;
        }
        localStorage.setItem(key+'_backup_'+new Date().getTime(),raw);
        return true;
      }catch(e){return false;}
    }
    function storeWrite(key,value,what){
      if(storeBlocked[key]){
        if(!storeWarned[key]){
          storeWarned[key]=true;
          ffdNotice('Änderungen an "'+what+'" werden nicht gespeichert, solange die vorhandenen Daten unlesbar sind. Bitte über Einstellungen → Zurücksetzen bereinigen oder einen Export einspielen.','warn');
        }
        return false;
      }
      try{localStorage.setItem(key,value);return true;}
      catch(e){
        ffdNotice('Speichern fehlgeschlagen ('+what+'): '+(e&&e.name==='QuotaExceededError'?'Speicher voll':'Browser verweigert das Speichern')+'. Änderungen gehen beim Neuladen verloren — bitte exportieren.','warn');
        return false;
      }
    }
    /* Let the user break out of a write lock deliberately (reset / import). */
    function storeUnblock(key){delete storeBlocked[key];delete storeWarned[key];}
    /* ── CSV parsing ─────────────────────────────────────────────────────────
       A plain split() breaks on quoted fields: a note containing the delimiter
       shifts every following column. This walks the line and honours RFC-4180
       style quoting ("" escapes a quote inside a quoted field). */
    /* Split a CSV document into records, honouring newlines inside quoted
       fields — splitting on '\n' first truncated any multi-line note and
       silently dropped the remainder as a short row. */
    ...
    /* Locale-tolerant number parsing. The hard part is that a single separator
       is ambiguous: "10,000" is ten thousand, but "0,234" is a fraction of a
       bitcoin. A group is only read as a thousands separator when the leading
       group is 1-3 digits, is not "0", and every following group is exactly 3
       digits. Pass plainDecimal=true for BTC columns, where a grouping
       separator never occurs and misreading 0.234 as 234 would be disastrous. */
    function csvNum(v,plainDecimal){
      if(v==null)return 0;
      ...
      if(lastC>=0&&lastD>=0){
        ...
        var only=lastC>=0?',':'.';
        var parts=s.split(only);
        var grouped=!plainDecimal&&parts.length>1&&
          /^\d{1,3}$/.test(parts[0])&&parts[0]!=='0'&&
          parts.slice(1).every(function(x){return /^\d{3}$/.test(x);});
        if(grouped)s=parts.join('');
        else dec=only;
      }
      if(dec===',')s=s.split(',').join('.');
      /* Malformed leftovers: keep only the last separator as the decimal mark */
      var bits=s.split('.');
      if(bits.length>2)s=bits.slice(0,-1).join('')+'.'+bits[bits.length-1];
      var n=parseFloat(s);
      if(!isFinite(n))return 0;
      return neg?-Math.abs(n):n;
    }
    /* ── Record normalisation — never let unvalidated data reach the renderer ── */
    function nStr(v){return typeof v==='string'?v:(v==null?'':String(v));}
    function nNum(v,def){var n=typeof v==='number'?v:parseFloat(v);return isFinite(n)?n:(def||0);}
    /* Normalise a date to YYYY-MM-DD, or null if it is not a real calendar
       date. Inventing today's date (the previous behaviour) silently moved a
       loan's whole schedule; a day-first guess like 03/15/2024 even produced
       the structurally invalid "2024-15-03", which no later load could repair. */
    function normDate(v){
      var t=nStr(v).trim();
      if(!t)return null;
      var y,mo,da,m;
      if((m=/^(\d{4})-(\d{1,2})-(\d{1,2})$/.exec(t))){y=+m[1];mo=+m[2];da=+m[3];}
      else if((m=/^(\d{1,2})[.\/](\d{1,2})[.\/](\d{4})$/.exec(t))){da=+m[1];mo=+m[2];y=+m[3];}
      else return null;
    ...
    function normLoan(l){
      ...
      var amount=nNum(l.amount,0),term=Math.round(nNum(l.term!=null?l.term:l.termMonths,12));
      var col=nNum(l.col!=null?l.col:l.collateralBtc,0),rate=nNum(l.rate,0);
      var start=normDate(nStr(l.start||l.startDate));
      if(!start)return null;
      if(!(amount>=0)||!isFinite(amount))amount=0;
      if(!(term>0)||!isFinite(term))term=12;
      if(!(col>=0)||!isFinite(col))col=0;
      if(!(rate>=0)||!isFinite(rate))rate=0;
      var c=nStr(l.c||l.currency||'EUR');
      if(ALL_CCYS.indexOf(c)<0)c='EUR';
      return {
        id:nStr(l.id)||uid(),
        name:nStr(l.name)||'Kredit',
        amount:amount,c:c,rate:rate,term:term,
        start:start,
        col:col,
        status:l.status==='closed'?'closed':'active',
        btcStart:l.btcStart!=null&&isFinite(nNum(l.btcStart,0))&&nNum(l.btcStart,0)>0?nNum(l.btcStart,0):null,
        feeBtc:Math.max(0,nNum(l.feeBtc,0)),
        /* Tatsächliches Rückzahlungsdatum — optional. Eine ungültige Angabe
           wird verworfen, dann gilt wieder das Fälligkeitsdatum. */
        closedDate:normDate(nStr(l.closedDate))||null,
        /* Nachschüsse als eigene Ereignisse. `col` bleibt die Gesamtsumme,
           damit jede bestehende LTV- und Liquidationsrechnung unberührt
           bleibt; die Liste sagt nur, wann welcher Teil dazukam. */
        topups:Array.isArray(l.topups)?l.topups.map(function(t){
          var btc=nNum(t&&t.btc,0),dt=normDate(nStr(t&&t.date));
          if(!(btc>0)||!isFinite(btc)||!dt)return null;
          return {id:nStr(t.id)||uid(),btc:btc,date:dt};
        }).filter(Boolean):[],
        chainId:l.chainId?nStr(l.chainId):null,
        note:nStr(l.note)
      };
    }
    /* Ursprünglich hinterlegtes Collateral: alles ausser den Nachschüssen */
    function baseCol(l){
      if(!l)return 0;
      var up=(l.topups||[]).reduce(function(s,t){return s+(t.btc||0);},0);
      return Math.max(0,parseFloat(((l.col||0)-up).toFixed(8)));
    }
    /* Effect of one transaction on the BTC balance. For a 'fee' entry the BTC
       amount already IS the fee, so the separate fee field must not be
       subtracted a second time. */
    function txDelta(t){
      var sign=(t.type==='buy'||t.type==='transfer_in'||t.type==='mining')?1:
               (t.type==='sell'||t.type==='transfer_out'||t.type==='fee')?-1:0;
      var extraFee=t.type==='fee'?0:(t.fee||0);
      return sign*t.btc-extraFee;
    }
    var TX_TYPES=['buy','sell','transfer_in','transfer_out','mining','fee'];
    function normTx(t){
      if(!t||typeof t!=='object')return null;
      var btc=nNum(t.btc,0);
      if(!(btc>0)||!isFinite(btc))return null;
      var date=nStr(t.date);
      if(!/^\d{4}-\d{2}-\d{2}/.test(date))return null;
      var o={
        id:nStr(t.id)||('tx'+new Date().getTime()+Math.random().toString(36).slice(2,6)),
        date:date,
        type:TX_TYPES.indexOf(t.type)>=0?t.type:'buy',
        btc:btc,
        price:Math.max(0,nNum(t.price,0)),
        fee:Math.max(0,nNum(t.fee,0)),
        note:nStr(t.note)
      };
      /* Verknüpfung zu einem Kredit — nur gesetzt bei automatisch gebuchten
         Einträgen. Ohne diese Felder verlöre ein Import die Zuordnung und die
         Buchungen blieben beim Bearbeiten oder Löschen des Kredits zurück. */
      var lid=nStr(t.loanId);
      if(lid){
        o.loanId=lid;
        var role=nStr(t.loanRole);
        /* Nachschüsse tragen eine Rolle je Ereignis («topup:<id>»), es gibt
           also keine feste Liste. Ohne diesen Zweig verlöre eine Nachschuss-
           Buchung beim Laden ihre Rolle, würde beim nächsten Abgleich nicht
           wiedererkannt und dabei gelöscht und neu angelegt. */
        if(['collateral','fee','return'].indexOf(role)>=0||/^topup:[A-Za-z0-9_-]+$/.test(role))o.loanRole=role;
      }
      return o;
    }
    var STORE_KEY='ffd_loans';
    function save(){storeWrite(STORE_KEY,JSON.stringify(loans),'Kredite');}
    function load(){
      var s=null;
      try{s=localStorage.getItem(STORE_KEY);}catch(e){}
      if(!s){loans=[];return;}
      var p=null;
      try{p=JSON.parse(s);}catch(e){p=null;}
      if(!Array.isArray(p)){
        /* Unreadable data: preserve it, tell the user, and stop all writes so the
           next save() cannot destroy a still-recoverable value. */
        storeBlocked[STORE_KEY]=true;
        var kept=backupRaw(STORE_KEY,s);
        ffdNotice('Die gespeicherten Kreditdaten konnten nicht gelesen werden. '+(kept?'Eine Sicherungskopie liegt im Browser-Speicher (ffd_loans_backup_…). ':'')+'Es wird nichts überschrieben — bitte Daten aus einem Export wiederherstellen.','warn');
        loans=[];
        return;
      }
      loans=p.map(normLoan).filter(Boolean);
      if(loans.length<p.length){
        /* Records we cannot interpret are kept out of the UI, but the stored
           value must survive so the user can export or repair it. */
        var lost=p.length-loans.length;
        storeBlocked[STORE_KEY]=true;
        backupRaw(STORE_KEY,s);
        ffdNotice(lost+' gespeicherte(r) Kredit(e) konnten nicht gelesen werden (z. B. ungültiges Startdatum). Sie werden nicht angezeigt, und es wird vorerst nichts überschrieben — eine Sicherungskopie liegt im Browser-Speicher.','warn');
      }
    }
    var TX_KEY='ffd_txs';
    var txs=[];
    function saveTxs(){storeWrite(TX_KEY,JSON.stringify(txs),'Transaktionen');}
    function loadTxs(){
      var s=null;
      try{s=localStorage.getItem(TX_KEY);}catch(e){}
      if(!s){txs=[];return;}
      var p=null;
      try{p=JSON.parse(s);}catch(e){p=null;}
      if(!Array.isArray(p)){
        storeBlocked[TX_KEY]=true;
        var kept=backupRaw(TX_KEY,s);
        ffdNotice('Die gespeicherten Transaktionen konnten nicht gelesen werden. '+(kept?'Eine Sicherungskopie liegt im Browser-Speicher (ffd_txs_backup_…). ':'')+'Es wird nichts überschrieben.','warn');
        txs=[];
        return;
      }
      txs=p.map(normTx).filter(Boolean);
      if(txs.length<p.length){
        var lostTx=p.length-txs.length;
        storeBlocked[TX_KEY]=true;
        backupRaw(TX_KEY,s);
        ffdNotice(lostTx+' gespeicherte Transaktion(en) konnten nicht gelesen werden. Sie werden nicht angezeigt, und der Verlauf wird vorerst nicht überschrieben.','warn');
      }
    }
        ...
        if(s){
          ...
          if(p&&p.ccys){
            return {
              ...
              btcReserve: p.btcReserve!=null?p.btcReserve:0,
              btcReserveManual: p.btcReserveManual||false,
              mainCcy: p.mainCcy||'USD',
              lastExportTs: p.lastExportTs!=null?p.lastExportTs:0,
              backupDays: p.backupDays!=null?p.backupDays:30
            };
          }
        }
      }catch(e){}
      return{ccys:['EUR','CHF','USD','BTC'],defaultTab:'ov',defaultView:'grid',defaultCcy:'EUR',ltvWarn:73,ltvCrit:79,ltvDanger:86,ltvDisplay:73,countdownDays:30,hideAmounts:false,hideBreakEven:false,navOrder:null,darkMode:false,btcReserve:0,btcReserveManual:false,mainCcy:'USD',lastExportTs:0,backupDays:30};
    }
    function saveSettings(cfg){storeWrite(SETTINGS_KEY,JSON.stringify(cfg),'Einstellungen');}
    var cfg=loadSettings();
    function visC(c){return cfg.ccys.indexOf(c)>=0;}
    /* Apply cfg defaults */
    ltvThresh=cfg.ltvDisplay!=null?cfg.ltvDisplay:73;
    lView=cfg.defaultView||'grid';

    function g(id){return document.getElementById(id);}
    /* Resolve a loan from a handler argument. Handlers used to carry only the
       array index, which goes stale as soon as an import or delete reorders
       `loans` — the price suggestion then wrote to the wrong loan. */
    function lRef(i,id){
      if(id){
        var m=loans.filter(function(x){return x.id===id;});
        if(m.length===1)return m[0];
      }
      return loans[i]||null;
    }
    /* Show an "empty" note for a chart WITHOUT removing its <canvas> — wiping
       the parent's innerHTML used to delete the canvas permanently. */
    function emptyChart(canvas,msg){
      if(!canvas)return;
      canvas.style.display='none';
      var note=canvas.parentElement&&canvas.parentElement.querySelector('.chart-empty');
      if(!note){
        note=document.createElement('p');
        note.className='note2 chart-empty';
        note.style.cssText='text-align:center;padding:2rem';
        if(canvas.parentElement)canvas.parentElement.appendChild(note);
      }
    ...
    /* HTML-escape — every user-controlled string must pass through this before
       being concatenated into innerHTML. */
    function esc(v){
    ...
    /* Escape for use inside a single-quoted JS string in an inline handler
       (which itself sits inside a double-quoted HTML attribute). The value is
       first escaped for JavaScript, then HTML-escaped: the parser decodes the
       entities back to exactly the JS-escaped text, so an entity that was
       already in the data (e.g. &#39;) can no longer turn into a real quote. */
    function escJs(v){
      var js=String(v==null?'':v)
        .replace(/\\/g,'\\\\')
        .replace(/'/g,"\\'")
        .replace(/[\r\n\u2028\u2029]/g,'');
      return esc(js);
    ...
    /* Rueckzahlung an Firefish: Kapital + Zinsen. Die Bearbeitungsgebuehr wird
       einmalig beim Matching in BTC bezahlt und ist am Ende nicht mehr faellig. */
    function dueU(l){return toU(l.amount,l.c)+intU(l);}
    /* Gesamtkosten des Kredits inkl. der einmaligen Bearbeitungsgebuehr —
       Basis fuer Break-even, nicht fuer den faelligen Betrag.
       Bei Krediten in Fremdwaehrung bleibt ein kleiner Rest: die Gebuehr ist
       in USD fixiert, der Kreditbetrag wird zum heutigen Devisenkurs
       umgerechnet. Der Break-even bewegt sich dadurch minimal mit EURUSD —
       um den Gebuehrenanteil (rund 1-2 %) mal der Kursaenderung. Exakt waere
       es nur mit dem Devisenkurs vom Kreditdatum, der nicht gespeichert ist. */
    function costU(l){return dueU(l)+feeU(l);}
    /* Debt basis for every LTV/margin-call calculation: principal + interest,
       fees excluded (see changelog v1.3.0). */
    function ltvU(l){return toU(l.amount,l.c)+intU(l);}
    function ltvPct(l){return l.col>0?(ltvU(l)/(l.col*R.BTC))*100:null;}
    /* Configured thresholds as fractions */
    function tW(){return (cfg.ltvWarn!=null?cfg.ltvWarn:73)/100;}
    function tC(){return (cfg.ltvCrit!=null?cfg.ltvCrit:79)/100;}
    function tD(){return (cfg.ltvDanger!=null?cfg.ltvDanger:86)/100;}
    /* Dieselben Schwellen als Prozentzahl fuer Beschriftungen und Markierungen */
    function tWp(){return tW()*100;}
    function tDp(){return tD()*100;}
    /* Parse a date-only string as LOCAL midnight — new Date('YYYY-MM-DD') is
       parsed as UTC and shifts the day for users west of UTC. */
    function pD(ds){
      if(ds instanceof Date)return new Date(ds.getTime());
      var m=/^(\d{4})-(\d{2})-(\d{2})/.exec(String(ds==null?'':ds));
      if(m)return new Date(+m[1],+m[2]-1,+m[3]);
      var d=new Date(ds);
      return isNaN(d.getTime())?new Date(NaN):d;
    }
    /* Add months, clamping to the last day of the target month —
       31.08. + 6 months is the end of February, not the 3rd of March. */
    function aM(ds,m){
      var d=pD(ds);
      ...
      t.setDate(day<dim?day:dim);
      return t;
    }
    function dL(ds,m){var e=aM(ds,m);if(isNaN(e.getTime()))return 0;var n=new Date();return Math.ceil((e-new Date(n.getFullYear(),n.getMonth(),n.getDate()))/864e5);}
    /* Auto-close loans whose end date has passed */
    function syncStatus(){
      var closedNow=[];
      loans.forEach(function(l){if(l.status==='active'&&dL(l.start,l.term)<0){l.status='closed';closedNow.push(l);}});
      /* Ein automatisch geschlossener Kredit hat sein Collateral zurück. Wer
         den Kredit ins Ledger gebucht hat, braucht die Rückbuchung auch dann,
         wenn er den Abschluss nicht selbst eingetragen hat. */
      closedNow.forEach(function(l){
        if(typeof d!=='undefined'&&d.loanTxs&&d.loanTxs(l.id).length)d.syncLoanTxs(l);
      });
    }
    ...
    function lc(v){return v<(cfg.ltvWarn!=null?cfg.ltvWarn:73)?'#16a34a':v<(cfg.ltvCrit!=null?cfg.ltvCrit:79)?'#d97706':v<(cfg.ltvDanger!=null?cfg.ltvDanger:86)?'#ea580c':'#dc2626';}
    function fmt(a,c){
      ...
      var s={EUR:'€',CHF:'Fr.',USD:'$',USDC:'USDC ',USDT:'USDT '}[c]||esc(c)+' ';
    ...
    function pills(){
      /* Never rewrite the field the user is currently typing into — doing so
         resets the caret and makes the rate pills untypeable. */
      var af=document.activeElement;
      var pbtc=g('p-btc');
      if(pbtc&&pbtc!==af) pbtc.value=Math.round(R.BTC);
      var peur=g('p-eur');if(peur&&peur!==af)peur.value=R.EUR.toFixed(4);
      var pchf=g('p-chf');if(pchf&&pchf!==af)pchf.value=R.CHF.toFixed(4);
      ...
      if(!el)return;
      /* If one of these inputs has focus, update the others in place instead of
         rebuilding the markup (which would drop focus mid-edit). */
      if(el.contains(af)){
        el.querySelectorAll('input[data-ccy]').forEach(function(inp){
          if(inp!==af)inp.value=Math.round(frU(R.BTC,inp.dataset.ccy));
        });
        return;
      }
      el.innerHTML=FIATS.filter(visC).map(function(c){
        var price=Math.round(frU(R.BTC,c));
        return '<span class="pill">BTC/'+labels[c]+'<input type="number" data-ccy="'+c+'" title="BTC/'+c+' bearbeiten" oninput="d.rateFiat(this)" onblur="d.rateBlur()" step="1" value="'+price+'" style="width:80px"></span>';
      }).join('');
    }
    /* ── Main display currency ───────────────────────────────────────────────
       All internal maths stays in USD; these helpers render a USD amount in
       whatever the user picked as their main display currency. */
    function mainC(){
      var m=cfg&&cfg.mainCcy;
      return (m&&ALL_CCYS.indexOf(m)>=0&&cfg.ccys.indexOf(m)>=0)?m:'USD';
    }
    function csym(c){
      return {EUR:'€',CHF:'Fr.',USD:'$',USDC:'USDC ',USDT:'USDT ',CZK:'Kč ',PLN:'zł '}[c]||(c+' ');
    }
    function fmtU(usd){var c=mainC();return fmt(frU(usd,c),c);}
    function faxU(usd){var c=mainC();return fax(frU(usd,c),c);}
    /* A BTC price given in USD, rendered in the main currency */
    function pU(usd){
      var c=mainC(),v=frU(usd,c);
      if(!isFinite(v))return '–';
      if(c==='BTC')return v.toFixed(8)+' ₿';
      if(c==='SAT')return Math.round(v).toLocaleString('de-CH')+' sats';
      return csym(c)+Math.round(v).toLocaleString('de-CH');
    }
    ...
    /* Re-render the tab that is currently open. Called after a rate change. */
    function renderActiveTab(){
      var at=document.querySelector('#ffd-root .tab.on, #ffd-root .nav-item.on');
      if(!at)return;
      var m=(at.getAttribute('onclick')||'').match(/'([^']+)'/);
      if(!m)return;
      var fn={ov:d.ov,lo:d.loans,st:d.stress,tl:d.tl,ca:d.cal,ch:d.ch,sx:d.sx,ro:d.ro,bk:d.bk,
        to:function(){d.vorFillAll();d.nachPopulate();d.nlpPopulate();d.se2Populate();d.gvlPopulate();}};
      if(fn[m[1]])fn[m[1]]();
    }
    /* Rate inputs fire on every keystroke; defer the expensive re-render so a
       half-typed price (8 → 85 → 850 …) cannot flash a false liquidation alarm. */
    var _rateT=null;
    var manualRateAt=0;
    function rateRerender(){
      if(_rateT)clearTimeout(_rateT);
      _rateT=setTimeout(function(){
        _rateT=null;
        pills();d.checkAlarms();d.updateHdrStats();renderActiveTab();
      },400);
    }
    window.d={
      rate:function(){
        ...
        manualRateAt=new Date().getTime();
        rateRerender();
      },
      /* Leaving a rate field re-syncs it with the value actually in use —
         otherwise a field cleared with Ctrl+A/Delete stayed blank for the rest
         of the session while every calculation still used the old rate. */
      rateBlur:function(){setTimeout(function(){pills();},0);},
      rateFiat:function(input){
        ...
        R.SAT=R.BTC/1e8;
        manualRateAt=new Date().getTime();
        var pb=g('p-btc');if(pb&&pb!==document.activeElement)pb.value=Math.round(R.BTC);
      ...
      updateHdrStats:function(){
        ...
        var dash='–';
        /* Clear every tooltip first — each is only refilled when its stat has a
           value, so a stale one would otherwise keep showing old figures. */
        ['hdr-bep-tooltip','hdr-col-tooltip','hdr-due-tooltip','hdr-mc1-tooltip','hdr-next-due-tooltip'].forEach(function(id){var t=g(id);if(t)t.innerHTML='';});
        /* Active count */
        ...
        if(ndEl){
          ...
          if(nd){
            ...
            var ndAmtStr=fmtU(ndAmt);
            ndEl.innerHTML='<span style="color:'+col+'">'+days+'d</span>'+
              ' <span style="font-weight:400;font-size:11px;color:var(--text3)">'+esc(nd.name)+'</span>';
        ...
        if(cvEl){
          ...
          cvEl.textContent=colUSD>0?(''+pU(Math.round(colUSD))):dash;
          if(ctEl&&colUSD>0){
            var CC=(['EUR','CHF','USD','USDT','USDC','CZK','PLN']).filter(visC).filter(function(c){return c!==mainC();});
        ...
        if(dueEl){
          ...
          dueEl.textContent=aDue>0?(''+pU(Math.round(aDue))):dash;
          if(dueBtcEl){dueBtcEl.textContent=aDue>0&&R.BTC>0?('₿'+(aDue/R.BTC).toFixed(4)):dash;}
          if(dueTtEl&&aDue>0){
            var CC2=(['EUR','CHF','USD','USDT','USDC','CZK','PLN']).filter(visC).filter(function(c){return c!==mainC();});
        ...
        if(mc1El){
          ...
          if(withCol.length){
            ...
            mc1El.innerHTML=
              ...
              ' <span style="font-weight:400;font-size:11px;color:var(--text3)">'+nearest.lbl+' ('+pU(Math.round(nearest.price))+') '+esc(nearest.name)+'</span>';
            if(mc1tEl){
              mc1tEl.innerHTML=loanItems.slice().sort(function(a,b){return a.dist-b.dist;}).map(function(x){
                ...
                return '<b>'+esc(x.name)+'</b> → '+x.lbl+' <span style="color:'+c+'">'+x.dist.toFixed(1)+'%</span> ('+pU(Math.round(x.price))+')';
              }).join('<br>');
            }
        ...
        if(bepEl){
          /* Ohne hinterlegten Startpreis gibt es keinen Break-even — der
             Live-Kurs als Ersatz wuerde eine Zahl liefern, die einfach dem
             Markt hinterherlaeuft. Solche Kredite bleiben aussen vor. */
          var bepSkipped=loans.filter(function(l){return !(l.btcStart>0);}).length;
          var bepLoans=loans.filter(function(l){return l.btcStart>0&&toU(l.amount,l.c)>0;});
          if(bepLoans.length){
            /* Halten statt verkaufen: btcStart x Gesamtkosten / Kreditbetrag —
               dieselbe Definition wie auf den Karten und in den Statistiken. */
            var bepItems=bepLoans.map(function(l){
              var lU=toU(l.amount,l.c);
              return {name:l.name,status:l.status,bep:l.btcStart*(costU(l)/lU),w:lU};
            });
            /* Portfolio break-even: nach Kreditvolumen gewichtet */
            var bepWDen=bepItems.reduce(function(s,x){return s+x.w;},0);
            var portfolioBep=bepItems.reduce(function(s,x){return s+x.bep*x.w;},0)/bepWDen;
            ...
            bepEl.innerHTML=
              ...
              ' <span style="font-weight:400;font-size:11px;color:var(--text3)">'+pU(Math.round(portfolioBep))+'</span>';
            if(bepTtEl){
              bepTtEl.innerHTML=bepItems.slice().sort(function(a,b){return a.bep-b.bep;}).map(function(x){
                ...
                return badge+'<b>'+esc(x.name)+'</b> <span style="color:'+c+'">'+(d>=0?'+':'')+d.toFixed(1)+'%</span> ('+pU(Math.round(x.bep))+')';
              }).join('<br>')+(bepSkipped?'<br><span style="color:var(--text4);font-size:10px">'+bepSkipped+' Kredit(e) ohne BTC-Startpreis nicht berücksichtigt</span>':'');
            }
          } else {
            bepEl.textContent=dash;
            if(bepTtEl)bepTtEl.innerHTML=loans.length?'<span style="color:var(--text4)">Kein Kredit hat einen BTC-Startpreis hinterlegt.</span>':'';
          }
        }
      },
      tab:function(t,el){
        ...
        sec.style.display=(t==='to')?'flex':(t==='bk'?'block':'grid');
        ...
        var fn={ov:d.ov,lo:d.loans,st:d.stress,ca:d.cal,tl:d.tl,ch:d.ch,sx:d.sx,se:d.se,ro:d.ro,to:d.vorInit,bk:d.bk,wae:function(){d.nachPopulate();d.nlpPopulate();d.se2Populate();d.gvlPopulate();d.nlpPopulate();},zukunft:function(){var el=g('zk-btc');if(el&&!el.value&&R.BTC)el.value=Math.round(R.BTC);d.zukunft();}};if(fn[t])fn[t]();
      },
      ...
      ov:function(){
        ...
        var portLtvColor=portLtv>=95?'#dc2626':lc(portLtv);
        /* Next due loan */
        ...
        var costByCcyGrid=ccyBreakdownGrid(function(l){return l.amount*(l.rate/100)*(l.term/12)+frU(feeU(l),l.c);});
        ...
        var ccyLtvParts=activeCcys.map(function(c){
          ...
          var col=ltv>=95?'var(--err)':ltv>=(cfg.ltvCrit||79)?'#ea580c':ltv>=(cfg.ltvWarn||73)?'var(--warn)':'var(--ok)';
          return '<b>'+c+'</b> <span style="color:'+col+'">'+ltv.toFixed(1)+'%</span>';
        }).filter(Boolean);
        var ltvSub=empty?'&nbsp;':(portLtv<(cfg.ltvWarn||73)?'✓ Sicher':portLtv<(cfg.ltvCrit||79)?'⚠ Beobachten':portLtv<(cfg.ltvDanger||86)?'⚠ MC2':portLtv<95?'⚠ MC3':'⚠ Kritisch');
        ...
        var grpPortfolio=
          ...
          '<div class="mc" style="margin:0"><span class="mc-lbl">Offene Schuld ('+mainC()+')</span><span class="mc-val">'+(empty?dash:fmtU(aDue))+'</span><span class="mc-sub" style="font-size:10px;line-height:1.4">'+(empty?'&nbsp;':dueByCcyGrid)+'</span></div>'+
        ...
        var grpRisiko=
          ...
          '<div class="mc" style="margin:0"><span class="mc-lbl">Nächster '+(empty?'MC':nextMClbl)+'</span><span class="mc-val">'+(empty?dash:(nextMC?'<span style="color:'+(nextMCdist<0?'var(--err)':nextMCdist<5?'var(--err)':nextMCdist<15?'#d97706':'inherit')+'">'+pU(Math.round(nextMC))+'</span>':'–'))+'</span><span class="mc-sub">'+(empty?'&nbsp;':(nextMCdist!==null?nextMCdist.toFixed(1)+'% Abstand':''))+'</span></div>'+
          (nearestLiq?
            '<div class="mc" style="margin:0"><span class="mc-lbl">Nächster Liq.-Preis</span>'+
            '<span class="mc-val"><span style="color:'+(nearestLiq.dist<0?'#dc2626':nearestLiq.dist<5?'#dc2626':nearestLiq.dist<15?'#d97706':'inherit')+'">'+pU(Math.round(nearestLiq.price))+'</span></span>'+
            '<span class="mc-sub">'+nearestLiq.dist.toFixed(1)+'% Abstand · '+esc(nearestLiq.name)+'</span>'+
            '</div>'
          :'');
        var grpKosten=
        ...
        var soonHtml=soonLoans.length?soonLoans.map(function(l){
          ...
          return '<div class="mc" style="margin:0">'+
            '<span class="mc-lbl">'+esc(l.name)+'</span>'+
        ...
        var grpDue=dueWindows.map(function(w){
          ...
          var valHtml=count===0
            ...
            :'<span style="color:'+accentClr+'">'+fmtU(totalUSD)+'</span>';
          return '<div class="mc" style="margin:0">'+
        ...
        /* Ballung: mehrere Fälligkeiten dicht beieinander. Die Fenster-Kacheln
           oben zeigen Summen, aber nicht, dass sich drei Kredite auf elf Tage
           drängen — genau das ist das Liquiditätsrisiko. */
        var clusterHtml=(function(){
          var WIN=14;
          var items=act.map(function(l){return {l:l,dl:dL(l.start,l.term),end:aM(l.start,l.term),due:toU(l.amount,l.c)+intU(l)};})
            .filter(function(x){return x.dl>=0&&!isNaN(x.end.getTime());})
            .sort(function(a,b){return a.end-b.end;});
          var best=null;
          for(var i=0;i<items.length;i++){
            var j=i,sum=0,n=0;
            while(j<items.length&&(items[j].end-items[i].end)<=WIN*864e5){sum+=items[j].due;n++;j++;}
            if(n>=2&&(!best||n>best.n||(n===best.n&&sum>best.sum)))
              best={n:n,sum:sum,from:items[i].end,to:items[j-1].end,dl:items[i].dl,
                    names:items.slice(i,j).map(function(x){return x.l.name;})};
          }
          if(!best)return '';
          var urgent=best.dl<=(cfg.countdownDays||30);
          var span=Math.round((best.to-best.from)/864e5);
          return '<div style="margin-top:.5rem;padding:.5rem .7rem;border-radius:8px;font-size:11px;line-height:1.5;'+
            'background:'+(urgent?'var(--warn-bg)':'var(--bg2)')+';border:1px solid '+(urgent?'var(--warn-border)':'var(--border)')+';color:'+(urgent?'var(--warn)':'var(--text3)')+'">'+
            (urgent?'⚠ ':'')+'<b>'+best.n+' Kredite</b> fällig innerhalb von '+(span===0?'einem Tag':span+' Tagen')+
            ' ('+best.from.toLocaleDateString('de-CH')+(span===0?'':' – '+best.to.toLocaleDateString('de-CH'))+') — <b>'+fmtU(best.sum)+'</b>'+
            '<span style="display:block;color:var(--text4);font-size:10px;margin-top:1px">'+esc(best.names.join(' · '))+'</span>'+
          '</div>';
        })();
        var dueTilesHtml=
          '<div class="card ov-due-card" style="padding:.75rem;display:flex;flex-direction:column">'+
            ...
            '<div class="due-windows-grid">'+grpDue+'</div>'+
            clusterHtml+
          '</div>';
        g('metrics').innerHTML=
        ...
        var filtered_ltv=act.filter(function(l){if(!l.col)return false;return ltvPct(l)>=ltvThresh;});
        g('ov-ltv').innerHTML=filtered_ltv.length?filtered_ltv.map(function(l){
          var cU=l.col*R.BTC,lU=toU(l.amount,l.c),due3=ltvU(l),ltv=(due3/cU)*100,w=Math.min(ltv,100),dl=dL(l.start,l.term);
          var interest=l.amount*(l.rate/100)*(l.term/12);
          var end=aM(l.start,l.term).toLocaleDateString('de-CH');
          var started=pD(l.start),now2=new Date(),totalMs=aM(l.start,l.term)-started;
          var pct=totalMs>0?Math.min(100,Math.max(0,((now2-started)/totalMs*100))).toFixed(1):100;
          var mc1P=due3/(tW()*l.col);
          var beDist=l.col>0?((R.BTC-mc1P)/R.BTC*100).toFixed(1):null;
          var beHtml=l.col>0?'<div style="font-size:11px;color:var(--text4);margin-top:.4rem">MC1 bei '+pU(Math.round(mc1P))+' ('+(beDist>0?'−'+beDist+'% vom aktuellen Preis':'⚠ überschritten')+')</div>':'';
          return '<div class="card">'+
            '<div class="lh"><span class="lid">'+esc(l.name)+' <span class="lid-sub">'+esc(l.id)+'</span></span><div class="fx" style="gap:4px"><span class="badge ba">Aktiv</span>'+(l.chainId?'<span class="badge" style="background:var(--accent-bg);color:var(--accent);border:1px solid var(--accent);cursor:pointer" onclick="goRo()">🔗 Roll-Over</span>':'')+' </div></div>'+
            ...
            '<div class="lmeta" style="grid-template-columns:1fr 1fr;margin-top:.4rem">'+
              '<div><span class="ll">Start</span><span class="lv">'+pD(l.start).toLocaleDateString('de-CH')+'</span></div>'+
            ...
            (l.status==='active'&&l.col>0?(function(){
              ...
              return '<div class="prog-row" style="margin-top:.4rem"><span>LTV</span><span style="color:'+lc(ltv)+';font-weight:600">'+ltv.toFixed(1)+'%</span></div>'+
                '<div class="prog-bg" style="background:var(--bg3);position:relative">'+
                  ...
                  '<div title="MC1 '+tWp()+'%" style="position:absolute;left:'+tWp()+'%;top:-3px;width:1px;height:10px;background:#d97706;opacity:.6"></div>'+
                  '<div title="MC3 '+tDp()+'%" style="position:absolute;left:'+tDp()+'%;top:-3px;width:1px;height:10px;background:#ea580c;opacity:.6"></div>'+
                  '<div title="Liquidation 95%" style="position:absolute;left:95%;top:-3px;width:1px;height:10px;background:#dc2626"></div>'+
                '</div>'+
                '<div style="position:relative;height:12px;font-size:10px;color:var(--border2);margin-top:2px">'+
                  '<span style="position:absolute;left:'+tWp()+'%;transform:translateX(-50%)">'+tWp()+'</span>'+
                  '<span style="position:absolute;left:'+tDp()+'%;transform:translateX(-50%)">'+tDp()+'</span>'+
            ...
            /* ⑧ Break-even */
            beHtml+
            (l.note?'<div style="margin-top:.6rem;padding:.45rem .65rem;border-radius:8px;background:var(--bg2);border:1px solid var(--border);font-size:12px;color:var(--text3)"><span style="font-weight:600;color:var(--text2)">&#128172; </span>'+esc(l.note)+'</div>':'')+
            '</div>';
      ...
      loansTable:function(sorted){
        ...
        el.innerHTML='<div class="ovx"><table class="loans-table">'+
          ...
          '<tbody>'+sorted.map(function(l){
            ...
            var dueAmount=l.amount+interest;
            ...
            return '<tr>'+
              '<td><div class="tbl-name">'+esc(l.name)+(l.chainId?' <span class="badge" title="Roll-Over" style="background:var(--accent-bg);color:var(--accent);border:1px solid var(--accent);cursor:pointer;font-size:11px;padding:1px 4px;vertical-align:middle" onclick="goRo()">🔗</span>':'')+'</div><div class="tbl-id">'+esc(l.id)+'</div></td>'+
              ...
              '<td><div class="tbl-actions">'+
                '<button class="sm" style="padding:2px 8px;font-size:12px" onclick="d.openEdit('+i+',\''+escJs(l.id)+'\')">&#9998;</button>'+
                '<button class="sm" style="padding:2px 8px;font-size:12px" onclick="d.openTopup('+i+',\''+escJs(l.id)+'\')" title="Collateral nachlegen">+&#8383;</button>'+'<button class="sm" style="padding:2px 8px;font-size:12px" onclick="d.dupLoan('+i+',\''+escJs(l.id)+'\')" title="Duplizieren">&#10064;</button>'+
                '<button class="del" onclick="d.openDel('+i+',\''+escJs(l.id)+'\')">&#10005;</button>'+
              '</div></td>'+
            '</tr>';
          }).join('')+
          '</tbody></table></div>';
      ...
      loans:function(){
        ...
        sorted.sort(function(a,b){
          var v=0;
          if(lSort.key==='start')v=pD(a.start)-pD(b.start);
          ...
          else if(lSort.key==='ltv'){var la=a.col>0?(ltvU(a)/(a.col*R.BTC)):0,lb2=b.col>0?(ltvU(b)/(b.col*R.BTC)):0;v=la-lb2;}
          ...
          else if(lSort.key==='liq'){var la2=a.col>0?ltvU(a)/(0.95*a.col):0,lb3=b.col>0?ltvU(b)/(0.95*b.col):0;v=la2-lb3;}
        ...
        el.innerHTML=sorted.map(function(l){
          var i=l._i;
          var dl=dL(l.start,l.term),end=aM(l.start,l.term).toLocaleDateString('de-CH');
          var pct=l.status==='closed'?100:(function(){var st=pD(l.start),en=aM(l.start,l.term),tot=en-st;return tot>0?Math.min(100,Math.max(0,Math.round((new Date()-st)/tot*100))):100;})();
          var interest=l.amount*(l.rate/100)*(l.term/12),lU=toU(l.amount,l.c);
          /* Break-even */
          var btcAtStart=l.btcStart||null;
          var bep=btcAtStart?btcAtStart*(costU(l)/lU):null;
          var beHtml='';
          if(cfg.hideBreakEven){
            ...
            beHtml='<div class="amt" style="display:flex;align-items:center;gap:8px;margin-top:.6rem;padding:.5rem .65rem;border-radius:8px;background:'+(won?'var(--ok-bg)':'var(--warn-bg)')+';border:1px solid '+(won?'var(--ok-border)':'var(--warn-border)')+'">'+
              ...
              '<div>'+
                ...
                '<div style="font-size:11px;color:var(--text3)">Break-even: <b>'+pU(bep)+'</b> · Aktuell: <b>'+pU(R.BTC)+'</b>'+(won?' · <span style="color:var(--ok)">▲ '+pU((R.BTC-bep))+' über Break-even</span>':' · <span style="color:var(--warn)">noch '+pU((bep-R.BTC))+' bis Break-even</span>')+'</div>'+
              '</div>'+
            '</div>';
          } else {
            var bsWid='bsw-'+l.id;
            beHtml='<div id="'+esc(bsWid)+'" style="margin-top:.6rem;padding:.5rem .65rem;border-radius:8px;background:var(--bg2);border:1px solid var(--border)">'+
              '<div style="display:flex;align-items:center;gap:8px;flex-wrap: wrap;">'+
                '<span style="font-size:14px">📊</span>'+
                '<span style="font-size:11px;color:var(--text3)">Kein BTC-Startpreis hinterlegt</span>'+
                '<button onclick="d.loadHistBtc('+i+',\''+escJs(l.id)+'\',\''+escJs(l.start)+'\',this)" style="font-size:11px;color:var(--accent);background:none;border:1px solid var(--accent);border-radius:4px;cursor:pointer;padding:1px 7px">Historischen Preis laden</button>'+
              '</div>'+
              '<div id="'+esc(bsWid)+'-suggest" style="display:none;margin-top:.5rem"></div>'+
            '</div>';
            setTimeout(function(li,ls,wid){return function(){d.loadHistBtc_auto(li,ls,wid);};}(i,l.start,bsWid),0);
          }
          return '<div class="card">'+
            /* ① Kopfzeile */
            '<div class="lh"><span class="lid">'+esc(l.name)+' <span class="lid-sub">'+esc(l.id)+'</span></span>'+
            ...
            '<button class="sm" style="padding:2px 8px;font-size:12px" onclick="d.openEdit('+i+',\''+escJs(l.id)+'\')">&#9998;</button>'+
            '<button class="sm" style="padding:2px 8px;font-size:12px" onclick="d.openTopup('+i+',\''+escJs(l.id)+'\')" title="Collateral nachlegen">+&#8383;</button>'+'<button class="sm" style="padding:2px 8px;font-size:12px" onclick="d.dupLoan('+i+',\''+escJs(l.id)+'\')" title="Duplizieren">&#10064;</button>'+
            '<button class="del" onclick="d.openDel('+i+',\''+escJs(l.id)+'\')">&#10005;</button></div></div>'+
            /* ② Kernzahlen 3-spaltig */
            '<div class="lmeta" style="grid-template-columns:1fr 1fr 1fr">'+
              '<div><span class="ll">Betrag</span><span class="lv">'+fmt(l.amount,l.c)+'</span></div>'+
              '<div><span class="ll">Fälliger Betrag</span><span class="lv">'+fmt(l.amount+interest,l.c)+'</span></div>'+
            ...
            (l.status==='active'&&l.col>0?(function(){
              ...
              return '<div class="prog-row" style="margin-top:.4rem"><span>LTV</span><span style="color:'+lc(ltv)+';font-weight:600">'+ltv.toFixed(1)+'%</span></div>'+
                '<div class="prog-bg" style="background:var(--bg3);position:relative">'+
                  ...
                  '<div title="Liquidation 95%" style="position:absolute;left:95%;top:-3px;width:1px;height:10px;background:#dc2626"></div>'+
                '</div>'+
                '<div style="position:relative;height:12px;font-size:10px;color:var(--text4);margin-top:2px">'+
                  '<span style="position:absolute;left:'+tWp()+'%;transform:translateX(-50%)">'+tWp()+'</span>'+
                  '<span style="position:absolute;left:'+tDp()+'%;transform:translateX(-50%)">'+tDp()+'</span>'+
                  '<span style="position:absolute;left:95%;transform:translateX(-50%)">95</span>'+
                '</div>';
      ...
      togAdd:function(){
        ...
        if(isOpen){
          ...
          var fcd=g('fcd');if(fcd)fcd.value='';
          /* Buchen ist eine bewusste Entscheidung — bei jedem Öffnen wieder aus */
          var ftx=g('ftx');if(ftx)ftx.checked=false;
      ...
      addLoan:function(){
        ...
        var nl={id:customId||uid(),name:g('fn').value||'Neuer Kredit',
          ...
          closedDate:normDate(g('fcd').value)||null,
        ...
        if(txBox&&txBox.checked)d.txFromLoan(nl);
      },
      /* Legt zu einem neuen Kredit die passenden Ledger-Buchungen an: das
         Collateral verlässt den frei verfügbaren Bestand (Transfer), die
         Bearbeitungsgebühr ist endgültig weg (Gebühr). Ein bereits
         abgeschlossener Kredit bekommt das Collateral zum Fälligkeitsdatum
         wieder eingebucht — sonst würde der Bestand dauerhaft zu klein
         bleiben, obwohl die Bitcoin längst zurück sind. */
      loanTxs:function(id){
        return id?txs.filter(function(t){return t.loanId===id;}):[];
      },
      /* Soll-Buchungen aus dem aktuellen Stand eines Kredits */
      loanTxPlan:function(l){
        if(!l)return [];
        /* Die Kredit-ID gehört in die Notiz: Namen lassen sich ändern und
           doppeln, und im Ledger steht die Buchung sonst ohne belastbaren
           Bezug zu ihrem Kredit da. */
        var name=(l.name||'Kredit')+(l.id?' ('+l.id+')':''),plan=[];
        /* Der Erstbetrag geht auf das Startdatum, jeder Nachschuss auf seinen
           eigenen Tag — sonst stünde später hinterlegtes Collateral im Ledger
           zu einem Zeitpunkt, an dem es den Bestand noch gar nicht verlassen hat. */
        plan.push({role:'collateral',type:'transfer_out',btc:baseCol(l),date:l.start,note:'Collateral — '+name});
        (l.topups||[]).forEach(function(t){
          plan.push({role:'topup:'+t.id,type:'transfer_out',btc:t.btc,date:t.date,
            note:'Collateral-Nachschuss — '+name});
        });
        plan.push({role:'fee',type:'fee',btc:l.feeBtc||0,date:l.start,note:'Bearbeitungsgebühr — '+name});
        if(l.status==='closed'&&l.col>0){
          /* Das eingetragene Rückzahlungsdatum gewinnt — nur wenn keines da
             ist, dient das Fälligkeitsdatum als Näherung. */
          var rd=l.closedDate||'';
          if(!rd){
            var end=aM(l.start,l.term);
            if(!isNaN(end.getTime()))rd=end.getFullYear()+'-'+String(end.getMonth()+1).padStart(2,'0')+'-'+String(end.getDate()).padStart(2,'0');
          }
          plan.push({role:'return',type:'transfer_in',btc:l.col,date:rd,
            note:'Collateral zurück — '+name});
        }
        return plan.filter(function(p){return p.btc>0&&/^\d{4}-\d{2}-\d{2}/.test(p.date||'');});
      },
      /* Bringt die Buchungen eines Kredits auf den aktuellen Stand. Bestehende
         werden aktualisiert statt gelöscht und neu angelegt — sonst verlören
         sie ihre ID und ihre Stellung im Ledger. Was nicht mehr zutrifft,
         fällt weg: etwa die Rückbuchung, wenn ein Kredit wieder aktiv wird. */
      syncLoanTxs:function(l){
        var res={added:0,updated:0,removed:0};
        if(!l||!l.id)return res;
        var plan=d.loanTxPlan(l),keep={};
        plan.forEach(function(p){
          keep[p.role]=1;
          var ex=txs.find(function(t){return t.loanId===l.id&&t.loanRole===p.role;});
          var btc=parseFloat(p.btc.toFixed(8)),price=l.btcStart||0;
          if(ex){
            if(ex.date!==p.date||ex.btc!==btc||ex.type!==p.type||ex.note!==p.note||ex.price!==price)res.updated++;
            ex.date=p.date;ex.type=p.type;ex.btc=btc;ex.note=p.note;ex.price=price;
          } else {
            txs.push({id:'tx'+new Date().getTime()+Math.random().toString(36).slice(2,6),
              date:p.date,type:p.type,btc:btc,price:price,fee:0,note:p.note,
              loanId:l.id,loanRole:p.role});
            res.added++;
          }
        });
        for(var i=txs.length-1;i>=0;i--){
          if(txs[i].loanId===l.id&&!keep[txs[i].loanRole]){txs.splice(i,1);res.removed++;}
        }
        if(res.added||res.updated||res.removed){
          txs.sort(function(a,b){return pD(a.date)-pD(b.date);});
          saveTxs();d.bkRecalcReserve();d.bk();
        }
        return res;
      },
      removeLoanTxs:function(id){
        if(!id)return 0;
        var n=0;
        for(var i=txs.length-1;i>=0;i--){if(txs[i].loanId===id){txs.splice(i,1);n++;}}
        if(n){saveTxs();d.bkRecalcReserve();d.bk();}
        return n;
      },
      txFromLoan:function(l){
        var r=d.syncLoanTxs(l);
        if(!r.added){
          ffdNotice('Keine Transaktion angelegt — der Kredit hat weder Collateral noch Gebühr.','warn');
          return;
        }
        ffdNotice(r.added+' Transaktion'+(r.added===1?'':'en')+' im Portfolio angelegt.');
      },
      openReset:function(mode){
        ...
        var descs={
          ...
          all:'Alle '+loans.length+' Kredite'+(txs.length?' und '+txs.length+' Transaktion(en)':'')+' werden gel\u00f6scht und alle Einstellungen auf die Standardwerte zur\u00fcckgesetzt.'
        };
      ...
      confirmReset:function(){
        ...
        var defaultCfg={ccys:['EUR','CHF','USD','BTC'],defaultTab:'ov',defaultView:'grid',defaultCcy:'EUR',ltvWarn:73,ltvCrit:79,ltvDanger:86,ltvDisplay:73,countdownDays:30,hideAmounts:false,hideBreakEven:false,navOrder:null,darkMode:false,btcReserve:0,btcReserveManual:false,mainCcy:'USD',lastExportTs:0,backupDays:30};
        if(mode==='loans'||mode==='all'){
          /* An explicit reset is the user's way out of a write lock */
          storeUnblock(STORE_KEY);
          loans.length=0;
          save();
        }
        if(mode==='all'){
      ...
      /* ─── Collateral nachlegen ─── */
      openTopup:function(i,id){
        var l=lRef(i,id);
        if(!l)return;
        var bg=g('tu-modal-bg');
        bg.dataset.idx=i;bg.dataset.lid=l.id||'';
        g('tu-name').textContent=l.name+' ('+fmt(l.amount,l.c)+', '+l.rate+'% p.a.)';
        g('tu-btc').value='';
        var n=new Date();
        g('tu-date').value=n.getFullYear()+'-'+String(n.getMonth()+1).padStart(2,'0')+'-'+String(n.getDate()).padStart(2,'0');
        /* Gebuchte Kredite buchen den Nachschuss standardmässig mit, sonst
           liefe das Ledger sofort auseinander. Ungebuchte bleiben ungebucht. */
        var booked=d.loanTxs(l.id).length>0;
        var box=g('tu-tx');if(box)box.checked=booked;
        var hint=g('tu-tx-hint');
        if(hint)hint.textContent=booked
          ? 'Der Kredit ist im Portfolio gebucht — der Nachschuss wird als Transfer auf das gewählte Datum ergänzt.'
          : 'Der Kredit ist nicht im Portfolio gebucht. Angehakt wird nur der Nachschuss gebucht.';
        d.topupPreview();
        bg.classList.add('open');
      },
      closeTopup:function(){g('tu-modal-bg').classList.remove('open');},
      topupPreview:function(){
        var bg=g('tu-modal-bg');
        var l=lRef(parseInt(bg.dataset.idx),bg.dataset.lid);
        if(!l)return;
        var add=parseFloat(g('tu-btc').value);
        if(!(add>0)||!isFinite(add))add=0;
        var due=ltvU(l);
        var oldCol=l.col||0,newCol=oldCol+add;
        function liq(c){return c>0?due/(0.95*c):null;}
        function ltv(c){return c>0?(due/(c*R.BTC))*100:null;}
        var lo=liq(oldCol),ln=liq(newCol),vo=ltv(oldCol),vn=ltv(newCol);
        g('tu-col').textContent=oldCol.toFixed(8)+' BTC'+(add>0?' → '+newCol.toFixed(8)+' BTC':'');
        g('tu-ltv').textContent=(vo!=null?vo.toFixed(1)+'%':'–')+(add>0&&vn!=null?' → '+vn.toFixed(1)+'%':'');
        var ltvEl=g('tu-ltv');ltvEl.style.color=(add>0?vn:vo)!=null?lc(add>0?vn:vo):'';
        g('tu-liq').textContent=(lo!=null?pU(lo):'–')+(add>0&&ln!=null?' → '+pU(ln):'');
        var diff=g('tu-diff');
        if(add>0&&lo!=null&&ln!=null){
          diff.textContent='Liquidation '+pU(lo-ln)+' tiefer · '+(((lo-ln)/lo)*100).toFixed(1)+'%';
        } else if(add>0&&ln!=null){
          diff.textContent='Liquidationspreis bei '+pU(ln);
        } else {
          diff.textContent='–';
        }
      },
      confirmTopup:function(){
        var bg=g('tu-modal-bg');
        var l=lRef(parseInt(bg.dataset.idx),bg.dataset.lid);
        if(!l){d.closeTopup();return;}
        var add=parseFloat(g('tu-btc').value);
        if(!(add>0)||!isFinite(add)){alert('Bitte einen Betrag grösser als 0 eingeben.');return;}
        var date=normDate(g('tu-date').value);
        if(!date){alert('Bitte ein gültiges Datum eingeben.');return;}
        if(!Array.isArray(l.topups))l.topups=[];
        l.topups.push({id:uid(),btc:parseFloat(add.toFixed(8)),date:date});
        l.col=parseFloat(((l.col||0)+add).toFixed(8));
        save();
        var box=g('tu-tx');
        if(box&&box.checked)d.syncLoanTxs(l);
        d.closeTopup();
        d.loans();d.ov();d.extPopulate();d.checkAlarms();d.updateHdrStats();
        ffdNotice(add.toFixed(8)+' BTC nachgelegt — Collateral jetzt '+l.col.toFixed(8)+' BTC.');
      },
      openDel:function(i,id){
        var l=lRef(i,id);
        if(!l)return;
        g('del-modal-name').textContent=l.name+' ('+fmt(l.amount,l.c)+', '+l.rate+'% p.a.)';
        /* Verknüpfte Buchungen verschwinden mit — das gehört vor die Bestätigung */
        var linked=d.loanTxs(l.id).length,txWarn=g('del-modal-tx');
        if(txWarn){
          txWarn.style.display=linked?'block':'none';
          txWarn.textContent=linked?'⚠ '+linked+' verknüpfte Portfolio-Transaktion'+(linked===1?'':'en')+' wird ebenfalls gelöscht.':'';
        }
        g('del-modal-bg').dataset.idx=i;
        g('del-modal-bg').dataset.lid=l.id||'';
        g('del-modal-bg').classList.add('open');
      },
      closeDel:function(){g('del-modal-bg').classList.remove('open');},
      confirmDel:function(){
        var l=lRef(parseInt(g('del-modal-bg').dataset.idx),g('del-modal-bg').dataset.lid);
        var i=loans.indexOf(l);
        if(i<0){d.closeDel();return;}
        var lid=l.id;
        loans.splice(i,1);
        save();
        var gone=d.removeLoanTxs(lid);
        d.closeDel();
        d.loans();d.ov();d.extPopulate();d.checkAlarms();
        if(gone)ffdNotice(gone+' verknüpfte Transaktion'+(gone===1?'':'en')+' mitgelöscht.');
      },
      openEdit:function(i,id){
        var l=lRef(i,id);
        if(!l)return;
        g('ef-n').value=l.name;
        g('ef-id').value=l.id||'';
        ...
        var efcd=g('ef-cd');if(efcd)efcd.value=l.closedDate||'';
        var eftx=g('ef-tx');if(eftx)eftx.checked=d.loanTxs(l.id).length>0;
        ...
        g('edit-modal-bg').dataset.lid=l.id||'';
        g('edit-modal-bg').classList.add('open');
      },
      closeEdit:function(){g('edit-modal-bg').classList.remove('open');},
      saveEdit:function(){
        var i=loans.indexOf(lRef(parseInt(g('edit-modal-bg').dataset.idx),g('edit-modal-bg').dataset.lid));
        if(i<0){d.closeEdit();return;}
        var l=loans[i];
        var newId=(g('ef-id').value||'').trim();
        if(newId&&newId!==l.id){
          /* Reject a duplicate id — the id-based handlers rely on uniqueness */
          if(loans.some(function(x){return x!==l&&x.id===newId;})){alert('Diese ID ist bereits vergeben.');return;}
          /* Verknüpfte Buchungen mitnehmen, sonst zeigen sie auf eine ID, die
             es nicht mehr gibt, und der Abgleich legt sie ein zweites Mal an. */
          var oldId=l.id,moved=0;
          txs.forEach(function(t){if(t.loanId===oldId){t.loanId=newId;moved++;}});
          /* Sofort sichern: der spätere Abgleich findet dann keine Änderung
             mehr und käme von sich aus nicht mehr zum Speichern. */
          if(moved)saveTxs();
          l.id=newId;
        }
        /* 0 is a legitimate value for amount, rate and collateral, so these
           cannot use `||` — a released collateral could never be set to 0. */
        var num=function(id,fallback){var v=parseFloat(g(id).value);return isFinite(v)&&v>=0?v:fallback;};
        l.name=g('ef-n').value||l.name;
        l.amount=num('ef-a',l.amount);
        l.rate=num('ef-r',l.rate);
        l.feeBtc=num('ef-fee',0);
        l.col=num('ef-b',l.col);
        var newStart=normDate(g('ef-d').value);
        if(g('ef-d').value&&!newStart){alert('Bitte ein gültiges Startdatum eingeben.');return;}
        l.start=newStart||l.start;
        l.term=parseInt(g('ef-t').value)||l.term;
        l.status=g('ef-s').value;
        var cdRaw=g('ef-cd').value;
        if(cdRaw&&!normDate(cdRaw)){alert('Bitte ein gültiges Rückzahlungsdatum eingeben.');return;}
        l.closedDate=normDate(cdRaw)||null;
        l.c=g('ef-c').value;
        ...
        if(chainSel==='__new__'){
          ...
          /* Adopt the predecessor's chain — if it has none yet, create one and
             tag the predecessor too, otherwise the "chain" would contain only
             this loan and the selection would not survive a re-open. */
          var pred=loans.find(function(x){return x.id===chainSel;});
          if(pred&&pred.chainId){l.chainId=pred.chainId;}
          else if(pred){var cid=uid();pred.chainId=cid;l.chainId=cid;}
          else {l.chainId=chainSel;}
        } else {
          delete l.chainId;
        }
        syncStatus();save();d.closeEdit();d.loans();d.ov();d.extPopulate();d.checkAlarms();
        /* Buchungen nachziehen. Angehakt heisst «soll gebucht sein» — ein
           entfernter Haken löscht sie wieder. */
        var efTxBox=g('ef-tx');
        if(efTxBox&&efTxBox.checked){
          var r=d.syncLoanTxs(l);
          if(!d.loanTxs(l.id).length)ffdNotice('Keine Transaktion angelegt — der Kredit hat weder Collateral noch Gebühr.','warn');
          else if(r.added||r.removed||r.updated)ffdNotice('Portfolio-Buchungen aktualisiert.');
        } else if(efTxBox){
          var gone=d.removeLoanTxs(l.id);
          if(gone)ffdNotice(gone+' verknüpfte Transaktion'+(gone===1?'':'en')+' aus dem Portfolio entfernt.');
        }
      },
      /* ─── Duplicate loan ─── */
      dupLoan:function(i,id){
        var src=lRef(i,id);
        if(!src)return;
        var copy=Object.assign({},src,{
          ...
          name:src.name+' (Kopie)',
          status:'active',
          /* A copy is a draft, not a member of the original's roll-over chain */
          chainId:null,
          /* Eigene Nachschuss-Einträge — sonst teilten sich Kopie und Original
             dieselbe Liste und ein Nachschuss beim einen träfe auch den anderen */
          topups:(src.topups||[]).map(function(t){return {id:uid(),btc:t.btc,date:t.date};})
        });
        loans.push(copy);
        syncStatus();
      ...
      checkCountdown:function(){
        ...
        el.innerHTML='<span style="font-size:11px;font-weight:600;color:var(--text3);display:block;width:100%;margin-bottom:.25rem">&#9203; Bald fällig</span>'+soon.map(function(l){
          ...
          return '<div class="'+chipCls+'">'+
            ...
            '<div class="cd-info">'+
              '<span class="cd-name" style="color:'+color+'">'+esc(l.name)+'</span>'+
      ...
      /* ─── CSV Export ─── */
      exportCSV:function(){
        var headers=['ID','Bezeichnung','Betrag','Währung','Zinssatz %','Gebühr BTC','Laufzeit Mo','Startdatum','Fälligkeitsdatum','Collateral BTC','Status','Rückzahlungsdatum','Zinsen (CCY)','Gebühr (BTC)','Fälliger Betrag (CCY)','Gebühr (USD)','LTV %','Liq.preis USD/BTC','BTC Startpreis','Roll-Over Kette','Notiz'];
        var rows=loans.map(function(l){
          var lU=toU(l.amount,l.c);
          var interest=l.amount*(l.rate/100)*(l.term/12);var feeBtcAmt=l.feeBtc||0;
          var ltvBase=ltvU(l);
          var liqP=l.col>0?ltvBase/(0.95*l.col):null;
          var ltv=ltvPct(l);
          ...
          return [
            ...
            q(l.start),q(endDate),q(l.col),q(l.status),q(l.closedDate||''),
      ...
      exportJSON:function(){
        var exportData={
          ...
          settings:{
            ...
            btcReserve:cfg.btcReserve||0,
            btcReserveManual:cfg.btcReserveManual||false,
            mainCcy:cfg.mainCcy||'USD',
            navOrder:cfg.navOrder||null,
            lastExportTs:cfg.lastExportTs||0,
            backupDays:cfg.backupDays!=null?cfg.backupDays:30
          },
          loans:loans.map(function(l){return{id:l.id,name:l.name,amount:l.amount,currency:l.c,rate:l.rate,termMonths:l.term,startDate:l.start,collateralBtc:l.col,status:l.status,closedDate:l.closedDate||null,topups:l.topups||[],btcStart:l.btcStart||null,feeBtc:l.feeBtc||0,chainId:l.chainId||null,note:l.note||''};}),
          transactions:txs
        };
        ...
        /* Zeitpunkt festhalten — localStorage kann jederzeit vom Browser
           geräumt werden, und ohne diesen Wert weiss niemand, wie alt die
           letzte Sicherung ist. */
        cfg.lastExportTs=new Date().getTime();
        saveSettings(cfg);
        d.renderBackupHint();
      },
      /* Alter der letzten Sicherung in Tagen, null wenn nie exportiert */
      backupAgeDays:function(){
        var ts=cfg&&cfg.lastExportTs;
        if(!ts||!isFinite(ts)||ts<=0)return null;
        return Math.floor((new Date().getTime()-ts)/864e5);
      },
      backupOverdue:function(){
        var lim=cfg&&cfg.backupDays!=null?cfg.backupDays:30;
        if(!(lim>0))return false;
        if(!loans.length&&!txs.length)return false;
        var age=d.backupAgeDays();
        return age===null||age>=lim;
      },
      renderBackupHint:function(){
        var el=g('backup-hint');if(!el)return;
        var lim=cfg&&cfg.backupDays!=null?cfg.backupDays:30;
        var age=d.backupAgeDays();
        if(!(lim>0)){
          el.innerHTML='<span style="color:var(--text4)">Erinnerung ausgeschaltet.</span>';
          return;
        }
        var over=d.backupOverdue();
        var txt=age===null?'Noch nie exportiert.':'Letzter Export vor '+age+' Tag'+(age===1?'':'en')+'.';
        el.innerHTML='<span style="color:'+(over?'var(--warn)':'var(--text3)')+'">'+(over?'⚠ ':'')+txt+'</span>'+
          (over?' <button class="sm" style="padding:2px 9px;font-size:11px;margin-left:6px" onclick="d.exportJSON()">Jetzt sichern</button>':'');
      },
      /* Einmal je Sitzung erinnern — bei jedem Seitenaufruf zu nerven wäre
         der schnellste Weg, dass die Meldung ignoriert wird. */
      backupRemind:function(){
        if(d._backupReminded)return;
        d._backupReminded=true;
        if(!d.backupOverdue())return;
        var age=d.backupAgeDays();
        ffdNotice((age===null
          ? 'Noch keine Sicherung vorhanden.'
          : 'Letzte Sicherung vor '+age+' Tagen.')+
          ' Die Daten liegen nur in diesem Browser — bitte über «↓ JSON» exportieren.','warn');
      },
      ...
      importCSV:function(input){
        ...
        reader.onload=function(e){
          ...
          try{
            ...
            var lines=splitCsvRecords(text.trim());
            if(lines.length<2)throw new Error('CSV ist leer oder hat keine Daten');
            /* Parse header */
            /* Delimiter detection — Firefish exports use ',', this app's own
               export uses ';'. Pick whichever appears more often in the header. */
            var delim=(lines[0].split(';').length>lines[0].split(',').length)?';':',';
            var hdr=splitCsvLine(lines[0],delim).map(function(h){return h.trim().toLowerCase();});
            /* Accept both the Firefish column names and this app's own export */
            var col=function(){
              for(var a=0;a<arguments.length;a++){
                var i=hdr.indexOf(arguments[a]);
                if(i>=0)return i;
              }
              return -1;
            };
            var iId=col('loan id','id');
            var iStart=col('start date (dd.mm.yyyy)','startdatum');
            var iMaturity=col('maturity date (dd.mm.yyyy)','fälligkeitsdatum','falligkeitsdatum');
            var iRate=col('interest rate (% p.a.)','zinssatz %');
            var iCcy=col('currency','währung','wahrung');
            var iAmount=col('loan amount','betrag');
            var iStatus=col('status');
            var iOrigFee=col('origination fee (sat)');
            var iFeeBtc=col('gebühr btc','gebuhr btc');
            var iTerm=col('laufzeit mo');
            var iCol=col('collateral sum (btc)','collateral btc');
            var iNote=col('note','notiz');
            var iName=col('bezeichnung');
            var iChain=col('roll-over kette');
            var iBtcStart=col('btc startpreis');
            var iClosed=col('rückzahlungsdatum','ruckzahlungsdatum');
            if(iId<0||iStart<0||iAmount<0)throw new Error('CSV-Format nicht erkannt. Bitte Firefish-Export oder einen Export dieses Dashboards verwenden.');
            /* Helper: parse dd.m.yyyy, dd.mm.yyyy or yyyy-mm-dd → yyyy-mm-dd */
            ...
            function calcTerm(startStr,endStr){
              var s=pD(startStr),e=pD(endStr);
              if(isNaN(s)||isNaN(e))return 12;
              var months=(e.getFullYear()-s.getFullYear())*12+(e.getMonth()-s.getMonth());
              /* Reine Monatsdifferenz verliert einen Monat, wenn das Enddatum
                 knapp vor dem Monatstag liegt (01.03.2024 -> 28.02.2025 waeren
                 11 statt 12 Monaten). Deshalb die Laufzeit nehmen, deren
                 berechnetes Enddatum dem tatsaechlichen am naechsten kommt. */
              var best=months,bestDiff=Infinity;
              for(var m=months-1;m<=months+1;m++){
                if(m<1)continue;
                var cand=aM(startStr,m);
                if(isNaN(cand.getTime()))continue;
                var diff=Math.abs(cand-e);
            ...
            for(var i=1;i<lines.length;i++){
              ...
              if(row.length<5){invalid++;continue;}
              var id=(row[iId]||'').trim();
              ...
              var ccyPre=(iCcy>=0?(row[iCcy]||'').trim():'EUR')||'EUR';
              var amount=csvNum(row[iAmount],ccyPre==='BTC'||ccyPre==='SAT');
              if(!id||!startDate||!amount){invalid++;continue;}
              if(strat==='skip'&&existingIds.indexOf(id)>=0){skipped++;continue;}
              var ccy=ccyPre;
              var rate=iRate>=0?csvNum(row[iRate],true):0;
              /* Prefer an explicit term column, else derive it from the dates */
              var term=iTerm>=0&&csvNum(row[iTerm])>0?Math.round(csvNum(row[iTerm])):calcTerm(startDate,maturityDate);
              var statusRaw=iStatus>=0?(row[iStatus]||'').trim().toUpperCase():'ACTIVE';
              /* Anything that is not explicitly running counts as closed —
                 REPAID/FINISHED used to be imported as active loans. */
              var status=(statusRaw==='ACTIVE'||statusRaw==='OPEN'||statusRaw==='RUNNING'||statusRaw==='')?'active':'closed';
              var feeBtcVal=iFeeBtc>=0?csvNum(row[iFeeBtc],true):(iOrigFee>=0?satToBtc(row[iOrigFee]):0);
              var colBtc=iCol>=0?csvNum(row[iCol],true):0;
              var note=iNote>=0?(row[iNote]||'').trim():'';
              /* Loan name: use the exported one, else currency + amount */
              var name=(iName>=0?(row[iName]||'').trim():'')||(ccy+' '+amount.toLocaleString('de-CH',{maximumFractionDigits:0}));
              var rec=normLoan({
                id:id,
                name:name,
                amount:amount,
                ...
                col:colBtc,
                status:status,
                closedDate:iClosed>=0?parseDate((row[iClosed]||'').trim()):null,
                btcStart:iBtcStart>=0?csvNum(row[iBtcStart])||null:null,
                feeBtc:feeBtcVal,
                chainId:iChain>=0?((row[iChain]||'').trim()||null):null,
                note:note
              });
              if(!rec){invalid++;continue;}
              /* Never create duplicate ids on merge */
              if(existingIds.indexOf(rec.id)>=0)rec.id=uid();
        ...
        reader.onload=function(e){
          ...
          try{
            ...
            loanArr.forEach(function(l){
              if(!l||typeof l!=='object'||(!(l.startDate)&&!(l.start))){rejected++;return;}
              /* Coerce and validate every field — unvalidated records break the
                 renderer permanently once they are persisted. */
              var rec=normLoan(l);
              if(!rec){rejected++;return;}
              if(strat==='skip'&&existingIds.indexOf(rec.id)>=0){skipped++;return;}
              /* Merge must not create duplicate ids — id-keyed lookups (chains,
                 delete, price suggestion) would resolve ambiguously. */
              if(existingIds.indexOf(rec.id)>=0)rec.id=uid();
              existingIds.push(rec.id);
              loans.push(rec);
              imported++;
            ...
            if(importedSettings&&typeof importedSettings==='object'){
              if(Array.isArray(importedSettings.preferredCurrencies)){
                var ccys=importedSettings.preferredCurrencies.filter(function(c){return ALL_CCYS.indexOf(c)>=0;});
                if(ccys.length)cfg.ccys=ccys;
              }
              var numSet=function(key,min,max){
                var v=nNum(importedSettings[key],NaN);
                if(isFinite(v)&&v>=min&&v<=max)cfg[key]=v;
              };
              if(importedSettings.defaultTab)cfg.defaultTab=nStr(importedSettings.defaultTab);
              ...
              if(!(cfg.ltvWarn<cfg.ltvCrit&&cfg.ltvCrit<cfg.ltvDanger)){
                cfg.ltvWarn=73;cfg.ltvCrit=79;cfg.ltvDanger=86;
                settingsNote+=' · LTV-Schwellen waren ungültig und wurden auf die Standardwerte gesetzt.';
              }
              if(importedSettings.ltvDisplay!=null){numSet('ltvDisplay',0,100);ltvThresh=cfg.ltvDisplay;}
              numSet('countdownDays',1,3650);numSet('btcReserve',-1e9,1e9);
              numSet('backupDays',0,365);numSet('lastExportTs',0,Number.MAX_SAFE_INTEGER);
              /* Navigationsreihenfolge: nur Zeichenketten übernehmen. Fehlende
                 Einträge ergänzt applyNavOrder() ohnehin, ein leeres Feld
                 bedeutet «Standardreihenfolge». */
              if(Array.isArray(importedSettings.navOrder)){
                var no=importedSettings.navOrder.filter(function(x){return typeof x==='string'&&x;});
                cfg.navOrder=no.length?no:null;
              } else if(importedSettings.navOrder===null){cfg.navOrder=null;}
              if(importedSettings.btcReserveManual!=null)cfg.btcReserveManual=!!importedSettings.btcReserveManual;
              ...
              if(importedSettings.darkMode!=null){cfg.darkMode=!!importedSettings.darkMode;var r=document.getElementById('ffd-root');if(cfg.darkMode){r.classList.add('dark');localStorage.setItem('ffd_dark','1');}else{r.classList.remove('dark');localStorage.setItem('ffd_dark','');}var db=g('dark-btn');if(db)db.textContent=cfg.darkMode?'☀':'☾';}
              saveSettings(cfg);
              lView=cfg.defaultView||'grid';
              d.renderCsw();d.applyNavOrder();
              settingsNote=' · Einstellungen importiert.'+settingsNote;
            }
            /* Transactions are independent of the settings block and follow the
               chosen merge strategy, like loans do. */
            if(strat==='replace'&&!(parsed&&Array.isArray(parsed.transactions))&&txs.length){txs.length=0;saveTxs();d.bkRecalcReserve();}
            var txNote='';
            if(parsed&&Array.isArray(parsed.transactions)){
              var incoming=parsed.transactions.map(normTx).filter(Boolean);
              var txBefore=txs.length;
              var txAdded=0;
              if(strat==='replace'){txs=incoming;txAdded=incoming.length;}
              else{
                var txIds=txs.map(function(t){return t.id;});
                incoming.forEach(function(t){
                  if(txIds.indexOf(t.id)>=0){if(strat==='skip')return;t.id=t.id+'-'+uid();}
                  txIds.push(t.id);txs.push(t);txAdded++;
                });
              }
              txs.sort(function(a,b){return pD(a.date)-pD(b.date);});
              saveTxs();
              d.bkRecalcReserve();
              if(g('bk-content')&&g('bk-content').innerHTML)d.bk();
              txNote=' · '+txAdded+' Transaktion(en)'+(incoming.length!==parsed.transactions.length?' ('+(parsed.transactions.length-incoming.length)+' ungültig)':'')+'.';
            }
            var stratLabel={'merge':'zusammengeführt','skip':'importiert ('+skipped+' Duplikat(e) übersprungen)','replace':'ersetzt'}[strat];
            if(rejected)settingsNote+=' · '+rejected+' ungültige(r) Datensatz/Datensätze übersprungen.';
            settingsNote+=txNote;
            syncStatus();save();d.loans();d.extPopulate();d.checkAlarms();d.ov();d.updateHdrStats();
            msg.style.display='block';
            msg.style.background='var(--ok-bg)';msg.style.color='var(--ok)';msg.style.border='1px solid var(--ok-border)';
      ...
      ltv:function(){
        ...
        /* Rate and term come from this card's own fields — they used to be read
           from the break-even calculator next to it, so editing that card moved
           these margin-call prices for no visible reason. */
        var lRate=parseFloat(g('tlr')?g('tlr').value:0)||0;
        var lTerm=parseInt(g('tlt')?g('tlt').value:0)||12;
        var lU=toU(l,c),cU=col*R.BTC;
        var intUv=lU*(lRate/100)*(lTerm/12);
        /* LTV includes interest, like every other LTV in the app */
        var ltv=((lU+intUv)/cU)*100;
        function mc(t){return((lU+intUv)/(t/100))/col;}
        g('ltv-r').style.display='block';
        g('rcol-usd').textContent=fmtU(cU);
        g('rcol-ccy').textContent=fmt(frU(cU,c),c);
        g('rltv').innerHTML='<span style="color:'+lc(ltv)+'">'+ltv.toFixed(2)+'%</span>';
        [['rmc1-lbl',cfg.ltvWarn!=null?cfg.ltvWarn:73,1],['rmc2-lbl',cfg.ltvCrit!=null?cfg.ltvCrit:79,2],['rmc3-lbl',cfg.ltvDanger!=null?cfg.ltvDanger:86,3]].forEach(function(x){
          var el=g(x[0]);if(el)el.textContent='MC '+x[2]+' ('+x[1]+'%) — BTC/USD';
        });
        g('rmc1').textContent=pU(mc(cfg.ltvWarn!=null?cfg.ltvWarn:73));
        g('rmc2').textContent=pU(mc(cfg.ltvCrit!=null?cfg.ltvCrit:79));
        g('rmc3').textContent=pU(mc(cfg.ltvDanger!=null?cfg.ltvDanger:86));
        g('rliq').textContent=pU(mc(95));
        var w=g('ltv-w');
        if(ltv>=(cfg.ltvDanger||86)){w.style.display='block';w.innerHTML='<span class="err">⚠ Kritisch: LTV nahe Liquidation!</span>';}
        else if(ltv>=(cfg.ltvWarn||73)){w.style.display='block';w.innerHTML='<span class="wrn">⚠ Margin Call Zone.</span>';}
      ...
      se2Populate:function(){
        ...
        sel.innerHTML='<option value="">\u2014 Kredit w\u00e4hlen \u2014</option>'+
          act.map(function(l){var i=loans.indexOf(l);return '<option value="'+i+'">'+esc(l.name)+' ('+fmt(l.amount,l.c)+')</option>';}).join('');
      ...
      /* Abgewählte Kredite im Ablöse-Rechner. Gemerkt wird die Ausnahme, nicht
         die Auswahl — so ist ein neu erfasster Kredit von sich aus dabei. */
      ablAll:function(on){
        loans.forEach(function(l){if(l.status==='active'){if(on)delete ablOff[l.id];else ablOff[l.id]=true;}});
        d.abl();
      },
      ablToggle:function(id){
        if(ablOff[id])delete ablOff[id];else ablOff[id]=true;
        d.abl();
      },
      /* ─── Ablöse-Rechner: offene Kredite auf einmal zurückzahlen ─── */
      abl:function(){
        var rEl=g('abl-r'),eEl=g('abl-empty'),pw=g('abl-pick-wrap'),nEl=g('abl-none');
        if(!rEl)return;
        var all=loans.filter(function(l){return l.status==='active';});
        if(!all.length){
          rEl.style.display='none';if(pw)pw.style.display='none';
          if(nEl)nEl.style.display='none';if(eEl)eEl.style.display='block';return;
        }
        if(eEl)eEl.style.display='none';
        /* Auswahlliste jedes Mal neu — der Zustand steht in ablOff, nicht im DOM */
        if(pw){
          pw.style.display='block';
          /* Nach Fälligkeit sortiert — mit sichtbarem Ablaufdatum ist das die
             Reihenfolge, in der man die Liste liest, und sie deckt sich mit den
             Kredit-Auswahlfeldern der übrigen Werkzeuge. */
          g('abl-pick').innerHTML=all.slice().sort(function(a,b){return aM(a.start,a.term)-aM(b.start,b.term);}).map(function(l){
            var on=!ablOff[l.id];
            var end=aM(l.start,l.term);
            var endStr=isNaN(end.getTime())?'—':end.toLocaleDateString('de-CH');
            return '<label style="display:inline-flex;align-items:center;gap:.35rem;font-size:12px;cursor:pointer;'+
              'background:var(--bg2);border:1px solid '+(on?'var(--accent)':'var(--border)')+';border-radius:8px;padding:.3rem .55rem">'+
              '<input type="checkbox" '+(on?'checked':'')+' style="accent-color:var(--accent);width:14px;height:14px" '+
              'onchange="d.ablToggle(\''+escJs(l.id)+'\')">'+
              '<span'+(on?'':' style="color:var(--text4)"')+'>'+esc(l.name)+
              ' <span class="abl-amt" style="color:var(--text4)">'+fmt(l.amount,l.c)+'</span>'+
              ' <span style="color:var(--text4)">· f&#228;llig '+endStr+'</span></span></label>';
          }).join('');
        }
        var act=all.filter(function(l){return !ablOff[l.id];});
        if(!act.length){
          rEl.style.display='none';if(nEl)nEl.style.display='block';return;
        }
        if(nEl)nEl.style.display='none';
        rEl.style.display='block';
        /* Rückzahlung = Kapital + Zinsen. Die Bearbeitungsgebühr ist bei der
           Kreditaufnahme in BTC bezahlt worden und wird nicht noch einmal fällig. */
        var dueUSD=act.reduce(function(s,l){return s+dueU(l);},0);
        var colBtc=act.reduce(function(s,l){return s+(l.col||0);},0);
        var needBtc=R.BTC>0?dueUSD/R.BTC:0;
        var netBtc=colBtc-needBtc;
        g('abl-cnt').textContent=act.length===all.length
          ? '('+act.length+' Kredit'+(act.length===1?'':'e')+')'
          : '('+act.length+' von '+all.length+' Krediten)';
        g('abl-due').textContent=fmtU(dueUSD);
        g('abl-need').textContent=needBtc.toFixed(8)+' BTC';
        g('abl-col').textContent=colBtc.toFixed(8)+' BTC · '+fmtU(colBtc*R.BTC);
        var netEl=g('abl-net'),netLbl=g('abl-net-lbl');
        netEl.textContent=(netBtc<0?'−':'')+Math.abs(netBtc).toFixed(8)+' BTC · '+fmtU(Math.abs(netBtc)*R.BTC);
        netEl.style.color=netBtc>=0?'var(--ok)':'var(--err)';
        if(netLbl)netLbl.textContent=netBtc>=0
          ? 'Verbleibend, wenn aus dem Collateral zurückgezahlt wird'
          : 'Fehlbetrag — das Collateral deckt die Schulden nicht';
        /* Gesamtbestand: was vom Collateral übrig bleibt plus die Bitcoin
           ausserhalb der Kredite. Der Fehlbetrag geht mit negativem Vorzeichen
           ein — dann zehrt die Rückzahlung am Cold Storage. */
        var resBtc=(cfg&&cfg.btcReserve)||0;
        var totalBtc=netBtc+resBtc;
        g('abl-res').textContent=resBtc>0?resBtc.toFixed(8)+' BTC · '+fmtU(resBtc*R.BTC):'—';
        var totEl=g('abl-total');
        totEl.textContent=(totalBtc<0?'−':'')+Math.abs(totalBtc).toFixed(8)+' BTC · '+fmtU(Math.abs(totalBtc)*R.BTC);
        totEl.style.color=totalBtc>=0?'var(--ok)':'var(--err)';
        var subEl=g('abl-total-sub');
        if(subEl)subEl.textContent=resBtc>0
          ? (netBtc<0?'−':'')+Math.abs(netBtc).toFixed(8)+' aus Collateral + '+resBtc.toFixed(8)+' Cold Storage'
          : 'kein Bitcoin-Bestand im Portfolio hinterlegt';
        /* Umrechnung in alle aktivierten Währungen, Hauptwährung zuerst */
        var m=mainC();
        var list=['EUR','CHF','USD','USDT','USDC','CZK','PLN','BTC','SAT'].filter(visC);
        if(list.indexOf(m)>=0)list=[m].concat(list.filter(function(c){return c!==m;}));
        g('abl-grid').innerHTML=list.map(function(c){
          return '<div class="ccyc"><span class="ccy-lbl">'+esc(c)+'</span><span class="ccy-val">'+fmt(frU(dueUSD,c),c)+'</span></div>';
        }).join('');
        /* Die umgerechnete Summe sagt nicht, was tatsächlich zu zahlen ist:
           Kredite laufen in verschiedenen Währungen. Deshalb zusätzlich die
           echte Schuld je Kreditwährung, sobald es mehr als eine gibt. */
        var byCcy={},order=[];
        act.forEach(function(l){
          if(order.indexOf(l.c)<0)order.push(l.c);
          byCcy[l.c]=(byCcy[l.c]||0)+l.amount+l.amount*(l.rate/100)*(l.term/12);
        });
        var wrap=g('abl-percy-wrap');
        if(order.length>1){
          wrap.style.display='block';
          g('abl-percy').innerHTML=order.map(function(c){
            return '<div class="ccyc"><span class="ccy-lbl">'+esc(c)+'</span><span class="ccy-val">'+fmt(byCcy[c],c)+'</span></div>';
          }).join('');
      ...
      nach:function(){
        ...
        g('cn75').textContent=nb(mc1t)>0?'+ '+nb(mc1t).toFixed(8)+' BTC':'✓ Kein Nachschuss nötig';
        g('cn95').textContent=nb(94.5)>0?'+ '+nb(94.5).toFixed(8)+' BTC':'✓ Kein Nachschuss nötig';
        var mc1lbl=g('cn75-lbl');if(mc1lbl)mc1lbl.textContent='Nachschuss um MC 1 ('+(cfg.ltvWarn!=null?cfg.ltvWarn:73)+'%) zu vermeiden';
      },
      be:function(){
        ...
        var interest=l*(r/100)*(t/12),fee=parseFloat(g('befee').value)||0,feeUSD=fee*bpUSD,iU=toU(interest,c),lU=toU(l,c),dueUSD=lU+iU+feeUSD,sold=lU/bpUSD,bep=bpUSD*(dueUSD/lU);
        g('be-r').style.display='block';
        g('becost').textContent=fmt(interest,c);g('becostusd').textContent=fmtU(iU);
      ...
      /* Tooltip-Inhalt für einen Balken der Roll-Over-Timeline */
      roTlTip:function(l,idx,total){
        function row(k,v){return '<div class="tl-tip-row"><span>'+k+'</span><span><b>'+v+'</b></span></div>';}
        var s=pD(l.start),e=aM(l.start,l.term);
        var act=l.status==='active';
        var dl=dL(l.start,l.term);
        var fee=feeU(l),int_=intU(l);
        var h='<div class="tl-tip-hd">'+esc(l.name)+'</div>'+
          '<div style="color:var(--text4);margin-bottom:2px">'+esc(l.id)+' · Roll-Over '+(idx+1)+' von '+total+
          ' · <span style="color:'+(act?'var(--ok)':'var(--text4)')+'">'+(act?'Aktiv':'Abgeschlossen')+'</span></div><hr>'+
          row('Betrag',fmt(l.amount,l.c))+
          row('Zinssatz',l.rate+'% p.a.')+
          row('Laufzeit',l.term+' Monate')+
          row('Zeitraum',s.toLocaleDateString('de-CH')+' – '+e.toLocaleDateString('de-CH'));
        if(act)h+=row('Fällig in',dl>0?dl+' Tag'+(dl===1?'':'en'):'überfällig');
        h+='<hr>'+
          row('Zinsen',fmtU(int_))+
          row('Gebühr',l.feeBtc?l.feeBtc+' BTC ≈ '+fmtU(fee):'–')+
          row('Gesamtkosten',fmtU(int_+fee))+
          row('Fälliger Betrag',fmt(frU(dueU(l),l.c),l.c));
        if(l.col>0){
          h+='<hr>'+row('Collateral',l.col.toFixed(8)+' BTC');
          var v=ltvPct(l);
          if(act&&v!=null){
            h+=row('LTV','<span style="color:'+lc(v)+'">'+v.toFixed(1)+'%</span>')+
              row('Liquidation',pU(ltvU(l)/(0.95*l.col)));
          }
        }
        return h;
      },
      /* Ein einziger schwebender Tooltip für alle Balken. Die Listener hängen
         am Container, nicht an den Balken — der Chart wird neu gezeichnet,
         seine Kinder also laufend ersetzt. */
      roTlBindTips:function(host){
        if(!host||host._tlTipBound)return;
        var root=document.getElementById('ffd-root');if(!root)return;
        var tip=document.getElementById('ffd-tl-tip');
        if(!tip){
          tip=document.createElement('div');
          tip.id='ffd-tl-tip';tip.className='tl-tip';
          root.appendChild(tip);
        }
        function hide(){tip.style.display='none';tip._src=null;}
        function show(ev){
          var t=ev.target;
          var bar=t&&t.closest?t.closest('[data-tip]'):null;
          if(!bar||!host.contains(bar)){hide();return;}
          if(tip._src!==bar){tip._src=bar;tip.innerHTML=bar.getAttribute('data-tip')||'';}
          tip.style.display='block';
          /* Erst einblenden, dann messen — sonst sind Breite und Höhe 0 und
             der Tooltip kippt am rechten Rand aus dem Bild. */
        ...
        /* Ohne Mauszeiger: Tippen zeigt den Tooltip, erneutes Tippen schliesst ihn */
        host.addEventListener('click',function(ev){
          var bar=ev.target&&ev.target.closest?ev.target.closest('[data-tip]'):null;
          if(bar&&tip._src===bar&&tip.style.display==='block')hide();else show(ev);
        });
        window.addEventListener('scroll',hide,true);
        host._tlTipBound=true;
      },
      renderHeatmap:function(){
        var hd=g('heat-desc');
        if(hd)hd.textContent='Zeigt bei welchem BTC-Preis jeder aktive Kredit die MC1- ('+(cfg.ltvWarn!=null?cfg.ltvWarn:73)+'%), MC3- ('+(cfg.ltvDanger!=null?cfg.ltvDanger:86)+'%) und Liquidations-Schwelle (95%) erreicht.';
        ...
        var rows=heatLoans.map(function(l){
          ...
          var cells=prices.map(function(p){
            ...
            else if(ltv>=(cfg.ltvDanger||86)){bg='#ea580c';fc='#fff';}
            else if(ltv>=(cfg.ltvWarn||73)){bg='#d97706';fc='#fff';}
          ...
          return '<div style="display:flex;align-items:stretch;margin-bottom:2px">'+
            '<div style="width:90px;flex-shrink:0;font-size:11px;font-weight:600;color:var(--text2);padding-right:6px;display:flex;align-items:center;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">'+esc(l.name)+'</div>'+
        ...
        var legend='<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:.75rem;font-size:10px">'+
          ...
          '<span style="display:flex;align-items:center;gap:4px"><span style="width:12px;height:12px;border-radius:2px;background:#d97706;display:inline-block"></span>MC1 \u2265'+tWp()+'%</span>'+
          '<span style="display:flex;align-items:center;gap:4px"><span style="width:12px;height:12px;border-radius:2px;background:#ea580c;display:inline-block"></span>MC3 \u2265'+tDp()+'%</span>'+
      ...
      stress:function(){
        var bp=R.BTC;g('st-btc').textContent=pU(bp);
        ...
        function pFmt(usd,l){return usd?''+pU(Math.round(usd)):'–';}
        var h='<div class="ovx"><table><thead><tr>'+
          '<th style="min-width:90px">Szenario</th>';
        act.forEach(function(l){
          h+='<th style="min-width:130px">'+esc(l.name)+
            '<br><span style="font-weight:400;color:var(--text4);font-size:10px">'+esc(l.c)+' · '+l.col+' BTC</span>'+
          '</th>';
        });
        h+='</tr></thead><tbody>';
        /* ── Aktueller LTV ── */
        ...
        var mcRows=[
          {label:'MC 1 ('+(cfg.ltvWarn!=null?cfg.ltvWarn:73)+'%)',thresh:(cfg.ltvWarn!=null?cfg.ltvWarn:73),color:'#d97706'},
          {label:'MC 2 ('+(cfg.ltvCrit!=null?cfg.ltvCrit:79)+'%)',thresh:(cfg.ltvCrit!=null?cfg.ltvCrit:79),color:'#ea580c'},
          {label:'MC 3 ('+(cfg.ltvDanger!=null?cfg.ltvDanger:86)+'%)',thresh:(cfg.ltvDanger!=null?cfg.ltvDanger:86),color:'#dc2626'},
        ...
        sc.forEach(function(p){
          ...
          h+='<tr><td style="color:'+(p<=-50?'#dc2626':'#d97706')+';font-weight:600;font-size:12px;white-space:nowrap">'+
            p+'%<br><span style="font-size:10px;font-weight:400;color:var(--text3)">'+pU(Math.round(pp))+'</span></td>';
          act.forEach(function(l){
            var v=l.col>0?stDue(l)/(l.col*pp)*100:0;
            var lb=v>=95?'LIQ':v>=(cfg.ltvDanger||86)?'MC3':v>=(cfg.ltvCrit||79)?'MC2':v>=(cfg.ltvWarn||73)?'MC1':'OK';
            var ok=lb==='OK';
      ...
      cal:function(){
        ...
        var evts={};
        loans.forEach(function(l){var e=aM(l.start,l.term);if(e.getFullYear()===calY&&e.getMonth()===calM){var dd=e.getDate();if(!evts[dd])evts[dd]=[];evts[dd].push({name:l.name,urg:(function(){var n=dL(l.start,l.term);return l.status==='active'&&n>=0&&n<=14;})()});}});
        ...
        for(var day=1;day<=dim;day++){
          ...
          if(evts[day])evts[day].forEach(function(ev){cell.innerHTML+='<span class="ce'+(ev.urg?' due':'')+'">'+esc(ev.name)+'</span>';});
        ...
        g('upcoming').innerHTML=up.length?up.map(function(l){
          ...
          return '<div class="card" style="padding:.75rem 1rem">'+
            '<div class="fxsb"><span style="font-size:13px;font-weight:600;color:var(--text)">'+esc(l.name)+' <span style="font-weight:400;color:#9ca3af">'+esc(l.c)+'</span></span>'+
            ...
            '<p class="note amt" style="margin-top:2px">= '+fmtU(lU)+' / '+fmt(frU(lU,'CHF'),'CHF')+' / '+fmt(frU(lU,'EUR'),'EUR')+'</p>'+
          '</div>';
      ...
      /* Toggle the charts tab's empty state WITHOUT replacing the section's
         markup — doing that deleted every <canvas> in it, so the charts never
         came back after the first loan was added (until a full page reload). */
      chEmpty:function(on){
        var sec=g('ch-content')||g('s-ch');
        if(!sec)return;
        var note=g('ch-empty-note');
        if(!note){
          note=document.createElement('div');
          note.id='ch-empty-note';
          note.className='empty';
          note.style.cssText='padding:2rem;text-align:center';
          note.textContent='Noch keine Kredite vorhanden. Diagramme werden nach dem Erfassen von Krediten angezeigt.';
          sec.appendChild(note);
        }
        note.style.display=on?'block':'none';
        Array.prototype.forEach.call(sec.children,function(ch){
          if(ch!==note)ch.style.display=on?'none':'';
        });
      },
      ch:function(){
        if(typeof Chart==='undefined')return;
        if(!loans.length){d.chEmpty(true);return;}
        d.chEmpty(false);
        ...
        setTimeout(function(){
          ...
          /* Horizontal reference line — the annotation plugin the previous
             config relied on is not loaded, so draw it directly. */
          function hLinePlugin(yVal,color,label){
            return {id:'hLine'+Math.round(yVal),afterDraw:function(chart){
              var yA=chart.scales.y;
              if(!yA||!isFinite(yVal))return;
              if(yVal<yA.min||yVal>yA.max)return;
              var y=yA.getPixelForValue(yVal),xA=chart.scales.x;
              if(!xA)return;
              var ctx2=chart.ctx;
              ctx2.save();
              ctx2.beginPath();ctx2.moveTo(xA.left,y);ctx2.lineTo(xA.right,y);
              ctx2.strokeStyle=color;ctx2.lineWidth=2;ctx2.setLineDash([6,3]);ctx2.stroke();
              if(label){
                ctx2.setLineDash([]);ctx2.fillStyle=color;ctx2.font='10px sans-serif';ctx2.textAlign='right';
                ctx2.fillText(label,xA.right-4,y-4);
              }
              ctx2.restore();
            }};
          }
          ...
          [{id:'ch-t-int',pre:'Zinskosten gesamt vs. bisher aufgelaufen (',post:')'},{id:'ch-t-mint',pre:'Monatliche Zinslast — Verlauf über Zeit (',post:')'},{id:'ch-t-cf',pre:'Fällige Gesamtbeträge nächste 12 Monate — Kapital + Zinsen (',post:')'},{id:'ch-t-dcol',pre:'Schulden vs. Collateral-Wert (',post:', 12 Monate)'}].forEach(function(t){
            var el=g(t.id);if(el)el.textContent=t.pre+mainC()+t.post;
          });
          var mcdT=g('ch-mcd-title');
          if(mcdT)mcdT.textContent='Margin-Call-Distanz (% BTC-Preisfall bis MC1 '+(cfg.ltvWarn!=null?cfg.ltvWarn:73)+'%)';
          /* ── 1. LTV pro Kredit — historisch ── */
          chLtv=dst(chLtv);
          var ltvSelEl=g('ch-ltv-sel');
          if(ltvSelEl&&act.length){
            /* Loan selector buttons — rebuilt on every render so added,
               closed or deleted loans cannot shift the selection */
            var selId=window._ltvSelId;
            if(!selId||!act.some(function(l){return l.id===selId;}))selId=act[0].id;
            window._ltvSelId=selId;
            ltvSelEl.innerHTML=act.map(function(l){
              return '<button class="sm'+(l.id===selId?' on':'')+'" data-lid="'+esc(l.id)+'" onclick="window._ltvSelId=this.dataset.lid;d.ch();" style="font-size:11px;padding:2px 8px">'+esc(l.name)+'</button>';
            }).join('');
          }
          var selIdx=Math.max(0,act.findIndex(function(l){return l.id===window._ltvSelId;}));
          var ltvC=mk('ch-ltv');
          if(ltvC&&act.length&&act[selIdx]&&!(act[selIdx].col>0)){
            emptyChart(ltvC,'Für diesen Kredit ist kein Collateral hinterlegt — es gibt keinen LTV-Verlauf.');
          } else if(ltvC&&act.length&&act[selIdx]){
            chartReady(ltvC);
            var l=act[selIdx];
            d.fetchBtcHistory(function(hist){
              ...
              if(hist){
                ...
                for(var dd=179;dd>=0;dd--){
                  ...
                  var hp=hist[key];
                  if(hp&&pD(l.start)<=dt){
                    var dueHist=ltvU(l);pts.push(parseFloat((dueHist/(l.col*hp)*100).toFixed(1)));
                    lbls.push(dd===0?'Heute':dt.toLocaleDateString('de-CH',{day:'2-digit',month:'2-digit'}));
                  }
                }
              }
              /* Always add current point */
              if(!pts.length&&l.col>0){pts=[parseFloat((ltvU(l)/(l.col*R.BTC)*100).toFixed(1))];lbls=['Heute'];}
              var ptColors=pts.map(function(v){return lc(v);});
              chLtv=new Chart(ltvC,{type:'line',
                data:{labels:lbls,datasets:[{data:pts,borderColor:'#F97316',backgroundColor:'rgba(249,115,22,.08)',fill:true,tension:0.3,borderWidth:2,pointRadius:0,pointHoverRadius:4,segment:{borderColor:function(ctx){return lc(pts[ctx.p1DataIndex]);}}}]},
          ...
          if(colC&&act.length){
            ...
            chCol=new Chart(colC,{type:'doughnut',
              data:{labels:act.map(function(l){return l.name;}),
                datasets:[{data:act.map(function(l){return l.col;}),backgroundColor:act.map(function(_,i){return COLORS[i%COLORS.length];}),borderWidth:2,borderColor:chCardBg}]},
          ...
          if(ccyC&&all.length){
            ...
            chCcy=new Chart(ccyC,{type:'doughnut',
              data:{labels:ccyKeys,datasets:[{data:ccyKeys.map(function(k){return Math.round(ccyMap[k]);}),backgroundColor:ccyKeys.map(function(_,i){return COLORS[i%COLORS.length];}),borderWidth:2,borderColor:chCardBg}]},
              options:{responsive:true,maintainAspectRatio:false,layout:{padding:{top:30,bottom:30,left:30,right:30}},plugins:{
                legend:{position:'right',labels:{font:{size:11},padding:8,generateLabels:function(chart){
                  return chart.data.labels.map(function(label,i){
                    var val=ccyMap[label]||0;
                    var pct=ccyTotal>0?Math.round(val/ccyTotal*100):0;
                    return{text:label+': '+pU(Math.round(val))+' ('+pct+'%)',fillStyle:COLORS[i%COLORS.length],fontColor:chTextColor,hidden:false,index:i};
                  });
                }}},
                tooltip:{callbacks:{label:function(x){
                  var pct=ccyTotal>0?Math.round(x.parsed/ccyTotal*100):0;
                  return ' '+x.label+': '+pU(x.parsed)+' ('+pct+'%)';
                }}},
                datalabels:{anchor:'end',align:'end',offset:6,
                  formatter:function(val,ctx){
                    var key=ccyKeys[ctx.dataIndex];
                    var pct=ccyTotal>0?Math.round((ccyMap[key]||0)/ccyTotal*100):0;
                    return key+': '+pU(Math.round(val))+'\n('+pct+'%)';
                  },
                  font:{size:10},color:chText2Color,
              ...
              plugins:[ChartDataLabels,{id:'ccyCenter',beforeDraw:function(chart){
                ...
                ctx2.fillStyle=getComputedStyle(document.getElementById('ffd-root')).getPropertyValue('--text');
                ctx2.font='bold 13px sans-serif';
                ctx2.fillText(''+pU(Math.round(ccyTotal)),cx,cy-8);
                ctx2.font='11px sans-serif';
                ctx2.fillStyle=getComputedStyle(document.getElementById('ffd-root')).getPropertyValue('--text3')||'#888';
          ...
          (function(){
            ...
            var histPrices={};
            /* Kein stiller Ersatz durch den Live-Kurs: laesst sich der Preis vom
               Kreditdatum nicht aufloesen, gibt es fuer diesen Kredit keinen
               Break-even und der Balken bleibt leer. */
            function bsFor(l){return l.btcStart||(l.start&&histPrices[l.start])||null;}
            function renderBep(){
              if(!bepC||!all.length){
                if(bepC)emptyChart(bepC,'Keine Kredite vorhanden.');
                return;
              }
              chBep=dst(chBep);
              chartReady(bepC);
              var bepLabels=all.map(function(l){return l.name+(l.btcStart?'':(bsFor(l)?' *':' †'));});
              var startVals=all.map(function(l){return bsFor(l);});
              var bepVals=all.map(function(l){
                var lU=toU(l.amount,l.c);var bs=bsFor(l);
                if(!bs||!lU)return null;
                return parseFloat((bs*(costU(l)/lU)).toFixed(0));
              });
              chBep=new Chart(bepC,{type:'bar',
                ...
                options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top',labels:{font:{size:11}}},
                  tooltip:{callbacks:{label:function(x){return ' '+x.dataset.label+': '+pU(x.parsed.y);}}}},
                  scales:{y:{suggestedMax:(function(){var vs=bepVals.concat(startVals,[R.BTC]).filter(function(v){return typeof v==='number'&&isFinite(v);});return vs.length?Math.max.apply(null,vs)*1.05:undefined;})(),ticks:{callback:function(v){return faxU(v);}},grid:{color:chGridColor}},x:{grid:{display:false}}}
                },
                plugins:[hLinePlugin(R.BTC,'#16a34a','Aktuell '+faxU(R.BTC))]});
            }
          ...
          if(intC&&act.length){
            ...
            var accruedInt=act.map(function(l){
              var pct=(function(){var st=pD(l.start),tot=aM(l.start,l.term)-st;return tot>0?Math.min(1,Math.max(0,(nowI-st)/tot)):1;})();
            ...
            chInt2=new Chart(intC,{type:'bar',
              ...
              options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top',labels:{font:{size:11}}},
                tooltip:{callbacks:{label:function(x){return ' '+x.dataset.label+': '+pU(x.parsed.y);}}}},
                scales:{x:{grid:{display:false}},y:{ticks:{callback:function(v){return faxU(v);}},grid:{color:chGridColor}}}}});
          }
          ...
          if(zinslastC&&act.length){
            ...
            var now_zl=new Date();
            var earliest=new Date(Math.min.apply(null,act.map(function(l){return pD(l.start);})));
            ...
            chZinslast=new Chart(zinslastC,{
              ...
              options:{
                ...
                plugins:{
                  ...
                  tooltip:{callbacks:{
                    label:function(x){return ' '+x.dataset.label+': '+pU(x.parsed.y);},
                    footer:function(items){var sum=items.reduce(function(s,x){return s+x.parsed.y;},0);return sum>0?'Gesamt: '+pU(Math.round(sum)):'';} 
                  }}
                },
                scales:{
                  x:{stacked:true,grid:{display:false},ticks:{maxTicksLimit:18,autoSkip:true}},
                  y:{stacked:true,ticks:{callback:function(v){return faxU(v);}},grid:{color:chGridColor}}
                }
              },
          ...
          (function(){
            ...
            if(!btcostC)return;
            if(!all.length){emptyChart(btcostC,'Keine Kredite vorhanden.');return;}
            chartReady(btcostC);
            var btcostLoans=all;
            var btcostPrices={};
            ...
            function renderBtcost(){
              var btcCosts=btcostLoans.map(function(l){
                ...
                /* The fee is denominated in BTC — converting it to USD at
                   today's price and back at the start price would inflate it
                   whenever BTC has risen. Count it as the BTC amount it is. */
                return parseFloat((intU(l)/bs+(l.feeBtc||0)).toFixed(8));
              });
              chBtcost=dst(chBtcost);
          ...
          if(ganttEl&&all.length){
            var ganttLoans=all.slice().sort(function(a,b){return pD(a.start)-pD(b.start);});
            var now=new Date();
            var minD=new Date(Math.min.apply(null,ganttLoans.map(function(l){return pD(l.start);})));
            ...
            ganttEl.innerHTML=
              ...
              ganttLoans.map(function(l){
                ...
                var clr=l.status==='closed'?'#9ca3af':l.col>0&&(ltvU(l)/(l.col*R.BTC))>=tW()?'#ea580c':'#F97316';
                return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px">'+
                  '<div style="width:90px;flex-shrink:0;font-size:11px;font-weight:600;color:var(--text2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(l.name)+'</div>'+
          ...
          if(roTlEl){
            ...
            if(!chainIds.length){
              ...
              chainIds.forEach(function(cid){chains[cid].sort(function(a,b){return pD(a.start)-pD(b.start);});});
              /* Timeline range */
              var allChainLoans=chainIds.reduce(function(a,cid){return a.concat(chains[cid]);},[]); 
              var minDR=new Date(Math.min.apply(null,allChainLoans.map(function(l){return pD(l.start);})));
              ...
              var ticksR='',linesR='';
              var curR=new Date(minDR.getFullYear(),minDR.getMonth(),1);
              while(curR<=new Date(maxDR.getTime()+1)){
                ...
                curR=new Date(curR.getFullYear(),curR.getMonth()+stepR,1);
              }
              ...
              chainIds.forEach(function(cid,ci){
                ...
                html+='<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">'+
                  '<div style="width:110px;flex-shrink:0;font-size:11px;font-weight:700;color:'+clr+';white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="'+chainLoans.length+' Kredite">🔗 '+esc(chainName)+'</div>'+
                ...
                chainLoans.forEach(function(l,li){
                  ...
                  var intCost=intU(l);
                  var gap=li>0?2:0;
                  html+='<div data-tip="'+esc(d.roTlTip(l,li,chainLoans.length))+'" style="position:absolute;left:calc('+left+'% + '+gap+'px);width:calc('+width+'% - '+gap+'px);height:100%;background:'+clr+';opacity:'+(isActive?1:.55)+';border-radius:3px;display:flex;align-items:center;overflow:hidden;padding:0 4px;z-index:1;box-sizing:border-box;cursor:default">'+
                    '<span style="font-size:9px;color:#fff;white-space:nowrap;overflow:hidden">'+esc(l.name)+'</span>'+
                  '</div>';
                });
                html+='<div style="position:absolute;left:'+nowPctR+'%;top:0;width:2px;height:100%;background:#ef4444;z-index:2"></div>'+
                  '</div>'+
                  '<div style="width:80px;flex-shrink:0;font-size:10px;font-weight:600;color:var(--text3);text-align:right">'+fmtU(totalCost)+'</div>'+
                '</div>';
              });
          ...
          if(cfC){
            ...
            var cfCur=new Date(now2.getFullYear(),now2.getMonth(),1);
            while(cfCur<=cfMax){
            ...
            var cfData=cfMonths.map(function(m){
              return act.reduce(function(sum,l){
                ...
                if(end.getFullYear()===m.getFullYear()&&end.getMonth()===m.getMonth()){
                  return sum+toU(l.amount,l.c)+intU(l);
                }
                return sum;
              },0);
            });
            chCf=new Chart(cfC,{type:'bar',
              ...
              options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},
                tooltip:{callbacks:{label:function(x){return x.parsed.y>0?' Fällig (Kapital+Zinsen): '+pU(x.parsed.y):'  Nichts fällig';}}}},
                scales:{y:{ticks:{callback:function(v){return v>0?faxU(v):'';}},grid:{color:chGridColor}},x:{grid:{display:false}}}},
          ...
          if(dcolC){
            (function(){
              ...
              var dcolLoans=loans;
              var oldest3=dcolLoans.reduce(function(e,l){var s=pD(l.start);return s<e?s:e;},now3);
                  ...
                  options:{responsive:true,maintainAspectRatio:false,
                    ...
                    plugins:{legend:{position:'top',labels:{font:{size:11}}},
                      tooltip:{callbacks:{label:function(x){
                        ...
                        return ' '+x.dataset.label+': '+pU(x.parsed.y);
                      }}}},
                    scales:{
                      y:{position:'left',ticks:{callback:function(v){return faxU(v);}},grid:{color:chGridColor}},
          ...
          if(termC&&act.length){
            ...
            chTerm=new Chart(termC,{type:'bar',
              ...
              options:{responsive:true,maintainAspectRatio:false,
                plugins:{legend:{position:'top',labels:{font:{size:11}}},
                  tooltip:{callbacks:{label:function(x){
                    if(x.dataset.yAxisID==='yVol')return ' Volumen: '+pU(Math.round(x.parsed.y));
                ...
                scales:{
                  ...
                  yVol:{position:'right',title:{display:true,text:'Volumen '+mainC(),font:{size:10}},ticks:{callback:function(v){return faxU(v);}},grid:{display:false}},
                  x:{grid:{display:false}}
                }
              }
            });
      ...
      checkAlarms:function(){
        ...
        if(liqd.length){
          ...
          _ffdSetBanner(el,'&#128308; LIQUIDIERT: '+liqd.map(function(l){var due=toU(l.amount,l.c)+intU(l);return esc(l.name)+' LTV '+(due/(l.col*R.BTC)*100).toFixed(1)+'%';}).join(' · ')+' — Kredit wurde liquidiert!');
        } else if(danger.length){
          el.className='alarm-banner crit show';
          _ffdSetBanner(el,'&#128308; GEFAHR MC3: '+danger.map(function(l){var due=toU(l.amount,l.c)+intU(l);return esc(l.name)+' LTV '+(due/(l.col*R.BTC)*100).toFixed(1)+'%';}).join(' · ')+' — Liquidationsgefahr!');
        } else if(crit.length){
          el.className='alarm-banner crit show';
          _ffdSetBanner(el,'&#9888; KRITISCH MC2: '+crit.map(function(l){var due=toU(l.amount,l.c)+intU(l);return esc(l.name)+' LTV '+(due/(l.col*R.BTC)*100).toFixed(1)+'%';}).join(' · ')+' — Margin Call!');
        } else if(warn.length){
          el.className='alarm-banner warn show';
          _ffdSetBanner(el,'&#9888; Warnung MC1: '+warn.map(function(l){var due=toU(l.amount,l.c)+intU(l);return esc(l.name)+' LTV '+(due/(l.col*R.BTC)*100).toFixed(1)+'%';}).join(' · ')+' — LTV \u00fcber '+cfg.ltvWarn+'%');
        } else {
      ...
      renderNextAction:function(){
        ...
        if(liqd.length){
          ...
          el.innerHTML=d._naCard('🔴','Kredit liquidiert','Der Kredit wurde durch Firefish liquidiert','<b>'+esc(l.name)+'</b> hat LTV '+ltv+'% — Liquidation erfolgt!','#dc2626','rgba(220,38,38,.08)');return;
        }
        ...
        if(danger.length){
          ...
          el.innerHTML=d._naCard('🔴','Sofortiger Handlungsbedarf','Collateral nachschiessen oder Kredit zurückzahlen','<b>'+esc(l.name)+'</b> hat LTV '+ltv+'% — Liquidationsgefahr!','#dc2626','rgba(220,38,38,.08)');return;
        }
        /* Priority 3 — crit LTV */
        ...
        if(crit.length){
          ...
          el.innerHTML=d._naCard('🟠','Margin Call Warnung','LTV kritisch — Collateral prüfen','<b>'+esc(l.name)+'</b> hat LTV '+ltv+'% — MC2 erreicht','#ea580c','rgba(234,88,12,.07)');return;
        }
        ...
        if(urgent.length){
          ...
          el.innerHTML=d._naCard('⏰','Kredit läuft ab','Roll-Over vorbereiten oder zurückzahlen','<b>'+esc(l.name)+'</b> — fällig in <b>'+dl+' Tag'+(dl===1?'':'en')+'</b> ('+fmt(frU(dueU(l),l.c),l.c)+')','#d97706','rgba(217,119,6,.07)');return;
        }
        ...
        if(soon.length){
          ...
          el.innerHTML=d._naCard('📅','Fälligkeit in Kürze','Roll-Over oder Rückzahlung planen','<b>'+esc(l.name)+'</b> — fällig in <b>'+dl+' Tagen</b> ('+aM(l.start,l.term).toLocaleDateString('de-CH',{day:'2-digit',month:'short',year:'numeric'})+')','#6366f1','rgba(99,102,241,.06)');return;
        }
        /* Priority 5 — warn LTV */
        ...
        if(warn.length){
          ...
          el.innerHTML=d._naCard('🟡','LTV im Beobachtungsbereich','BTC-Preisentwicklung im Auge behalten','<b>'+esc(l.name)+'</b> hat LTV '+ltv+'% — über MC1-Schwelle','#d97706','rgba(217,119,6,.06)');return;
        }
        /* All good */
        var next=act.slice().sort(function(a,b){return dL(a.start,a.term)-dL(b.start,b.term);})[0];
        var dl=dL(next.start,next.term);
        el.innerHTML=d._naCard('✅','Alles im grünen Bereich','Nächste Fälligkeit: <b>'+esc(next.name)+'</b> in '+dl+' Tagen','Portfolio-LTV und Margin Call-Distanzen sind unkritisch','#16a34a','rgba(22,163,74,.06)');
      },
      ...
      vorInit:function(){
        ...
        /* Start on the configured main currency so the tools agree with the
           rest of the dashboard; the picker can still be changed per session. */
        var startC=ccys.indexOf(mainC())>=0?mainC():'USD';
        if(!sel.options.length)
          sel.innerHTML=ccys.map(function(c){return '<option'+(c===startC?' selected':'')+'>'+c+'</option>';}).join('');
        d.vorCcyChange();
      },
      vorCcyChange:function(){
        ...
        var lbls={
          ...
          'gvl-fee-lbl':null /* handled dynamically */,
        ...
        /* Gebuehren-Label separat: textContent wuerde die Kind-Spans loeschen */
        var feeLbl=document.getElementById('gvl-fee-lbl');
        if(feeLbl){
          var feeRst=document.getElementById('gvl-fee-reset');
          var feeSrc=document.getElementById('gvl-fee-src');
          feeLbl.textContent='Bearbeitungsgebühr ('+ccy+', auto) ';
          if(feeRst)feeLbl.appendChild(feeRst);
          if(feeSrc)feeLbl.appendChild(feeSrc);
        }
        ...
        d.mkb();d.szb();d.mkr();d.btr();d.mkk();d.ltvc();d.alp();d.gz();d.nach();d.nlp();d.se2();d.gvl();d.abl();
      },
      vorGetCcy:function(){
        var sel=document.getElementById('vor-ccy');
      ...
      plInit:function(){
        ...
        /* Tool 1: Fair Price */
        if(g('pl1-btc'))g('pl1-btc').textContent=''+pU(btc);
        if(g('pl1-fair'))g('pl1-fair').textContent=''+pU(fair);
        ...
        /* Tool 3 & 4 */
        d.pl3();
        if(g('pl4-bottom'))g('pl4-bottom').textContent=''+pU(bottom);
        if(g('pl4-btc'))g('pl4-btc').textContent=''+pU(btc);
        d.pl4();
      },
      pl2:function(){
        var days=d.plDays();
        ...
        if(g('pl2-fair'))g('pl2-fair').textContent=''+pU(fair);
        if(g('pl2-bottom'))g('pl2-bottom').textContent=''+pU(bottom);
        if(g('pl2-needed'))g('pl2-needed').textContent=''+pU(needed);
        ...
        if(v2){
          if(btc<=needed){v2.style.background='var(--ok-bg)';v2.style.color='var(--ok)';v2.style.border='1px solid var(--ok-border)';v2.textContent='\u2192 Wenn ich bei einem Bitcoin-Preis von '+pU(needed)+' einen Kredit nehmen würde, dann beträgt der Liquidationspreis des Kredits '+pU(bottom)+'. Dies entspricht der heutigen Preisuntergrenze des Power Laws.';}
      ...
      pl3:function(){
        ...
        if(g('pl3-btc'))g('pl3-btc').textContent=''+pU(btc);
        if(g('pl3-target'))g('pl3-target').textContent=''+pU(target);
        ...
        if(v3){
          if(dev3>=thresh){v3.style.display='';v3.style.background='var(--ok-bg)';v3.style.color='var(--ok)';v3.style.border='1px solid var(--ok-border)';v3.textContent='\u2713 Bitcoin ('+pU(btc)+') hat den Zielpreis ('+pU(target)+') erreicht. Vorzeitige R\u00fcckzahlung ist gem\u00e4ss Strategie empfohlen.';}
          else{v3.style.display='';v3.style.background='var(--warn-bg)';v3.style.color='var(--warn)';v3.style.border='1px solid var(--warn-border)';v3.textContent='\u23f3 Bitcoin ('+pU(btc)+') hat den Zielpreis ('+pU(target)+') noch nicht erreicht. Noch '+(thresh-dev3).toFixed(1)+'% entfernt.';}
        }
      },
      pl4:function(){
        var days=d.plDays();
        ...
        if(g('pl4-formula'))g('pl4-formula').textContent='F\u00e4lliger Betrag '+pU(due)+' \u00f7 (Bottom Price '+pU(bottom)+' \u00d7 0.95)';
      },
      ...
      rosi:function(){
        ...
        var baseDate=startEl&&startEl.value?pD(startEl.value):new Date();
        ...
        var feeBtcFirst=0,totalFeeBtcSim=0;
        for(var i=0;i<n;i++){
          ...
          var startD=aM(baseDate,i*term);
          var endD=aM(startD,term);
          ...
          rows+='<tr'+(i>0?' style="background:var(--bg2)"':'')+'>'+
            ...
            (collateralBTC>0?'<td class="amt" style="font-size:11px">'+pU(Math.round(btcPusd+toU(costAmt,ccy)/collateralBTC))+'</td>':'<td class="amt" style="color:var(--text4)">—</td>')+
          '</tr>';
        ...
        var endDate=aM(baseDate,totalMonths);
        var endDateStr=endDate.toLocaleDateString('de-CH',{day:'2-digit',month:'long',year:'numeric'});
        var effRate=originalAmount>0&&totalMonths>0?((Math.pow(1+totalCost/originalAmount,12/totalMonths)-1)*100):0;
        ...
        rows+='<tr style="background:var(--bg2);font-weight:700;border-top:2px solid var(--border)">'+
          ...
          (collateralBTC>0?'<td class="amt" style="font-size:11px;font-weight:700;color:var(--accent)">'+pU(Math.round(btcPusd+toU(totalCost,ccy)/collateralBTC))+'</td>':'<td class="amt" style="color:var(--text4)">—</td>')+
        '</tr></tbody>';
        ...
        var feeTile=feeBtcFirst?tile('Geb\u00fchren gesamt',fmtC(totalFee),totalFeeBtcSim.toFixed(6)+' BTC'):tile('Geb\u00fchren gesamt',fmtC(totalFee),'');
      ...
      zukunft:function(){
        ...
        var futDate=dateEl&&dateEl.value?pD(dateEl.value):null;
        ...
        act.forEach(function(l,i){
          ...
          /* LTV at target */
          var ltvNow=colValueNow>0?ltvU(l)/colValueNow*100:null;
          var ltvFut=colValueFut>0?debtAtTarget/colValueFut*100:null;

          /* MC1 price (73% LTV threshold) */
          var mc1=l.col>0?debtAtTarget/(tW()*l.col):null;
        ...
        g('zk-summary').innerHTML=
          tile('BTC-Preis',''+pU(Math.round(R.BTC))+' → '+pU(Math.round(futBtc)),
            (btcChange>=0?'▲ +':' ▼ ')+Math.abs(btcChange).toFixed(1)+'%',btcChange>=0?'var(--ok)':'var(--err)')+
          tile('Collateral-Wert',fmtU(totalColFut),
            'Heute: '+fmtU(totalColNow)+' · '+(colChange>=0?'▲ +':' ▼ ')+Math.abs(colChange).toFixed(1)+'%',
            colChange>=0?'var(--ok)':'var(--err)')+
          tile('Schulden gesamt',fmtU(totalDebtFut),'Heute: '+fmtU(totalDebtNow),'var(--text)')+
          tile('Netto-Position (Equity)',fmtU(totalColFut-totalDebtFut),
            'Heute: '+fmtU(totalColNow-totalDebtNow),
            (totalColFut-totalDebtFut)>=0?'var(--ok)':'var(--err)')+
          tile('Netto P&L',( totalNetPnl>=0?'▲ +':'▼ ')+fmtU(Math.abs(totalNetPnl)),
          ...
          chZukunft=new Chart(ctx,{
            ...
            options:{
              ...
              plugins:{
                ...
                tooltip:{callbacks:{label:function(c){return c.dataset.label+': '+pU(Math.round(c.raw));}}},
                annotation:{} /* no dependency, skip */
              },
              scales:{
                y:{ticks:{callback:function(v){return faxU(v);}},grid:{color:chGridColor}},
        ...
        /* ── Table ── */
        var ltvColor=function(v){return v===null?'':lc(v);};
        var tbl='<thead><tr>'+
          '<th>Kredit</th>'+
          '<th>Collateral</th>'+
          '<th>Wert heute</th>'+
          '<th>Wert bei '+faxU(futBtc)+'</th>'+
        ...
        rows.forEach(function(r,i){
          ...
          tbl+='<tr'+(mc1Warn?' style="background:rgba(220,38,38,.06)"':'')+'>'+
            ...
            '<td class="amt">'+fmtU(r.colNow)+'</td>'+
            '<td class="amt" style="font-weight:600;color:'+(r.colFut>=r.colNow?'var(--ok)':'var(--err)')+'">'+fmtU(r.colFut)+'</td>'+
            '<td class="amt">'+fmtU(r.debtNow)+'</td>'+
            '<td class="amt" style="font-weight:600">'+fmtU(r.debtFut)+'</td>'+
            ...
            '<td class="amt" style="color:'+(r.mc1!==null&&futBtc<=r.mc1?'#dc2626':r.mc1!==null&&futBtc<=r.mc1*1.15?'#ea580c':'var(--text3)')+'">'+
              (r.mc1!==null?''+pU(Math.round(r.mc1)):'—')+'</td>'+
            '<td class="amt" style="font-weight:600;color:'+(r.netPnl>=0?'var(--ok)':'var(--err)')+'">'+
              (r.netPnl>=0?'▲ +':'▼ ')+fmtU(Math.abs(r.netPnl))+'</td>'+
          '</tr>';
        });
        ...
        tbl+='<tr style="font-weight:700;border-top:2px solid var(--border);background:var(--bg2)">'+
          ...
          '<td class="amt">'+fmtU(totalColNow)+'</td>'+
          '<td class="amt" style="color:'+(totalColFut>=totalColNow?'var(--ok)':'var(--err)')+'">'+fmtU(totalColFut)+'</td>'+
          '<td class="amt">'+fmtU(totalDebtNow)+'</td>'+
          '<td class="amt">'+fmtU(totalDebtFut)+'</td>'+
          '<td></td><td></td><td></td>'+
          '<td class="amt" style="color:'+(totalNetPnl>=0?'var(--ok)':'var(--err)')+'">'+
            (totalNetPnl>=0?'▲ +':'▼ ')+fmtU(Math.abs(totalNetPnl))+'</td>'+
        '</tr></tbody>';
      ...
      /* 2. Sicherheit zu Beginn */
      /* Merkt sich, dass der Nutzer ein auto-gefuelltes Feld selbst editiert hat */
      feeManual:function(id){var el=g(id);if(el)el._autoFilled=false;},
      ...
      mkr:function(){
        ...
        var feeEl=g('mkr-fee');
        if(!btc||!anz){
          g('mkr-wert').value='';g('mkr-zinsen').value='';if(feeEl)feeEl.value='';
        ...
        if(feeEl)feeEl.value=(Math.ceil(feeBTC*100000)/100000).toFixed(5)+' BTC';
      ...
      gvlPopulate:function(){
        ...
        var cur=sel.value;
        sel.innerHTML='<option value="">— Kredit wählen —</option>'+
          avail.map(function(l){var i=loans.indexOf(l);var tag=l.status==='closed'?' ✓':' ●';return '<option value="'+i+'">'+esc(l.name)+tag+' ('+fmt(l.amount,l.c)+')</option>';}).join('');
        if(cur)sel.value=cur;
      },
      gvlFill:function(){
        var sel=g('gvl-loan-sel');
        if(!sel||!sel.value){var fe0=g('gvl-fee');if(fe0){fe0._loanFeeUSD=null;fe0._autoFeeUSD=null;}return;}
        var l=loans[parseInt(sel.value)];if(!l)return;
        var ccy=d.vorGetCcy();
        var lU=toU(l.amount,l.c);
        ...
        var feeFieldEl=g('gvl-fee');feeFieldEl._manualFee=false;feeFieldEl._autoFeeUSD=null;feeFieldEl._loanFeeUSD=l.feeBtc?feeU(l):null;feeFieldEl.value=parseFloat(frU(feeUSD,ccy).toFixed(ccy==='CZK'||ccy==='PLN'?0:2));var feeBtn=g('gvl-fee-reset');if(feeBtn)feeBtn.style.display='none';
        ...
        if(ds)ds.textContent=l.start?('('+pD(l.start).toLocaleDateString('de-CH')+')'):'';
      ...
      prefetchHistPrices:function(){
        ...
        /* Nacheinander statt alle auf einmal: die CoinGecko-Gratis-API kippt
           schon bei wenigen Anfragen pro Minute in ein Rate-Limit. Nach dem
           ersten Limit wird abgebrochen — die restlichen Anfragen wuerden
           ohnehin nur abgewiesen und den Zaehler weiter hochtreiben. */
        var isos=Object.keys(dates).filter(function(iso){
          /* Aus Cache oder Preistabelle beantwortbar — kein Netzabruf noetig */
          try{var c=localStorage.getItem('ffd_btc_hist_'+iso);if(c){var pc=JSON.parse(c);if(pc&&pc.usd)return false;}}catch(e){}
          return btcHistLocal(iso)===null;
        });
        var idx=0,stopped=false;
        (function next(){
          if(stopped||idx>=isos.length)return;
          var iso=isos[idx++];
          d.btcHistPrice(iso,function(usd,err){
            if(err==='ratelimit'){stopped=true;return;}
      ...
      loadHistBtc_auto:function(i,startDate,wid){
        ...
        var lid=String(wid||'').replace(/^bsw-/,'');
        d.btcHistPrice(startDate,function(usd){
          if(!usd){
            ...
            sugEl.innerHTML=
              '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">'+
                ...
                '<button onclick="d.applyManualBtc('+i+',\''+escJs(lid)+'\')" style="font-size:11px;color:var(--accent);background:none;border:1px solid var(--accent);border-radius:4px;cursor:pointer;padding:2px 7px">Speichern</button>'+
              '</div>';
            return;
          }
          sugEl.style.display='block';
          sugEl.innerHTML=
            '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">'+
              '<span style="font-size:11px;color:var(--text3)">Historischer Preis am '+pD(startDate).toLocaleDateString('de-CH')+': <b style="color:var(--text)">'+pU(Math.round(usd))+'</b></span>'+
              '<button onclick="d.applyHistBtc('+i+','+Math.round(usd)+',\''+escJs(lid)+'\')" style="font-size:11px;color:#fff;background:var(--ok);border:none;border-radius:4px;cursor:pointer;padding:2px 8px">&#10003; Übernehmen</button>'+
      ...
      loadHistBtc:function(i,lid,startDate,btnEl){
      ...
      applyHistBtc:function(i,usd,id){
        var l=lRef(i,id);
        if(!l)return;
        l.btcStart=usd;
        save();d.loans();d.ov();
      },
      applyManualBtc:function(i,id){
      ...
      btcHistPrice:function(dateISO, cb){
        ...
        try{
          ...
          if(cached){var pc=JSON.parse(cached);if(pc&&pc.usd){cachedUsd=pc.usd;}}
        }catch(e){}
        if(cachedUsd){try{cb(cachedUsd,false);}catch(e){console.error('[btcHistPrice] callback:',e);}return;}
        ...
        /* Call the callback at most once — the old .catch() sat behind the
           success handler, so an exception thrown inside cb() triggered a
           second, contradicting call. */
        var done=false;
        function once(v,err){if(done)return;done=true;try{cb(v,err);}catch(e){console.error('[btcHistPrice] callback:',e);}}
        var retry=arguments.length>2?arguments[2]:0;
        fetch('https://api.coingecko.com/api/v3/coins/bitcoin/history?date='+cgDate+'&localization=false')
          .then(function(r){
            if(r.status===429){
              /* Back off and retry, like gvlFetchHistoric already does */
              if(retry<3){done=true;setTimeout(function(){d.btcHistPrice(dateISO,cb,retry+1);},Math.pow(2,retry)*5000);return null;}
              once(null,'ratelimit');return null;
            }
            if(r.status===401||r.status===403){once(null,'auth');return null;}
            if(!r.ok){once(null,'error');return null;}
            return r.json();
          })
          .then(function(data){
            if(done)return;               /* a retry was scheduled */
            if(data===null||data===undefined){once(null,'notfound');return;}
            var usd=data&&data.market_data&&data.market_data.current_price&&data.market_data.current_price.usd;
            if(usd){try{localStorage.setItem(CACHE_KEY,JSON.stringify({usd:usd,ts:new Date().getTime()}));}catch(e){}once(usd,false);}
            else{once(null,'notfound');}
          })
          .catch(function(){once(null,'error');});
      },
      ...
      autoFillBtcStart:function(dateId,priceId,hintId){
        ...
        d.btcHistPrice(iso,function(usd,err){
          if(usd){
            ...
            var dateStr=pD(iso).toLocaleDateString('de-CH');
      ...
      gvl:function(){
        ...
        var feeSrcEl=g('gvl-fee-src');
        if(!feeEl._manualFee){
          /* Gewaehlter Kredit hat eine echte Gebuehr -> diese gewinnt vor der Auto-Formel */
          var autoFeeUSD=feeEl._loanFeeUSD!=null?feeEl._loanFeeUSD:Math.round(lU*feerate);
          var ccy=d.vorGetCcy();
          ...
          if(feeSrcEl)feeSrcEl.style.display=feeEl._loanFeeUSD!=null?'inline':'none';
        } else {
          if(feeResetBtn)feeResetBtn.style.display='inline';
          if(feeSrcEl)feeSrcEl.style.display='none';
        }
      ...
      extPopulate:function(){
        /* Der Ablöse-Rechner hängt allein an den Krediten — hier mitziehen,
           weil extPopulate() bei jeder Kreditänderung läuft. */
        d.abl();
        var sel=g('ext-loan');if(!sel)return;
        var act=loans.filter(function(l){return l.status==='active';})
          .slice().sort(function(a,b){return aM(a.start,a.term)-aM(b.start,b.term);});
        sel.innerHTML='<option value="">— Kredit wählen —</option>'+
          act.map(function(l,i){return '<option value="'+loans.indexOf(l)+'">'+esc(l.name)+' ('+fmt(l.amount,l.c)+')</option>';}).join('');
        /* also populate rosi selector */
        var rsel=g('rosi-loan-sel');if(!rsel)return;
        rsel.innerHTML='<option value="">— Kredit wählen —</option>'+
          act.map(function(l){return '<option value="'+loans.indexOf(l)+'">'+esc(l.name)+' ('+fmt(l.amount,l.c)+')</option>';}).join('');
      },
      ...
      ext:function(){
        ...
        g('ext-due').textContent=fmt(frU(due,l.c),l.c)+' = '+pU(Math.round(due));
        ...
        g('ext-grid').innerHTML=exts.map(function(mo){
          ...
          return '<div class="card" style="border:1px solid var(--border);box-shadow:none;padding:.75rem">'+
            ...
            '<div style="font-size:12px;color:var(--text3);margin-bottom:2px">= '+pU(addUSD)+'</div>'+
      ...
      wc:function(){
        ...
        pctEl.textContent=(diff>0?'+':'')+diff+'% vs. aktuell ('+pU(R.BTC)+')';
        ...
        var rows=sorted.map(function(l){
          ...
          var mc1P=due/tW()/l.col;
          var mc2P=due/tC()/l.col;
          var mc3P=due/tD()/l.col;
          ...
          var statusLabel={'liq':'⚠️ Liquidiert','mc3':'⚠️ MC3 erreicht ('+(cfg.ltvDanger!=null?cfg.ltvDanger:86)+'%)','mc2':'⚠️ MC2 erreicht ('+(cfg.ltvCrit!=null?cfg.ltvCrit:79)+'%)','mc1':'⚠️ MC1 erreicht ('+(cfg.ltvWarn!=null?cfg.ltvWarn:73)+'%)','safe':'✓ Sicher'}[status];
          ...
          return '<div style="border-radius:10px;margin-bottom:.6rem;background:var(--bg2);border:1px solid '+(status==='safe'?'var(--border)':statusColor+'44')+';overflow:hidden">'+
            ...
            '<div style="display:flex;justify-content:space-between;align-items:center;padding:.6rem .85rem;border-bottom:1px solid var(--border)">'+
              '<div>'+
                '<b style="font-size:13px;color:var(--text)">'+esc(l.name)+'</b>'+
            ...
            '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:0;font-size:11px;text-align:center">'+
              ...
              '<div style="padding:.45rem .5rem;border-right:1px solid var(--border)">'+
                ...
                '<b style="color:'+(simBtc<=mc1P?'#dc2626':'#d97706')+'">'+(mc1P?''+pU(Math.round(mc1P)):'–')+'</b>'+
              '</div>'+
              '<div style="padding:.45rem .5rem;border-right:1px solid var(--border)">'+
                '<span style="color:var(--text4);display:block">MC3 bei</span>'+
                '<b style="color:'+(simBtc<=mc3P?'#dc2626':'#ea580c')+'">'+(mc3P?''+pU(Math.round(mc3P)):'–')+'</b>'+
              '</div>'+
              '<div style="padding:.45rem .5rem">'+
                '<span style="color:var(--text4);display:block">Liq. bei</span>'+
                '<b style="color:'+(simBtc<=liqP?'#dc2626':'#7c3aed')+'">'+(liqP?''+pU(Math.round(liqP)):'–')+'</b>'+
              '</div>'+
            '</div>'+
          ...
          .then(function(data){
            if(data&&data.prices){
              ...
              data.prices.forEach(function(p){
                /* CoinGecko liefert Tageswerte auf UTC-Mitternacht. Mit lokalen
                   Gettern faellt der Schluessel westlich von Greenwich auf den
                   Vortag — der Chart zeigt dann jeden Tag den falschen Kurs. */
                var dt=new Date(p[0]);
      ...
      ro:function(){
        ...
        chainIds.forEach(function(cid,ci){
          var members=chains[cid].slice().sort(function(a,b){return pD(a.start)-pD(b.start);});
          ...
          var firstStart=pD(members[0].start);
          /* The chain ends with the latest maturity, not with the loan that
             started last — a long first loan can outlive a short later one. */
          var lastEnd=members.reduce(function(mx,l){
            var e=aM(l.start,l.term);
            return (mx===null||e>mx)?e:mx;
          },null);
          var totalMonths=Math.max(1,Math.round((lastEnd-firstStart)/(1000*60*60*24*30.44)));
          ...
          /* Effective annual rate, compounded over the chain's real runtime */
          var effRate=avgAmount>0&&totalMonths>0?((Math.pow(1+totalCostUSD/avgAmount,12/totalMonths)-1)*100):0;
          ...
          html+='<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.5rem;margin-bottom:.75rem">'+
            '<div style="display:flex;align-items:center;gap:.6rem">'+
              ...
              '<div>'+
                '<div style="font-size:14px;font-weight:700;color:var(--text)">'+esc(chainName)+'</div>'+
            ...
            '<div style="display:flex;gap:1.5rem;flex-wrap:wrap">'+
              '<div style="text-align:right"><div style="font-size:10px;color:var(--text4);text-transform:uppercase;letter-spacing:.04em">Gesamtkosten</div><div style="font-size:16px;font-weight:700;color:var(--text)">'+fmtU(totalCostUSD)+'</div></div>'+
          ...
          html+='<div class="mg" style="margin-bottom:.75rem">'+
            '<div class="mc"><span class="mc-lbl">Kumulierter fälliger Betrag</span><span class="mc-val">'+fmtU(totalDueUSD)+'</span><span class="mc-sub">Kapital + alle Zinsen</span></div>'+
            '<div class="mc"><span class="mc-lbl">Zinsen gesamt</span><span class="mc-val">'+fmtU(totalInterestUSD)+'</span></div>'+
            '<div class="mc"><span class="mc-lbl">Gebühren gesamt</span><span class="mc-val">'+(totalFeeBtc?totalFeeBtc.toFixed(6)+' BTC':'—')+'</span>'+(totalFeeUSD?'<span class="mc-sub">≈ '+fmtU(totalFeeUSD)+'</span>':'')+'</div>'+
            '<div class="mc"><span class="mc-lbl">Ø Zinssatz (gewichtet)</span><span class="mc-val">'+avgRate.toFixed(2)+'%</span></div>'+
            '<div class="mc"><span class="mc-lbl">Ø Kreditbetrag</span><span class="mc-val">'+fmtU(avgAmount)+'</span></div>'+
          '</div>';
          ...
          members.forEach(function(l,mi){
            ...
            var end=aM(l.start,l.term).toLocaleDateString('de-CH');
            var isActive=l.status==='active';
            var lLU=toU(l.amount,l.c);
            var lBep=(l.btcStart>0&&lLU>0)?(l.btcStart*((lLU+costUSD)/lLU)):null;
            html+='<tr>'+
              '<td style="color:var(--text4);font-size:11px">'+(mi+1)+'</td>'+
              '<td class="tbl-name">'+esc(l.name)+'</td>'+
              '<td><span class="badge '+(isActive?'ba':'bc')+'">'+(isActive?'Aktiv':'Abg.')+'</span></td>'+
              '<td class="amt">'+fmt(l.amount,l.c)+'</td>'+
              '<td>'+l.rate+'%</td>'+
              '<td>'+l.term+' Mt.</td>'+
              '<td>'+pD(l.start).toLocaleDateString('de-CH')+'</td>'+
              '<td>'+end+'</td>'+
              '<td class="amt">'+fmtU(iUSD)+'</td>'+
              '<td class="amt" style="font-weight:600">'+fmtU(dueUSD)+'</td>'+
              '<td class="amt">'+(l.feeBtc?l.feeBtc.toFixed(6)+' BTC':'—')+'</td>'+
              '<td class="amt" style="font-weight:600">'+fmtU(costUSD)+'</td>'+
              (lBep?'<td class="amt" style="font-size:11px">'+pU(Math.round(lBep))+'</td>':'<td class="amt" style="color:var(--text4)">—</td>')+
            '</tr>';
          });
          /* Totals row */
          /* Break-even total: a roll-over carries the SAME principal forward,
             so the bitcoin one avoided selling is the one from the first loan —
             summing the members would divide the cost several times over and
             understate the break-even price.
             Without a stored start price there is no meaningful chain
             break-even — falling back to spot would produce a "target" that
             just tracks the current price. */
          var firstBtcStart=members[0].btcStart||null;
          var firstLU=toU(members[0].amount,members[0].c);
          var totalBep=(firstBtcStart&&firstLU>0)?(firstBtcStart*((firstLU+totalCostUSD)/firstLU)):null;
          html+='<tr style="background:var(--bg2);font-weight:700;border-top:2px solid var(--border)">'+
            ...
            '<td class="amt">'+fmtU(totalInterestUSD)+'</td>'+
            '<td class="amt" style="color:'+color+'">'+fmtU(totalDueUSD)+'</td>'+
            '<td class="amt">'+(totalFeeBtc?totalFeeBtc.toFixed(6)+' BTC':'—')+'</td>'+
            '<td class="amt" style="color:'+color+'">'+fmtU(totalCostUSD)+'</td>'+
            (totalBep?'<td class="amt" style="font-size:11px;color:'+color+'">'+pU(Math.round(totalBep))+'</td>':'<td class="amt" style="color:var(--text4)">—</td>')+
          '</tr>';
        ...
        if(standalone.length){
          html+='<div class="card" style="background:var(--bg2);border:1px dashed var(--border)">'+
            ...
            standalone.map(function(l){
              return '<span style="font-size:12px;background:var(--bg3);border-radius:6px;padding:2px 8px;color:var(--text3)">'+esc(l.name)+'</span>';
            }).join('')+
            '</div>'+
          '</div>';
        }
      ...
      sx:function(){
        ...
        var totalFeeActive=loans.filter(function(l){return l.status==='active';}).reduce(function(s,l){return s+feeU(l);},0);
        ...
        var dueThisYear=act.filter(function(l){return aM(l.start,l.term).getFullYear()===thisYear;}).length;
        var dueNextYear=act.filter(function(l){return aM(l.start,l.term).getFullYear()===thisYear+1;}).length;
        var dueIn2Years=act.filter(function(l){return aM(l.start,l.term).getFullYear()===thisYear+2;}).length;
        var due7=act.filter(function(l){var d=(aM(l.start,l.term)-today)/86400000;return d>=0&&d<=7;}).length;
        var due14=act.filter(function(l){var d=(aM(l.start,l.term)-today)/86400000;return d>=0&&d<=14;}).length;
        var due30=act.filter(function(l){var d=(aM(l.start,l.term)-today)/86400000;return d>=0&&d<=30;}).length;
        var due90=act.filter(function(l){var d=(aM(l.start,l.term)-today)/86400000;return d>=0&&d<=90;}).length;
        var due180=act.filter(function(l){var d=(aM(l.start,l.term)-today)/86400000;return d>=0&&d<=180;}).length;
        var due365=act.filter(function(l){var d=(aM(l.start,l.term)-today)/86400000;return d>=0&&d<=365;}).length;
        ...
        var dates=loans.map(function(l){return pD(l.start);}).filter(function(x){return !isNaN(x);});
        ...
        var effJahreszins=aU>0?act.reduce(function(s,l){
          /* Effektiv heisst: inklusive der einmaligen Bearbeitungsgebuehr */
          var annRate=l.term>0?((intU(l)+feeU(l))/toU(l.amount,l.c))*(12/l.term)*100:l.rate;
        ...
        var totalInterestEver=clo.reduce(function(s,l){return s+intU(l);},0)+act.reduce(function(s,l){
          var start=pD(l.start),end=aM(l.start,l.term),now5=new Date();
        ...
        html+='<div class="mc"><span class="mc-lbl">Volumen (Kreditbetrag)</span><span class="mc-val">'+fmtU(tU)+'</span>'+
          sub3(fmtU(aU),clo.length?fmtU(cloVolume):dash,fmtU(tU))+
          subCcy(
        ...
        html+='<div class="mc"><span class="mc-lbl">Volumen (Schulden)</span><span class="mc-val">'+fmtU(tDue)+'</span>'+
          sub3(fmtU(aDueS),clo.length?fmtU(cloDue):dash,fmtU(tDue))+
          subCcy(
        ...
        html+='<div class="mc"><span class="mc-lbl">\u00d8 Kredith\u00f6he</span><span class="mc-val">'+fmtU(avgAmt)+'</span>'+
          sub3(act.length?fmtU(avgAmtAct):dash,clo.length?fmtU(avgAmtClo):dash,fmtU(avgAmt))+
          subCcy(
        ...
        html+='<div class="mc"><span class="mc-lbl">Zinsen (bis F\u00e4lligkeit)</span><span class="mc-val">'+fmtU(totalInterestOut+totalInterestPaid)+'</span>'+
          sub3(fmtU(totalInterestOut),clo.length?fmtU(totalInterestPaid):dash,fmtU(totalInterestOut+totalInterestPaid))+
          subCcy(
        ...
        html+=tile('Zinsen seit Beginn (effektiv)',fmtU(totalInterestEver),'aufgelaufene Zinsen bis heute');
        ...
        html+=tile('Gesamtkosten (Zins+Geb\u00fchr)',fmtU(totalInterestOut+totalFeeActive),'aktive Kredite');
        html+='</div>';
        ...
        html+=tile('H\u00f6chster LTV',maxLtv?maxLtv.ltv.toFixed(1)+'%':dash,maxLtv?esc(maxLtv.name):'',maxLtv?lc(maxLtv.ltv):'');
        html+=tile('Niedrigster LTV',minLtv?minLtv.ltv.toFixed(1)+'%':dash,minLtv?esc(minLtv.name):'',minLtv?lc(minLtv.ltv):'');
        html+=tile('\u00d8 Puffer bis MC1',avgPuffer.toFixed(1)+'%','bis '+(cfg.ltvWarn!=null?cfg.ltvWarn:73)+'% LTV-Schwelle',avgPuffer<10?'#dc2626':avgPuffer<20?'#d97706':'#16a34a');
        html+='</div>';
        ...
        var beAvg=beLoans.length?beLoans.reduce(function(s,l){
          var bep=l.btcStart*(costU(l)/toU(l.amount,l.c));
          return s+bep;
        ...
        html+='<div class="mc"><span class="mc-lbl">Effektiver Jahreszins</span><span class="mc-val">'+effJahreszinsAll.toFixed(2)+'%</span>'+
          sub3(effJahreszins.toFixed(2)+'%',null,effJahreszinsAll.toFixed(2)+'%')+
          '<span class="mc-sub" style="color:var(--text4)">inkl. Bearbeitungsgebühr</span></div>';
        html+=tile('G\u00fcnstigster Kredit',cheapest?cheapest.rate.toFixed(2)+'%':dash,cheapest?esc(cheapest.name):'');
        html+=tile('Teuerster Kredit',priciest?priciest.rate.toFixed(2)+'%':dash,priciest?esc(priciest.name):'');
        if(beAvg!==null){
          html+='<div class="mc"><span class="mc-lbl">Break-even \u00d8 (einfach)</span>'+
            '<span class="mc-val" style="color:'+(beReached?'var(--ok)':'var(--warn)')+'">'+pU(Math.round(beAvg))+'</span>'+
            '<span class="mc-sub" style="color:'+(beReached?'var(--ok)':'var(--warn)')+'">'+
              (beReached?'\u2713 Erreicht — Kredite haben sich gelohnt':'\u2715 Noch nicht erreicht')+
            '</span>'+
            '<span class="mc-sub">Aktuell '+pU(Math.round(R.BTC))+
              (beReached?' · \u25b2 '+pU((Math.round(R.BTC-beAvg)))+' \u00fcber Break-even':
                         ' · noch '+pU((Math.round(beAvg-R.BTC)))+' bis Break-even')+
            '</span>'+
        ...
        if(beWeighted!==null){
          html+='<div class="mc"><span class="mc-lbl">Break-even gewichtet</span>'+
            '<span class="mc-val" style="color:'+(beWReached?'var(--ok)':'var(--warn)')+'">'+pU(Math.round(beWeighted))+'</span>'+
            ...
            '</span>'+
            '<span class="mc-sub">Aktuell '+pU(Math.round(R.BTC))+
              (beWReached?' · \u25b2 '+pU((Math.round(R.BTC-beWeighted)))+' \u00fcber Break-even':
                          ' · noch '+pU((Math.round(beWeighted-R.BTC)))+' bis Break-even')+
            '</span>'+
        ...
        html+='<span class="sh">R&#252;ckblick — hat sich das Beleihen gelohnt?</span>'+
          '<div class="card" style="margin-bottom:1rem;overflow-x:auto" id="sx-retro"></div>';
        el.innerHTML=html;
        d.renderRetro();
      },
      /* Tag, an dem der Kredit tatsächlich beendet wurde. Ohne eingetragenes
         Rückzahlungsdatum dient die Fälligkeit als Näherung; ein Datum in der
         Zukunft wird auf heute begrenzt, sonst gäbe es keinen Kurs dazu. */
      retroCloseIso:function(l){
        var iso=l.closedDate||'';
        if(!iso){
          var e=aM(l.start,l.term);
          if(isNaN(e.getTime()))return '';
          iso=e.getFullYear()+'-'+String(e.getMonth()+1).padStart(2,'0')+'-'+String(e.getDate()).padStart(2,'0');
        }
        var n=new Date();
        var today=n.getFullYear()+'-'+String(n.getMonth()+1).padStart(2,'0')+'-'+String(n.getDate()).padStart(2,'0');
        return iso>today?today:iso;
      },
      /* Rückblick auf abgeschlossene Kredite: der Break-even sagt, ab welchem
         Kurs sich ein Kredit lohnt — hier steht fest, ob er es getan hat. */
      renderRetro:function(){
        var el=g('sx-retro');if(!el)return;
        d._retroPx=d._retroPx||{};
        var items=loans.filter(function(l){
          return l.status==='closed'&&l.btcStart>0&&toU(l.amount,l.c)>0&&d.retroCloseIso(l);
        });
        var open=loans.filter(function(l){
          return l.status==='active'&&l.btcStart>0&&toU(l.amount,l.c)>0;
        });
        if(!items.length&&!open.length){
          el.innerHTML='<p class="note2" style="margin:0">Noch keine Kredite mit hinterlegtem BTC-Startpreis. '+
            'Sobald einer vorliegt, steht hier, ob das Beleihen gegenüber einem Verkauf günstiger war.</p>';
          return;
        }
        d._retroOpen=open;
        var missing=[];
        items.forEach(function(l){
          var iso=d.retroCloseIso(l);
          if(d._retroPx[iso]===undefined&&missing.indexOf(iso)<0)missing.push(iso);
        });
        if(!missing.length){d.retroDraw(el,items);return;}
        /* Preise einzeln nachladen — dieselbe Zurückhaltung wie beim Prefetch */
        var i=0;
        (function next(){
          if(i>=missing.length){d.retroDraw(el,items);return;}
          var iso=missing[i++];
          d.btcHistPrice(iso,function(usd,err){
            d._retroPx[iso]=usd||null;
            if(err==='ratelimit'){d.retroDraw(el,items);return;}
            next();
          });
        })();
      },
      /* Baut einen Block der Gegenüberstellung. `live` unterscheidet den
         abgeschlossenen Kredit (Kurs am Rückzahlungstag, Ergebnis steht fest)
         vom laufenden (heutiger Kurs, Ergebnis noch in Bewegung). */
      retroBlock:function(list,live){
        var totGain=0,totCost=0,won=0,known=0;
        var rows=list.map(function(l){
          var iso=live?'':d.retroCloseIso(l);
          var px=live?R.BTC:d._retroPx[iso];
          var lU=toU(l.amount,l.c);
          /* Die Bitcoin, die man für den Kreditbetrag hätte verkaufen müssen */
          var soldBtc=lU/l.btcStart;
          /* Volle Kosten der Laufzeit — die Zinsen sind bis zum Ende geschuldet,
             auch wenn der Kredit noch läuft. Damit deckt sich das Vorzeichen
             hier genau mit dem Break-even auf der Kreditkarte. */
          var cost=intU(l)+feeU(l);
          var gain=px?soldBtc*(px-l.btcStart):null;
          var res=gain===null?null:gain-cost;
          if(res!==null){totGain+=gain;totCost+=cost;known++;if(res>0)won++;}
          var col=res===null?'var(--text4)':res>0?'var(--ok)':'var(--err)';
          return '<tr>'+
            '<td><div class="tbl-name">'+esc(l.name)+'</div><div class="tbl-id">'+esc(l.id)+'</div></td>'+
            '<td>'+pD(l.start).toLocaleDateString('de-CH')+'<br><span style="color:var(--text4);font-size:11px">'+
              (live?'heute':pD(iso).toLocaleDateString('de-CH'))+'</span></td>'+
            '<td class="amt">'+pU(l.btcStart)+'<br><span style="color:var(--text4);font-size:11px">'+(px?pU(px):'—')+'</span></td>'+
            '<td class="amt">'+soldBtc.toFixed(8)+' BTC</td>'+
            '<td class="amt">'+(gain===null?'—':fmtU(gain))+'</td>'+
            '<td class="amt">'+fmtU(cost)+'</td>'+
            '<td class="amt" style="color:'+col+';font-weight:700">'+(res===null?'—':(res>0?'+':'')+fmtU(res))+'</td>'+
          '</tr>';
        }).join('');
        var total=totGain-totCost;
        return {
          known:known,missing:list.length-known,total:total,
          html:(known?'<div class="mc" style="margin:0 0 .6rem"><span class="mc-lbl">'+
              (live?'Stand heute über ':'Ergebnis über ')+known+' Kredit'+(known===1?'':'e')+'</span>'+
              '<span class="mc-val" style="color:'+(total>0?'var(--ok)':'var(--err)')+'">'+(total>0?'+':'')+fmtU(total)+'</span>'+
              '<span class="mc-sub">'+won+' von '+known+(live?' liegen vorn':' haben sich gelohnt')+
              ' · Kursgewinn '+fmtU(totGain)+' gegen Kosten '+fmtU(totCost)+'</span></div>':'')+
            '<table class="loans-table"><thead><tr>'+
              '<th>Kredit</th><th>Start / '+(live?'heute':'Ende')+'</th><th>BTC-Kurs</th><th>Nicht verkauft</th><th>Kursgewinn</th><th>Kosten</th><th>Ergebnis</th>'+
            '</tr></thead><tbody>'+rows+
            (known>1?'<tr style="background:var(--bg2);font-weight:700;border-top:2px solid var(--border)">'+
              '<td colspan="4">Total</td>'+
              '<td class="amt">'+fmtU(totGain)+'</td><td class="amt">'+fmtU(totCost)+'</td>'+
              '<td class="amt" style="color:'+(total>0?'var(--ok)':'var(--err)')+'">'+(total>0?'+':'')+fmtU(total)+'</td></tr>':'')+
            '</tbody></table>'
        };
      },
      retroDraw:function(el,items){
        var open=d._retroOpen||[];
        var html='<p class="note2" style="margin:0 0 .75rem">Gegenüberstellung je Kredit: Was hätten die Bitcoin eingebracht, '+
          'die man für den Kreditbetrag hätte verkaufen müssen — und was kostet der Kredit stattdessen? '+
          'Die Kosten sind immer die der vollen Laufzeit, denn die Zinsen sind bis zum Ende geschuldet.</p>';
        if(items.length){
          var b=d.retroBlock(items,false);
          html+='<div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:var(--text3);margin:0 0 .5rem">Abgeschlossen — steht fest</div>'+
            b.html+
            (b.missing?'<p class="note2" style="margin:.6rem 0 0;color:var(--warn)">⚠ Für '+b.missing+' Kredit(e) liess sich der Kurs am Rückzahlungstag nicht ermitteln.</p>':'');
        }
        if(open.length){
          var o=d.retroBlock(open,true);
          html+='<div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:var(--text3);margin:'+(items.length?'1.4rem':'0')+' 0 .5rem">Laufend — Stand heute</div>'+
            '<p class="note2" style="margin:0 0 .6rem">Bewertet zum aktuellen BTC-Kurs von '+pU(Math.round(R.BTC))+
            '. Das Ergebnis bewegt sich mit dem Kurs; ein Pluszeichen bedeutet, dass der Kredit über seinem Break-even liegt.</p>'+
            o.html;
        }
        el.innerHTML=html;
      },
      ...
      se:function(){
        ...
        el.innerHTML=ALL_CCYS.map(function(c){
          ...
          return '<label style="display:inline-flex;align-items:center;gap:6px;padding:6px 12px;border:1px solid '+(on?'var(--accent)':'var(--border)')+';border-radius:20px;cursor:pointer;background:'+(on?'var(--accent-bg)':'var(--bg)')+';font-size:13px;color:var(--text)">'+
            '<input type="checkbox" data-c="'+c+'" '+(on?'checked':'')+' onchange="d.seMainCcyRender(true)" style="accent-color:var(--accent);width:14px;height:14px">'+c+'</label>';
        }).join('');
        d.seMainCcyRender();
        ...
        var sbk=g('se-backup');if(sbk)sbk.value=cfg.backupDays!=null?cfg.backupDays:30;
        d.renderBackupHint();
        /* Render nav order */
        d.seNavOrderRender();
      },
      ccyAll:function(){
        ...
        d.seMainCcyRender(true);
      },
      /* The options mirror the currently ticked checkboxes, so the main
         currency can never point at a currency that is switched off. */
      seMainCcyRender:function(fromCheckboxes){
        var sel=g('se-main-ccy');if(!sel)return;
        var list=[];
        if(fromCheckboxes){
          document.querySelectorAll('#ccy-toggles input[type=checkbox]').forEach(function(cb){if(cb.checked)list.push(cb.dataset.c);});
        }
        if(!list.length)list=cfg.ccys.slice();
        var cur=sel.value||mainC();
        if(list.indexOf(cur)<0)cur=list.indexOf('USD')>=0?'USD':list[0];
      ...
      ccySave:function(){
        ...
        cfg.ccys=checked;
        var mSel=g('se-main-ccy');
        var wanted=mSel&&mSel.value?mSel.value:cfg.mainCcy;
        cfg.mainCcy=(checked.indexOf(wanted)>=0)?wanted:(checked.indexOf('USD')>=0?'USD':checked[0]);
        saveSettings(cfg);
        /* The main currency touches every view, so refresh all of them */
        dCcy=null;
        d.renderCsw();
        d.seMainCcyRender();
        d.updateHdrStats();d.checkAlarms();
        if(typeof d.vorCcyChange==='function'){var vs=g('vor-ccy');if(vs){vs.value=mainC();d.vorCcyChange();}}
        renderActiveTab();
      ...
      seLtvSave:function(){
        ...
        if(w<1||danger>94){alert('Die Schwellen müssen zwischen 1% und 94% liegen (Liquidation liegt bei 95%).');return;}
        if(disp<0||disp>100){alert('Der Anzeige-Filter muss zwischen 0% und 100% liegen.');return;}
      ...
      NAV_ITEMS:[
        ...
        {id:'to', label:'Tools'},
        {id:'bk', label:'Portfolio'}
      ],
      applyNavOrder:function(){
        var order=cfg.navOrder||d.NAV_ITEMS.map(function(x){return x.id;});
        /* Ensure all known NAV_ITEMS appear — add missing ones after their predecessor */
        d.NAV_ITEMS.forEach(function(item,i){
          if(order.indexOf(item.id)<0){
            /* Insert after predecessor if possible, otherwise at end */
            var prevId=i>0?d.NAV_ITEMS[i-1].id:null;
            var prevIdx=prevId?order.indexOf(prevId):-1;
            if(prevIdx>=0)order.splice(prevIdx+1,0,item.id);
      ...
      bk:function(){
        /* Der Ablöse-Rechner rechnet den Cold-Storage-Bestand mit ein */
        d.abl();
        var el=g('bk-content');if(!el)return;
        var act=loans.filter(function(l){return l.status==='active';});
        var btcReserve=cfg.btcReserve||0;
        var txBal=d.bkTxBalance();
        var aC=act.reduce(function(s,l){return s+l.col;},0);
        var aDue=act.reduce(function(s,l){return s+toU(l.amount,l.c)+intU(l);},0);
        var totalBTC=btcReserve+aC;
        var totalBTCvalUSD=totalBTC*R.BTC;
        var collateralShare=totalBTC>0?(aC/totalBTC*100):0;
        var reserveShare=totalBTC>0?(btcReserve/totalBTC*100):0;
        var netWorthUSD=totalBTCvalUSD-aDue;
        /* Was vom Bestand bliebe, wenn die offenen Schulden mit Bitcoin
           beglichen würden. Mal dem Kurs genommen ergibt das wieder das
           Netto-Vermögen — dieselbe Zahl, nur in Bitcoin statt in Fiat. */
        var dueBTC=R.BTC>0?aDue/R.BTC:0;
        var netBTC=totalBTC-dueBTC;
        var stackLTV=totalBTCvalUSD>0?(aDue/totalBTCvalUSD*100):0;
        var monthlyInt=act.reduce(function(s,l){return s+toU(l.amount*(l.rate/100)/12,l.c);},0);
        var CC2=(['EUR','CHF','USD','USDT','USDC','CZK','PLN','BTC']).filter(visC);
        var CC3=CC2.filter(function(c){return c!=='BTC';});
        var CC4=CC2.filter(function(c){return c!=='BTC';}); /* main currency is dropped by usdGridSub */
        function tile(lbl,val,sub,col){
          return '<div class="mc">'+
            '<span class="mc-lbl">'+lbl+'</span>'+
            '<span class="mc-val"'+(col?' style="color:'+col+'"':'')+'>'+val+'</span>'+
            (sub?'<span class="mc-sub" style="font-size:10px;line-height:1.5">'+sub+'</span>':'')+
          '</div>';
        }
        var sLtvCol=stackLTV>=(cfg.ltvDanger||86)?'var(--err)':stackLTV>=(cfg.ltvWarn||73)?'var(--warn)':'var(--ok)';
        /* MC1-Reserve: extra BTC needed from cold storage to bring all loans back to MC1 */
        var mc1ReserveBTC=0;
        act.forEach(function(l){
          if(!l.col)return;
          var due=toU(l.amount,l.c)+intU(l);
          var mc1Price=due/(tW()*l.col);
          if(R.BTC<mc1Price){mc1ReserveBTC+=Math.max(0,due/(tW()*R.BTC)-l.col);}
        });
        /* Liquidity buffer: months of interest covered by cold storage */
        var liquidityMonths=(monthlyInt>0&&btcReserve>0)?Math.floor(btcReserve*R.BTC/monthlyInt):null;
        /* Hebel-Faktor: Gesamtschulden / Netto-Vermögen */
        var leverage=netWorthUSD>0?(aDue/netWorthUSD):null;
        /* Zinslast in % des Stack-Werts: Jahreszinsen / totalBTCvalUSD */
        var annualInt=monthlyInt*12;
        var intPctStack=totalBTCvalUSD>0?(annualInt/totalBTCvalUSD*100):null;
        /* Rebalancing to target LTV */
        var targetLTV=(cfg.ltvWarn||73)/100;
        var targetCol=R.BTC>0?aDue/(targetLTV*R.BTC):0;
        var rebalDelta=aC-targetCol;
        /* Scenario table */
        var scenarios=[
          {lbl:'\u221250%',factor:0.5,col:'#dc2626'},
          {lbl:'\u221230%',factor:0.7,col:'#ea580c'},
          {lbl:'\u221220%',factor:0.8,col:'#d97706'},
          {lbl:'\u221210%',factor:0.9,col:'#ca8a04'},
          {lbl:'Aktuell',factor:1,col:'var(--text)'},
          {lbl:'+10%',factor:1.1,col:'#16a34a'},
          {lbl:'+20%',factor:1.2,col:'#15803d'},
          {lbl:'+50%',factor:1.5,col:'#166534'}
        ];
        var scRows='<table class="loans-table"><thead><tr>'+
          '<th>Szenario</th><th>BTC-Preis</th><th>Stack-Wert</th><th>Netto-Wert</th><th>Stack-LTV</th><th>Portfolio-LTV</th>'+
        '</tr></thead><tbody>';
        scenarios.forEach(function(sc){
          var p=R.BTC*sc.factor;
          var colVal=aC*p;
          var stackVal=totalBTC*p;
          var net=stackVal-aDue;
          var sLtv=stackVal>0?(aDue/stackVal*100):0;
          var pLtv=colVal>0?(aDue/colVal*100):0;
          var netCol=net<0?'color:#dc2626':'color:#16a34a';
          scRows+='<tr'+(sc.factor===1?' style="background:var(--bg2);font-weight:700"':'')+'>'+
            '<td style="color:'+sc.col+';font-weight:600">'+sc.lbl+'</td>'+
            '<td class="amt">'+pU(Math.round(p))+'</td>'+
            '<td class="amt">'+pU(Math.round(stackVal))+'</td>'+
            '<td class="amt" style="'+netCol+'">'+pU(Math.round(net))+'</td>'+
            '<td class="amt" style="color:'+(sLtv>=95?'#dc2626':lc(sLtv))+'">'+sLtv.toFixed(1)+'%</td>'+
            '<td class="amt" style="color:'+(pLtv>=95?'#dc2626':lc(pLtv))+'">'+pLtv.toFixed(1)+'%</td>'+
          '</tr>';
        });
        scRows+='</tbody></table>';
        var html=
          '<span class="sh">Portfolio \u2014 Gesamt\u00fcbersicht</span>'+
          '<div class="mg" style="margin-bottom:1.25rem">'+
            /* Vermögen */
            tile('Gesamt-BTC',totalBTC.toFixed(8)+' \u20bf',usdGrid(CC3,totalBTCvalUSD))+
            tile('Cold Storage / Reserve',(btcReserve>0?btcReserve.toFixed(8)+' \u20bf':'—'),(btcReserve>0?usdGrid(CC3,btcReserve*R.BTC)+' ('+reserveShare.toFixed(1)+'% des Stacks)':'Bitcoin-Stack eingeben \u2193'))+
            tile('Hinterlegtes Collateral',(aC>0?aC.toFixed(8)+' \u20bf':'—'),(aC>0?usdGrid(CC3,aC*R.BTC)+' ('+collateralShare.toFixed(1)+'%)':''))+
            /* Schulden & Kosten */
            tile('Offene Schulden',(aDue>0?fmtU(aDue):'—'),(aDue>0?usdGridSub(CC4,aDue):''))+
            tile('Monatl. Zinslast',(monthlyInt>0?usdGrid(CC3,monthlyInt):'—'),'laufende Zinskosten')+
            tile('Zinslast / Stack',(intPctStack!==null?intPctStack.toFixed(2)+'% p.a.':'—'),(intPctStack!==null?'Jahreszinsen relativ zum Gesamtportfolio-Wert':''),(intPctStack!==null?(intPctStack>=10?'var(--err)':intPctStack>=5?'var(--warn)':'var(--ok)'):null))+
            /* Ergebnis */
            tile('Netto-Verm\u00f6gen',fmtU(netWorthUSD),usdGridSub(CC4,netWorthUSD),(netWorthUSD<0?'var(--err)':'var(--ok)'))+
            tile('BTC nach Schuldentilgung',
              (totalBTC>0||aDue>0?(netBTC<0?'\u2212':'')+Math.abs(netBTC).toFixed(8)+' \u20bf':'\u2014'),
              (aDue>0
                ? (netBTC>=0
                    ? 'abz\u00fcglich '+dueBTC.toFixed(8)+' \u20bf f\u00fcr die offenen Schulden'
                    : '\u26a0 Der Bestand deckt die Schulden nicht \u2014 es fehlen '+Math.abs(netBTC).toFixed(8)+' \u20bf')
                : (totalBTC>0?'Keine offenen Schulden':'')),
              (netBTC<0?'var(--err)':'var(--ok)'))+
            tile('Stack-LTV',(totalBTCvalUSD>0?stackLTV.toFixed(1)+'%':'—'),'Schulden \u00f7 Gesamtportfolio',sLtvCol)+
            tile('Hebel-Faktor',(leverage!==null?leverage.toFixed(2)+'\u00d7':'—'),(leverage!==null?(leverage<1?'\u2713 Ungehebelt':leverage<2?'Moderat':leverage<5?'Hoch':'\u26a0 Sehr hoch'):'Kein Eigenkapital'),(leverage!==null?(leverage>=5?'var(--err)':leverage>=2?'var(--warn)':'var(--ok)'):null))+
            tile('Liquidit\u00e4ts-Puffer',(liquidityMonths!==null?liquidityMonths+' Monate':'—'),(btcReserve>0?'Cold Storage deckt Zinsen':'Kein Reserve eingegeben'))+
          '</div>'+
          '<span class="sh">Risiko &amp; Handlungsbedarf</span>'+
          '<div class="mg" style="margin-bottom:1.25rem">'+
            tile('MC1-Reserve',(mc1ReserveBTC>0.000001?mc1ReserveBTC.toFixed(6)+' \u20bf':'✓ Alle MC1 sicher'),(mc1ReserveBTC>0.000001?'BTC aus Cold Storage um alle MC1 abzuwenden':''),(mc1ReserveBTC>0.000001?'var(--warn)':'var(--ok)'))+
            tile('Rebalancing',(Math.abs(rebalDelta)>0.000001?(rebalDelta>0?'\u2212'+rebalDelta.toFixed(6)+' \u20bf freigeben':'+'+Math.abs(rebalDelta).toFixed(6)+' \u20bf hinzuf\u00fcgen'):'✓ Kein Handlungsbedarf'),'f\u00fcr Ziel-LTV '+(cfg.ltvWarn||73)+'%',(rebalDelta<-0.000001?'var(--warn)':rebalDelta>0.000001?'var(--ok)':null))+
          '</div>'+
          '<span class="sh">Szenarien \u2014 BTC-Preisver\u00e4nderung</span>'+
          '<div class="card" style="margin-bottom:1.25rem;overflow-x:auto">'+scRows+'</div>'+
          '<span class="sh">Exit-Strategie</span>'+
          '<div class="card" style="margin-bottom:1.25rem">'+
            '<p class="note2" style="margin-bottom:.75rem">Bei welchem BTC-Preis kannst du alle Kredite zur\u00fcckzahlen und trotzdem eine gew\u00fcnschte Reserve behalten?</p>'+
            '<div class="fg">'+
              '<div class="ff"><label>Gew\u00fcnschte Reserve nach Exit (BTC)</label>'+
                '<input type="number" id="bk-exit-keep" min="0" step="0.001" placeholder="z.B. 0.1" oninput="d.bkCalcExit()">'+
              '</div>'+
            '</div>'+
            '<div id="bk-exit-result" style="margin-top:.75rem"></div>'+
          '</div>'+
          '<span class="sh">Transaktionen</span>'+
          '<div class="card" style="margin-bottom:1rem">'+
            '<span class="card-title">Neue Transaktion</span>'+
            '<div class="fg" style="margin-top:.75rem">'+
              '<div class="ff"><label>Datum</label><input type="date" id="bk-tx-date"></div>'+
              '<div class="ff"><label>Typ</label><select id="bk-tx-type">'+
                '<option value="buy">&#128722; Kauf</option>'+
                '<option value="sell">&#128200; Verkauf</option>'+
                '<option value="transfer_in">&#8594; Transfer eingehend</option>'+
                '<option value="transfer_out">&#8592; Transfer ausgehend</option>'+
                '<option value="mining">&#9935; Mining</option>'+
                '<option value="fee">&#128181; Geb\u00fchr</option>'+
              '</select></div>'+
              '<div class="ff"><label>BTC-Betrag</label><input type="number" id="bk-tx-btc" min="0" step="0.00000001" placeholder="0.01"></div>'+
              '<div class="ff"><label>Preis (USD/BTC)</label><input type="number" id="bk-tx-price" min="0" step="1" placeholder="optional"></div>'+
              '<div class="ff"><label>Geb\u00fchr (BTC)</label><input type="number" id="bk-tx-fee" min="0" step="0.00000001" placeholder="0.00000000"></div>'+
              '<div class="ff"><label>Notiz</label><input type="text" id="bk-tx-note" placeholder="optional"></div>'+
            '</div>'+
            '<div style="margin-top:.75rem;display:flex;gap:8px;align-items:center">'+
              '<button class="sm primary" id="bk-tx-submit-btn" onclick="d.bkTxAdd()">&#43; Hinzuf\u00fcgen</button>'+
              '<button class="sm" id="bk-tx-cancel-btn" style="display:none" onclick="d.bkTxEditCancel()">Abbrechen</button>'+
              '<input type="hidden" id="bk-tx-edit-id">'+
            '</div>'+
          '</div>'+
          (txs.length?'<div class="card" style="margin-bottom:1rem;overflow-x:auto">'+
            '<table class="loans-table"><thead><tr>'+
            '<th>Datum</th><th>Typ</th><th>BTC</th><th>Preis (USD)</th><th>Geb\u00fchr (BTC)</th><th>Notiz</th><th></th>'+
            '</tr></thead><tbody>'+
            (function(){
              var typeLabel={buy:'&#128722; Kauf',sell:'&#128200; Verkauf',transfer_in:'&#8594; Eingang',transfer_out:'&#8592; Ausgang',mining:'&#9935; Mining',fee:'&#128181; Geb\u00fchr'};
              var typeCol={buy:'var(--ok)',sell:'var(--err)',transfer_in:'var(--accent)',transfer_out:'var(--warn)',mining:'#f59e0b',fee:'var(--text4)'};
              var runBal=0;
              /* Anzeige neueste zuerst. Gespeichert bleibt chronologisch —
                 daran hängen Saldo und Import; und weil der Saldo eine Summe
                 über alle Einträge ist, ändert die Anzeigerichtung ihn nicht.
                 Bei gleichem Datum bleibt die gespeicherte Reihenfolge (der
                 Sortierer ist stabil), also etwa Collateral vor Gebühr. */
              return txs.slice().sort(function(a,b){return pD(b.date)-pD(a.date);}).map(function(t){
                var signed=(t.type==='buy'||t.type==='transfer_in'||t.type==='mining')?t.btc:-t.btc;
                runBal+=txDelta(t);
                return '<tr>'+
                  '<td>'+esc(t.date)+'</td>'+
                  '<td style="color:'+(typeCol[t.type]||'var(--text)')+'">'+(typeLabel[t.type]||esc(t.type))+'</td>'+
                  '<td class="amt" style="color:'+(typeCol[t.type]||'var(--text)')+'">'+(signed>=0?'+':'\u2212')+t.btc.toFixed(8)+'</td>'+
                  '<td class="amt">'+(t.price?''+pU(Math.round(t.price)):'—')+'</td>'+
                  '<td class="amt">'+(t.type==='fee'?'<span style="color:var(--text4)" title="Der BTC-Betrag ist bereits die Geb\u00fchr">—</span>':(t.fee?t.fee.toFixed(8):'—'))+'</td>'+
                  '<td style="font-size:11px;color:var(--text3)">'+esc(t.note)+'</td>'+
                  '<td style="white-space:nowrap">'+
                    '<button class="sm" style="padding:2px 7px;font-size:11px;margin-right:3px" data-txid="'+esc(t.id)+'" onclick="d.bkTxEdit(this.dataset.txid)">&#9998;</button>'+
                    '<button class="sm" style="padding:2px 7px;font-size:11px" data-txid="'+esc(t.id)+'" onclick="d.bkTxDel(this.dataset.txid)">×</button>'+
                  '</td>'+
                '</tr>';
              }).join('')+
              '<tr style="background:var(--bg2);font-weight:700;border-top:2px solid var(--border)">'+
              '<td colspan="2">Saldo</td>'+
              '<td class="amt" style="color:'+(runBal>=0?'var(--ok)':'var(--err)')+'">'+runBal.toFixed(8)+' \u20bf</td>'+
              '<td colspan="4"></td></tr>';
            })()+
            '</tbody></table></div>':'<p class="note2" style="margin-bottom:1.25rem">Noch keine Transaktionen erfasst.</p>')+
          '<span class="sh">Einstellungen</span>'+
          '<div class="card" style="margin-bottom:1rem">'+
            '<span class="card-title">&#8383; Bitcoin-Stack eingeben</span>'+
            '<p class="note2" style="margin-bottom:.75rem">Gib deinen Bitcoin-Bestand ausserhalb der Kredite ein (Cold Storage, Exchange etc.). Der Saldo wird automatisch aus deinen Transaktionen berechnet — du kannst ihn hier auch manuell überschreiben.</p>'+
            (cfg.btcReserveManual?'<p class="note2" style="margin-bottom:.75rem;color:var(--warn)">⚠ Manuell überschrieben — Transaktionen ändern diesen Wert nicht mehr. Ledger-Saldo: '+txBal.toFixed(8)+' ₿ <button class="sm" style="margin-left:6px" onclick="d.bkReserveAuto()">Aus Transaktionen übernehmen</button></p>':'')+
            (txBal<0?'<p class="note2" style="margin-bottom:.75rem;color:var(--err)">⚠ Der Ledger-Saldo ist negativ ('+txBal.toFixed(8)+' ₿) — bitte die erfassten Transaktionen prüfen.</p>':'')+
            '<div class="fg" style="margin-top:.75rem">'+
              '<div class="ff">'+
                '<label>Bitcoin ausserhalb Kredite (BTC)</label>'+
                '<input type="number" id="bk-reserve" step="0.00000001" placeholder="z.B. 0.5" value="'+(cfg.btcReserve||'')+'">'+
              '</div>'+
            '</div>'+
            '<div style="margin-top:.75rem">'+
              '<button class="sm primary" onclick="d.bkSave()">&#10003; Speichern</button>'+
              '<span id="bk-save-msg" style="display:none;margin-left:.75rem;font-size:12px;color:var(--ok)">&#10003; Gespeichert.</span>'+
            '</div>'+
          '</div>';
        el.innerHTML=html;
        d.bkCalcExit();
      ...
      bkCalcExit:function(){
        var el=g('bk-exit-result');if(!el)return;
        var act=loans.filter(function(l){return l.status==='active';});
        var aDue=act.reduce(function(s,l){return s+toU(l.amount,l.c)+intU(l);},0);
        var keepEl=g('bk-exit-keep');
        var keep=keepEl?parseFloat(keepEl.value)||0:0;
        var btcReserve=cfg.btcReserve||0;
        var aC=act.reduce(function(s,l){return s+l.col;},0);
        var totalBTC=btcReserve+aC;
        var sellable=totalBTC-keep;
        if(sellable<=0.000001){
          el.innerHTML='<span style="color:var(--warn)">Reserve-Wunsch \u00fcbersteigt Gesamt-BTC ('+totalBTC.toFixed(6)+' \u20bf).</span>';
          return;
        }
        if(aDue<=0){el.innerHTML='<span style="color:var(--ok)">\u2713 Keine offenen Schulden.</span>';return;}
        var exitP=aDue/sellable;
        var distPct=((R.BTC-exitP)/R.BTC*100);
        var col=exitP>R.BTC?'var(--err)':distPct<10?'var(--warn)':'var(--ok)';
        el.innerHTML=
          '<div class="mg">'+
            '<div class="mc"><span class="mc-lbl">Ben\u00f6tigter Exit-Preis</span>'+
            '<span class="mc-val" style="color:'+col+'">'+pU(Math.round(exitP))+'</span>'+
            '<span class="mc-sub">'+(exitP>R.BTC?'\u26a0 \u00dcber aktuellem Kurs':distPct.toFixed(1)+'% Abstand zum aktuellen Kurs')+'</span></div>'+
            '<div class="mc"><span class="mc-lbl">Zu verkaufende BTC</span>'+
            '<span class="mc-val">'+sellable.toFixed(6)+' \u20bf</span>'+
            '<span class="mc-sub">\u2248 '+pU(Math.round(sellable*exitP))+'</span></div>'+
            '<div class="mc"><span class="mc-lbl">Verbleibende Reserve</span>'+
            '<span class="mc-val">'+keep.toFixed(6)+' \u20bf</span>'+
            '<span class="mc-sub">\u2248 '+pU(Math.round(keep*exitP))+' bei Exit-Preis</span></div>'+
          '</div>';
      },
      bkSave:function(){
        var v=parseFloat(g('bk-reserve').value)||0;
        cfg.btcReserve=v;
        cfg.btcReserveManual=Math.abs(v-d.bkTxBalance())>1e-8;
        saveSettings(cfg);
        var msg=g('bk-save-msg');
        if(msg){msg.style.display='inline';setTimeout(function(){msg.style.display='none';},2000);}
        d.bk();
      },
      bkTxAdd:function(){
        var date=g('bk-tx-date').value;
        var type=g('bk-tx-type').value;
        var btc=parseFloat(g('bk-tx-btc').value);
        var price=parseFloat(g('bk-tx-price').value)||0;
        var fee=parseFloat(g('bk-tx-fee').value)||0;
        var note=(g('bk-tx-note').value||'').trim();
        if(!date||!type||!btc||btc<=0){alert('Datum, Typ und BTC-Betrag sind Pflichtfelder.');return;}
        var eidEl=g('bk-tx-edit-id');var editId=eidEl?eidEl.value:'';
        if(editId){
          var tx=txs.find(function(t){return t.id===editId;});
          if(tx){tx.date=date;tx.type=type;tx.btc=btc;tx.price=price;tx.fee=fee;tx.note=note;}
        } else {
          txs.push({id:'tx'+new Date().getTime()+Math.random().toString(36).slice(2,6),date:date,type:type,btc:btc,price:price,fee:fee,note:note});
        }
        txs.sort(function(a,b){return pD(a.date)-pD(b.date);});
        saveTxs();
        d.bkRecalcReserve();
      ...
      bkTxEdit:function(id){
        var tx=txs.find(function(t){return t.id===id;});
        if(!tx)return;
        var f=g('bk-tx-date');if(f)f.value=tx.date;
        var ty=g('bk-tx-type');if(ty)ty.value=tx.type;
        var b=g('bk-tx-btc');if(b)b.value=tx.btc;
        var p=g('bk-tx-price');if(p)p.value=tx.price||'';
        var fe=g('bk-tx-fee');if(fe)fe.value=tx.fee||'';
        var n=g('bk-tx-note');if(n)n.value=tx.note||'';
        var eid=g('bk-tx-edit-id');if(eid)eid.value=id;
        var btn=g('bk-tx-submit-btn');if(btn)btn.textContent='✓ Speichern';
        var cancel=g('bk-tx-cancel-btn');if(cancel)cancel.style.display='inline';
        var form=g('bk-tx-date');if(form)form.scrollIntoView({behavior:'smooth',block:'center'});
      },
      bkTxEditCancel:function(){
        var eid=g('bk-tx-edit-id');if(eid)eid.value='';
        var btn=g('bk-tx-submit-btn');if(btn)btn.textContent='+ Hinzufügen';
        var cancel=g('bk-tx-cancel-btn');if(cancel)cancel.style.display='none';
        ['bk-tx-date','bk-tx-btc','bk-tx-price','bk-tx-fee','bk-tx-note'].forEach(function(id){var el=g(id);if(el)el.value='';});
        var ty=g('bk-tx-type');if(ty)ty.selectedIndex=0;
      },
      bkTxDel:function(id){
      ...
      bkRecalcReserve:function(){
        /* A manually saved reserve wins — the card promises it can be
           overridden, so recording a transaction must not silently reset it. */
        if(cfg.btcReserveManual)return;
        cfg.btcReserve=d.bkTxBalance();
        saveSettings(cfg);
        d.abl();
      },
      /* Ledger balance, unclamped — a negative result is a data-entry error the
         user needs to see, not something to hide behind Math.max(0,…). */
      bkTxBalance:function(){
        return parseFloat(txs.reduce(function(s,t){return s+txDelta(t);},0).toFixed(8));
      },
      bkReserveAuto:function(){
        cfg.btcReserveManual=false;
        cfg.btcReserve=d.bkTxBalance();
        saveSettings(cfg);
        d.bk();
      },
      seBackupSave:function(){
        var days=parseInt(g('se-backup').value);
        if(isNaN(days)||days<0||days>365){alert('Bitte eine Zahl zwischen 0 und 365 eingeben (0 = aus).');return;}
        cfg.backupDays=days;
        saveSettings(cfg);
        d.renderBackupHint();
        var msg=g('se-backup-msg');if(msg){msg.style.display='inline';setTimeout(function(){msg.style.display='none';},2000);}
      },
      ...
      renderCsw:function(){
        ...
        if(!dCcy)dCcy=mainC();
        var cur=dCcy;if(vis.indexOf(cur)<0){cur=vis.indexOf(mainC())>=0?mainC():vis[0];dCcy=cur;}
      ...
      tl:function(){
        ...
        g('tl-list').innerHTML='<div class="tl">'+sorted.map(function(l){
          ...
          return '<div class="ti"><div class="dot '+dot+'"></div>'+
            '<span class="tdate">'+pD(l.start).toLocaleDateString('de-CH')+' → '+end+'</span>'+
            '<span class="tlabel">'+esc(l.name)+' <span class="badge '+(l.status==='active'?'ba':'bc')+'" style="margin-left:6px">'+(l.status==='active'?'Aktiv':'Abgeschlossen')+'</span></span>'+
            '<span class="tsub amt">'+fmt(l.amount,l.c)+' · '+l.rate+'% · '+l.term+' Mo'+(l.status==='active'&&dl>0?' · noch <b>'+dl+' Tage</b>':'')+'</span>'+
            '<span class="note" style="margin-top:2px">= '+fmtU(lU)+' / '+fmt(frU(lU,'CHF'),'CHF')+' / '+fmt(frU(lU,'EUR'),'EUR')+'</span></div>';
        }).join('')+'</div>';
        ...
        setTimeout(function(){
          ...
          if(c2&&act.length)chI=new Chart(c2,{type:'bar',
            ...
            options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{ticks:{color:chTextColor,callback:function(v){return ''+pU(v);}},grid:{color:chGridColor}},x:{ticks:{color:chTextColor},grid:{display:false}}}}
          });
        },120);
      }
    };
    ...
    function init(){
      ...
      flushNotices();
      /* Safe to persist unconditionally: storeWrite() refuses to write while
         the stored data is unreadable, so this cannot destroy a backup. */
      syncStatus();save();
      pills();d.renderCsw();d.checkAlarms();d.extPopulate();d.vorInit();d.plInit();d.ov();
      d.backupRemind();
      /* Re-render active tab now that Chart.js is available — fixes tabs that bail early on typeof Chart */
      (function(){
        var at=document.querySelector('#ffd-root .tab.on, #ffd-root .nav-item.on');
        if(at){var m=at.getAttribute('onclick').match(/'([^']+)'/);if(m){var t=m[1];var fn={ch:d.ch,st:d.stress,tl:d.tl,sx:d.sx,lo:d.loans,ca:d.cal};if(fn[t]){try{fn[t]();}catch(e){console.error('[init] Tab-Render:',e);}}}}
      })();
      d.refreshBtc();
    }
    ...
    d.refreshBtc=async function(){
      ...
      try{
        try{
          ...
          var res=await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd,eur,chf,pln&include_24hr_change=true',ctrl?{signal:ctrl.signal}:undefined);
          if(killer)clearTimeout(killer);
          var data=await res.json();
          if(data&&data.bitcoin&&data.bitcoin.usd){
            /* Discard the response if the user entered a rate while it was in
               flight — otherwise their input is overwritten seconds later. */
            if(manualRateAt>startedAt){
              /* Keep the fetched price available for the live-rate button and
                 the rate-limit banner, just don't overwrite the manual entry */
              liveBtc=data.bitcoin.usd;
              console.info('[CoinGecko] Kursübernahme verworfen — manueller Kurs ist neuer');
            } else {
            ...
            console.warn('[CoinGecko] Unerwartete Antwortstruktur — kein bitcoin.usd');
          }
        }catch(e){console.error('[CoinGecko] Fetch-Fehler:', e);}
        if(!liveBtc){showCgRateLimit();}else{hideCgRateLimit();}
        if(!d._prefetched){d._prefetched=true;setTimeout(function(){d.prefetchHistPrices();},3000);}
        pills();d.ov();d.checkAlarms();d.vorFillAll();d.nachPopulate();d.nlpPopulate();d.se2Populate();d.gvlPopulate();d.abl();d.updateHdrStats();
        renderActiveTab();
      } finally {
        /* Must run even if a re-render throws, or the button stays disabled */
    ...
    if(typeof Chart!=='undefined'){
      ...
      /* Without this the whole of init() — dark mode, sidebar state, the tab
         from the URL and, most importantly, the exchange-rate fetch — never ran
         when the CDN was unreachable, and nothing told the user why every
         figure was based on the built-in fallback rates. */
      s.onerror=function(){
        console.warn('[init] Chart.js konnte nicht geladen werden — Diagramme sind nicht verfügbar');
        ffdNotice('Chart.js konnte nicht geladen werden (kein Internet oder CDN blockiert). Diagramme werden nicht angezeigt; alle übrigen Funktionen stehen zur Verfügung.');
        init();
      };
    ...
    (function(){
      var validTabs=['ov','lo','to','st','ca','tl','ch','sx','se','ro','bk'];
